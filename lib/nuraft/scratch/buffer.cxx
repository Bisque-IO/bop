...
ptr<buffer> buffer::alloc(const size_t size) {
    ...
    if (size >= 0x80000000) {
        throw std::out_of_range("size exceed the max size that nuraft::buffer could support");
    }
    ...
    ptr<buffer> buf(reinterpret_cast<buffer*>((char*)mi_malloc(len)), &free_buffer);
...
size_t buffer::container_size() const {
    return mi_malloc_usable_size(this);
