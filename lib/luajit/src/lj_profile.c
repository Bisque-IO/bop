...
#if LJ_PROFILE_CONCURRENT
#if LJ_PROFILE_WTHREAD
...
#if !LJ_PROFILE_SIGPROF || LJ_PROFILE_CONCURRENT
void LJ_FASTCALL lj_profile_hook_enter(global_State *g)
{
#if LJ_PROFILE_CONCURRENT
  ProfileState *ps = &g->profile_state;
#else
...
{
#if LJ_PROFILE_CONCURRENT
  ProfileState *ps = &G(L)->profile_state;
#else
  ProfileState *ps = &profile_state;
#endif
  global_State *g = G(L);
...
LUA_API void luaJIT_profile_trigger(lua_State *L) {
#if LJ_PROFILE_CONCURRENT
  profile_trigger(&G(L)->profile_state);
#endif
}
				  ...
				  luaJIT_profile_callback cb, void *data)
{
  ...
  ps->samples = 0;
  lj_buf_init(L, &ps->sb);
#if !defined (LJ_PROFILE_CONCURRENT) || LJ_PROFILE_CONCURRENT == 0
