...
local root_dir = os.projectdir() .. "/lib/luajit/"
...
rule("dasc")
    ...
    before_build_file(function(target, sourcefile)
        ...
        local argv = {root_dir .. "dynasm/dynasm.lua", "-LN"}
...
rule("buildvm")
    before_build_files(function (target, sourcebatch)
        ...
        os.vrunv(buildvm_bin, {"-m", "bcdef", "-o", root_dir .. "src/lj_bcdef.h", unpack(sourcefiles)})
        os.vrunv(buildvm_bin, {"-m", "ffdef", "-o", root_dir .. "src/lj_ffdef.h", unpack(sourcefiles)})
        os.vrunv(buildvm_bin, {"-m", "libdef", "-o", root_dir .. "src/lj_libdef.h", unpack(sourcefiles)})
        os.vrunv(buildvm_bin, {"-m", "recdef", "-o", root_dir .. "src/lj_recdef.h", unpack(sourcefiles)})
        os.vrunv(buildvm_bin, {"-m", "vmdef", "-o", root_dir .. "src/lj_vmdef.h", unpack(sourcefiles)})
        os.vrunv(buildvm_bin, {"-m", "folddef", "-o", root_dir .. "src/lj_folddef.h", root_dir .. "src/lj_opt_fold.c"})
        if is_plat("windows", "mingw") then
...
target("buildvm")
    ...
    add_defines("LUAJIT_USE_SYSMALLOC=1")
    if is_host("windows") then
        add_defines("_CRT_SECURE_NO_DEPRECATE", "_CRT_STDIO_INLINE=__declspec(dllexport)__inline")
    end
    ...
    if is_plat("macosx", "iphoneos", "watchos") then
        ...
        if is_plat("iphoneos", "watchos") then 
            add_defines("TARGET_OS_IPHONE", {public = true})
        end
...
target("luajit")
    ...
    set_languages("c11")
    set_optimize("fastest")
    add_defines("LUAJIT_ENABLE_CHECKHOOK=1", {public = true})
    add_defines("LUAJIT_USE_SYSMALLOC=1", {public = true})
    add_defines("LUAJIT_ENABLE_LUA52COMPAT", {public = true})
