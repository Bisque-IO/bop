...
template<auto N>
constexpr auto logger_clean_source_file(const char (&src)[N]) {
  std::string_view s(src);
  // std::string_view ss(char[256]);

#if defined(_WIN32)
  auto index = s.rfind(std::string_view("\\bisque\\src\\"));
  if (index != std::string_view::npos) {
    s.remove_prefix(index + std::string_view("\\bisque\\src\\").size());
    return s.data();
  }
  index = s.rfind(std::string_view("\\bisque\\lib\\"));
  if (index != std::string_view::npos) {
    s.remove_prefix(index + std::string_view("\\bisque\\lib\\").size());
    return s.data();
  }
  index = s.rfind(std::string_view("\\bisque\\server\\"));
  if (index != std::string_view::npos) {
    s.remove_prefix(index + std::string_view("\\bisque\\").size());
    return s.data();
  }
  index = s.rfind(std::string_view("\\bisque\\test\\"));
  if (index != std::string_view::npos) {
    s.remove_prefix(index + std::string_view("\\bisque\\").size());
    return s.data();
  }
  index = s.rfind(std::string_view("src\\"));
  if (index != std::string_view::npos) {
    s.remove_prefix(index + std::string_view("src\\").size());
    return s.data();
  }
  index = s.rfind(std::string_view("lib\\"));
  if (index != std::string_view::npos) {
    s.remove_prefix(index + std::string_view("lib\\").size());
    return s.data();
  }
#else
  auto index = s.rfind(std::string_view("bisque/src/"));
  ...
  index = s.rfind(std::string_view("bisque/test/"));
  if (index != std::string_view::npos) {
    s.remove_prefix(index + std::string_view("bisque/").size());
    return s.data();
  }
...
inline void init_log(
  Severity min_severity, const std::string& filename = "", bool async = true,
  bool enable_console = true, size_t max_file_size = 0, size_t max_files = 0,
  bool flush_every_time = false
) {
  logger<Id>::instance().init(
    min_severity, async, enable_console, filename, max_file_size, max_files,
    flush_every_time
  );
}
...
#define ELOG_IMPL(severity, Id, ...)                                           \
  if (!easylog::logger<Id>::instance().check_severity(severity)) {             \
    ...
    easylog::logger<Id>::instance() +=                                         \
      easylog::record_t(                                                       \
        std::chrono::system_clock::now(), severity, "",                        \
        easylog::const_source_file_path(__FILE__).location(                    \
          __LINE__, __func__                                                   \
        ),                                                                     \
        ""                                                                     \
      )                                                                        \
