...
class appender {
  ...
  void write_record(record_t& record) {
    ...
    auto time_str    = std::string_view(buf, 31);
    auto tid_str     = get_tid_buf(record.get_tid());
    auto name        = record.get_name();
    auto file_str    = record.get_file_str();
    auto file_path   = record.get_file_path();
    auto line_number = record.get_line_number();
    auto func        = record.get_func();
    ...
    if (has_init_) {
      ...
      write_file(":");
      char c[22];
      auto cr = std::to_chars(c, c + 22, line_number);
      write_file(std::string_view{c, (std::size_t)(cr.ptr - c)});
      write_file(" ] ");
      // write_file(file_str);
      write_file(msg);
      write_file("\n");
    ...
    if constexpr (enable_console) {
      std::cout << time_str.substr(0, 23) << " ";
      add_color(record.get_severity());
      std::cout << time_str.substr(23, time_str.size() - 1);
      ...
      std::cout << " " << tid_str;
      if (!name.empty()) {
        // if (!record.get_module_path().empty()) {
        //   std::cout << "[" << record.get_module_path() << "/" << name << "]
        //   ";
        // } else {
        // }
        std::cout << "[" << name << "] ";
      ...
      std::cout << "[" << file_path << ":" << line_number << "] ";


      if (!record.get_func().empty()) {
        std::cout << "[" << record.get_func() << "] ";
      }
#ifndef _WIN32
  ...
  void add_color(Severity severity) {
    ...
    //   std::cout << "\x1B[44m\x1B[34m\x1B[1m";
    if (severity == Severity::INFO)
      std::cout << "\x1B[97m\x1B[40m";
    if (severity == Severity::TRACE)
      std::cout << "\x1B[37m";
    if (severity == Severity::WARN)
      std::cout << "\x1B[37m\x1B[43m\x1B[1m";
    if (severity == Severity::ERROR)
      std::cout << "\x1B[37m\x1B[41m\x1B[1m";
    if (severity == Severity::CRITICAL)
...
#elif __APPLE__
#else
    if (severity >= Severity::WARN || severity == Severity::TRACE ||
