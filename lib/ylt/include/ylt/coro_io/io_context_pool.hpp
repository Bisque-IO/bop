...
class io_context_pool {
 public:
  ...
  explicit io_context_pool(std::size_t pool_size, bool cpu_affinity = false)
    ...
    for (std::size_t i = 0; i < pool_size; ++i) {
      ...
      work_ptr work(new asio::executor_work_guard<asio::any_io_executor>(io_context->get_executor()));
 ...
 private:
  ...
  using work_ptr = std::shared_ptr<asio::executor_work_guard<asio::any_io_executor>>;
...
class multithread_context_pool {
 public:
  multithread_context_pool(size_t thd_num = std::thread::hardware_concurrency())
      : work_(std::make_unique<asio::executor_work_guard<asio::any_io_executor>>(ioc_.get_executor())),
 ...
 private:
  ...
  std::unique_ptr<asio::executor_work_guard<asio::any_io_executor>> work_;
