...
/**
 * @see [Event Meanings](https://dev.mysql.com/doc/internals/en/event-meanings.html) for the original
 * documentation.
 */
enum class EventType(val eventNumber: Int) {
    ...
    * (In MySQL 4.0 and 4.1, this event is written only to the first binary log file
    * that the server creates after startup.) This event is used in MySQL 3.23 through
    * 4.1 and superseded in MySQL 5.0 by [.FORMAT_DESCRIPTION].
    */
   START_V3(1),
    ...
    * events for the statement. This is written only before a [.QUERY] and is not used in case of RBR.
    */
   INTVAR(5),
    ...
    * before a [.QUERY] and is not used in case of RBR.
    */
   RAND(13),

   /**
    * Written every time a statement uses a user variable; precedes other events for the statement. Indicates the
    * value to use for the user variable in the next statement. This is written only before a [.QUERY] and
   ...
   /**
    * A descriptor event that is written to the beginning of the each binary log file.
    * This event is used as of MySQL 5.0; it supersedes [.START_V3].
    */
    ...
    * Normal transactions are implemented by sending a [.QUERY] containing a BEGIN statement and a [.QUERY]
    ...
    *
    * @see [Replication Protocol](https://mariadb.com/kb/en/replication-protocol/) for the original doc.
    */
   ANNOTATE_ROWS(160),  //
   ...
   companion object {
      private val TYPE_BY_EVENT_NUMBER: Array<EventType> = buildEventTypeIndex()
      private fun buildEventTypeIndex(): Array<EventType> {
         val max = entries.maxOfOrNull { it.eventNumber }!!
         val index = Array(max + 1) {
            UNKNOWN
         }
         entries.forEach { index[it.eventNumber] = it }
         return index
      }

      fun isRowMutation(eventType: EventType): Boolean {
         return isWrite(eventType) || isUpdate(eventType) || isDelete(eventType)
      }

      fun isWrite(eventType: EventType): Boolean {
         return eventType == PRE_GA_WRITE_ROWS || eventType == WRITE_ROWS || eventType == EXT_WRITE_ROWS
      }

      fun isUpdate(eventType: EventType): Boolean {
         return eventType == PRE_GA_UPDATE_ROWS || eventType == UPDATE_ROWS || eventType == EXT_UPDATE_ROWS
      }

      fun isDelete(eventType: EventType): Boolean {
         return eventType == PRE_GA_DELETE_ROWS || eventType == DELETE_ROWS || eventType == EXT_DELETE_ROWS
      }

      @JvmStatic
      fun byEventNumber(num: Int): EventType? {
         return if (num >= 0 && num < TYPE_BY_EVENT_NUMBER.size) TYPE_BY_EVENT_NUMBER[num] else null
      }
   }
}
