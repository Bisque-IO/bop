...
/*
A bounded SPSC queue. Single-Producer Single-Consumer
...
*/
@(private)
spsc_next_queue_pos :: #force_inline proc "contextless" (
	q: ^SPSC($T, $SIZE),
	queue_pos: u64,
) -> u64 {
	...
	for {
		...
		if stamp_delta == 0 {
			...
			if atomic_load(&q.waiter, .Relaxed) == 0 {
				if _, ok := cas_weak(&q.waiter, 0, 1, .Acquire, .Relaxed); ok {
