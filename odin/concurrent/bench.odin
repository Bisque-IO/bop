...
bench_parallelism :: proc($THREADS: int, $QUEUE_SIZE: int, $ITERS: int) {
	...
	fmt.println(
		...
		"/op",
		"   ",
		u64(time.Second) / u64(time.stopwatch_duration(stopwatch) / time.Duration(ITERS * THREADS)),
		"/sec",
	)
}
	...
	for i in 0 ..< 1 {
		producers[i] = thread.create_and_start_with_poly_data2(
			...
			proc(q: ^bop.MPMC_Blocking, id: int) {
				...
				items: [BATCH_SIZE]u64
				for i in 0 ..< ITERS_BULK {
					for !mpmc_blocking_enqueue_bulk_token(
						    token,
						    raw_data(items[0:BATCH_SIZE]),
						    BATCH_SIZE,
					    ) {
						time.sleep(time.Nanosecond)
					}
