...
when ODIN_OS == .Linux && ODIN_ARCH == .amd64 {
    ...
    foreign import wasmtimelib {
        "lib/linux_arm64/libwasmtime.a",
    }
} else when ODIN_OS == .Linux && ODIN_ARCH == .riscv64 {
    foreign import wasmtimelib {
        "lib/linux_riscv64/libwasmtime.a",
    }
} else when ODIN_OS == .Darwin && ODIN_ARCH == .arm64 {
    foreign import wasmtimelib {
        "lib/macos_arm64/libwasmtime.a",
    }
} else when ODIN_OS == .Darwin && ODIN_ARCH == .amd64 {
    foreign import wasmtimelib {
        "lib/macos_amd64/libwasmtime.a",
    }
...
foreign wasmtimelib {
     ...
     * existence).
     */
    func_new_unchecked :: proc(
    	store: ^Context,
     	type: ^Func_Type,
      	callback: Func_Unchecked_Callback,
       	env: rawptr,
        finalizer: Finalizer,
        ret: ^Func,
    ) ---
     ...
     */
    func_call_unchecked :: proc(
    	store: ^Context,
    	func: ^Func,
     	args_and_results: [^]Val,
      	num_args_and_results: c.size_t,
      	trap: ^^Trap,
    ) -> ^Error ---
