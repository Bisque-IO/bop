...
when ODIN_OS == .Linux && ODIN_ARCH == .amd64 {
    ...
    foreign import wasmtimelib {
        "lib/linux_arm64/libwasmtime-min.a",
    }
} else when ODIN_OS == .Linux && ODIN_ARCH == .riscv64 {
    foreign import wasmtimelib {
        "lib/linux_riscv64/libwasmtime-min.a",
    }
} else when ODIN_OS == .Darwin && ODIN_ARCH == .arm64 {
    foreign import wasmtimelib {
        "lib/macos_arm64/libwasmtime-min.a",
    }
} else when ODIN_OS == .Darwin && ODIN_ARCH == .amd64 {
    foreign import wasmtimelib {
        "lib/macos_amd64/libwasmtime-min.a",
    }
...
foreign wasmtimelib {
	...
	wasmtime_config_wasm_gc_set :: proc(config: wasm_config, enabled: bool) ---
	...
	wasmtime_module_new :: proc(engine: wasm_engine, wasm: [^]byte, wasm_len: size_t, ret: ^wasmtime_module) -> wasmtime_error ---
