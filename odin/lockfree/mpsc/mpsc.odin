...
Queue :: struct($T: typeid, $SIZE: u64) where SIZE > 0 && (SIZE & (SIZE - 1)) == 0 {
...
queue_make :: proc($T: typeid, $SIZE: u64, allocator := context.allocator) -> ^Queue(T, SIZE) {
    #assert(SIZE > 0 && (SIZE & (SIZE - 1)) == 0, "must be power of 2")
    #assert(SIZE < 1024 * 1024 * 128, "max queue size exceeded")

    q := new(Queue(T, SIZE), allocator)
    q.allocator = allocator

    for i := 0; i < len(q.buffer); i += 1 {
        intrinsics.atomic_store(&q.buffer[i].stamp, u64(i))
    }

    //	q.closed_channel_mask = u64(math.next_power_of_two(SIZE))
    //	q.right_mask, _ = intrinsics.overflow_sub(q.closed_channel_mask << 1, 1)

    return q
}

destroy :: proc(q: ^Queue($T, $SIZE)) {
    assert(q != nil)
    if !is_closed(q) do close(q)
    free(q, q.allocator)
}

destroy_with_deleter :: proc(
    q: ^Queue($T, $SIZE),
    user_data: rawptr,
...
// should be cleared.
@(private)
next_queue_pos :: #force_inline proc "contextless" (q: ^Queue($T, $SIZE), queue_pos: u64) -> u64 {
// Bit mask covering both the buffer index and the 1-bit flag.
    ...
    sequence_count, _ = intrinsics.overflow_add(sequence_count, SEQUENCE_INCR)
...
push :: proc(q: ^Queue($T, $SIZE), value: T) -> Push_Error #no_bounds_check {
    assert(q != nil)
    ...
    enqueue_pos := intrinsics.atomic_load_explicit(&q.enqueue_pos, .Relaxed)
    ...
    for {
        ...
        stamp_delta, _ := intrinsics.overflow_sub(stamp, enqueue_pos)

        if stamp_delta == 0 {
            // The enqueue position matches the stamp: a push can be attempted.
            if pos, ok := intrinsics.atomic_compare_exchange_weak_explicit(
                &q.enqueue_pos,
                enqueue_pos,
                next_queue_pos(q, enqueue_pos),
                .Relaxed,
                .Relaxed,
            ); ok {
                slot.value = value
                stamp, _ = intrinsics.overflow_add(stamp, 1)
...
pop :: #force_inline proc(q: ^Queue($T, $SIZE)) -> (value: T, err: Pop_Error) #no_bounds_check {
    assert(q != nil)
    ...
    if dequeue_pos != stamp {
        q.dequeue_pos = next_queue_pos(q, dequeue_pos)

        // Read the value from the slot and set the stamp to the value of
        // the dequeue position increased by one sequence increment.
        value = slot.value

        incr, _ := intrinsics.overflow_add(stamp, RIGHT_MASK)
        intrinsics.atomic_store_explicit(&slot.stamp, incr, .Release)

        return value, .Success
    }

    if intrinsics.atomic_load_explicit(&q.enqueue_pos, .Relaxed) ==
    (dequeue_pos | CLOSED_CHANNEL_MASK) {
        err = .Closed
    } else {
        err = .Empty
    }
...
pop_wait :: proc(q: ^Queue($T, $SIZE)) -> (value: T, err: Pop_Error) {
    assert(q != nil)
    ...
    for i in 0 ..< SPINS {
        #unroll for _ in 0 ..< CPU_RELAX_SPINS do intrinsics.cpu_relax()
    ...
    for {
        ...
        // reset waiter
        _ = intrinsics.atomic_exchange_explicit(&q.waiter, 0, .Release)
...
// Closes the queue
close :: #force_inline proc(q: ^Queue($T, $SIZE)) {
    assert(q != nil)
    ...
    // writes.
    intrinsics.atomic_or_explicit(&q.enqueue_pos, CLOSED_CHANNEL_MASK, .Relaxed)
}
...
is_closed :: #force_inline proc(q: ^Queue($T, $SIZE)) -> bool {
    assert(q != nil)
    ...
    return intrinsics.atomic_load_explicit(&q.enqueue_pos, .Relaxed) & CLOSED_CHANNEL_MASK != 0
}
