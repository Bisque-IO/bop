...
Slot :: struct {
    ...
    index:             u64,
    flags:             Slot_Flags,
    _:                 [CACHE_LINE_SIZE - size_of(u8) - size_of(u64) - size_of(^int) - size_of(^int)]u8,
    cpu_time:          u64,
...
slot_schedule :: #force_inline proc "contextless" (self: ^Slot) -> bool {
    ...
    if (intrinsics.atomic_load_explicit(&self.flags, .Acquire) & Slot_Flags.Scheduled) != Slot_Flags.Idle {
        return false
    }

    // Try to flip schedule bit. Using relaxed semantics is ok since happens after an acquire load.
    previous_flags := intrinsics.atomic_or_explicit(&self.flags, Slot_Flags.Scheduled, .Relaxed)

    // Did we win the race to schedule?
    not_scheduled_nor_executing := (previous_flags & (Slot_Flags.Scheduled | Slot_Flags.Executing)) == Slot_Flags.Idle
...
slot_execute :: #force_inline proc(self: ^Slot) {
    ...
    intrinsics.atomic_store_explicit(&self.flags, Slot_Flags.Executing, .Release)
    self.counter += 1
    ...
    if result == Slot_Flags.Scheduled {
        intrinsics.atomic_store_explicit(&self.flags, Slot_Flags.Scheduled, .Release)
        if was_empty, was_set := signal_set(self.signal, self.index); was_empty && was_set {
            // Increment the waker
            waker_incr(self.waker)
        }
