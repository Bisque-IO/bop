...
// Signals may be composed into Signal Groups to take advantage of SIMD.
Signal :: ^u64
...
signal_nearest :: #force_inline proc "contextless" (value: u64, signal_index: u64) -> u64 {
	up := ctz(value >> signal_index)
	down := ctz(reverse_bits(value << (63 - signal_index)))
...
signal_nearest_branchless :: #force_inline proc "contextless" (
	value: u64,
	signal_index: u64,
) -> u64 {
	up := ctz(value >> signal_index)
...
signal_set :: #force_inline proc "contextless" (
	signal: Signal,
	index: u64,
) -> (
	was_empty: bool,
	...
	bit := u64(1) << index
	prev := atomic_or(signal, bit, .Acq_Rel)
...
signal_group_reserve :: #force_inline proc "contextless" (
	sg: ^Signal_Group,
	worker: proc(),
) -> ^Slot {
	...
	counter := atomic_add(&sg.counter, 1, .Relaxed) - 1
