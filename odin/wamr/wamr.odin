...
FunctionInstance :: wasm_function_inst_t
...
load :: proc(buf: []byte) -> (Module, bool) {
	mod := wasm_runtime_load(&buf[0], u32(len(buf)), &ErrorBuffer[0], len(ErrorBuffer))
	return mod, mod != nil
}
...
instantiate :: proc(mod: Module, stack_size, heap_size: u32) -> (ModuleInstance, bool) {
	inst := wasm_runtime_instantiate(mod, stack_size, heap_size, &ErrorBuffer[0], len(ErrorBuffer))
...
create_exec_env :: proc(inst: ModuleInstance, stack_size: u32) -> (ExecEnv, bool) {
	env := wasm_runtime_create_exec_env(inst, stack_size)
	return env, env != nil
}
...
lookup_function :: proc(inst: ModuleInstance, name: string) -> (FunctionInstance, bool) {
	func := wasm_runtime_lookup_function(inst, strings.unsafe_string_to_cstring(name))
...
call_wasm :: proc(
	...
	when RESULTCOUNT == 0 {
		func_ok := wasm_runtime_call_wasm_a(env, func, u32(RESULTCOUNT), &_dummy_val, num_args, arg_ptr)
		return {}, func_ok
	} else {
		results: [RESULTCOUNT]Value
		func_ok := wasm_runtime_call_wasm_a(env, func, u32(RESULTCOUNT), &results[0], num_args, arg_ptr)
...
module_malloc :: proc(inst: ModuleInstance, size: uint, backing: rawptr) -> (WasmPointer, bool) {
	ptr := wasm_runtime_module_malloc(inst, u64(size), backing)
	return WasmPointer(ptr), ptr != 0
}
module_free :: proc(inst: ModuleInstance, ptr: WasmPointer) {
	wasm_runtime_module_free(inst, u64(ptr))
}
