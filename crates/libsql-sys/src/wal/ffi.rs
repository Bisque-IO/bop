...
use maniac_libsql_ffi::{
    Error, PgHdr, SQLITE_CHECKPOINT_FULL, SQLITE_CHECKPOINT_PASSIVE, SQLITE_CHECKPOINT_RESTART,
    SQLITE_CHECKPOINT_TRUNCATE, SQLITE_OK, WAL_SAVEPOINT_NDATA, libsql_wal, libsql_wal_manager,
    libsql_wal_methods, sqlite3, sqlite3_file, sqlite3_vfs, wal_impl, wal_manager_impl,
};
...
pub unsafe extern "C" fn open<T: WalManager>(
    ...
    let this = unsafe { &*(wal_manager as *mut T) };
    let mut vfs = Vfs { vfs };
    ...
    match this.open(
        ...
        Ok(wal) => {
            ...
            unsafe { *out_wal = construct_libsql_wal(wal) };
            SQLITE_OK
        }
...
pub unsafe extern "C" fn close<T: WalManager>(
    ...
    let this = unsafe { &*(wal_manager as *mut T) };
    let mut wal = unsafe { Box::from_raw(wal as *mut T::Wal) };
    let scratch = if z_buf.is_null() {
        None
    } else {
        Some(unsafe { std::slice::from_raw_parts_mut(z_buf, n_buf as usize) })
    };
...
pub unsafe extern "C" fn destroy_wal_manager<T: WalManager>(wal_manager: *mut wal_manager_impl) {
    let this = unsafe { Box::from_raw(wal_manager as *mut T) };
    this.destroy();
}

pub unsafe extern "C" fn limit<T: Wal>(wal: *mut wal_impl, limit: i64) {
    let this = unsafe { &mut (*(wal as *mut T)) };
    this.limit(limit);
}
...
pub unsafe extern "C" fn find_frame<T: Wal>(
    ...
    match this.find_frame(NonZeroU32::new(pgno).expect("invalid page number")) {
        Ok(fno) => {
            unsafe { *frame = fno.map(|x| x.get()).unwrap_or(0) };
            SQLITE_OK
        }
        Err(code) => code.extended_code,
    }
...
pub unsafe extern "C" fn read_frame<T: Wal>(
    ...
    let this = unsafe { &mut (*(wal as *mut T)) };
    let buffer = unsafe { std::slice::from_raw_parts_mut(p_out, n_out as usize) };
...
pub unsafe extern "C" fn undo<T: Wal>(
    ...
    impl UndoHandler for SqliteUndoHandler {
        fn handle_undo(&mut self, page_no: u32) -> Result<(), maniac_libsql_ffi::Error> {
            let rc = unsafe { (self.f)(self.data, page_no) };
            if rc != 0 {
                Err(maniac_libsql_ffi::Error::new(rc))
            } else {
                Ok(())
            }
        }
...
pub unsafe extern "C" fn savepoint<T: Wal>(wal: *mut wal_impl, wal_data: *mut u32) {
    let this = unsafe { &mut (*(wal as *mut T)) };
    let data = unsafe { std::slice::from_raw_parts_mut(wal_data, WAL_SAVEPOINT_NDATA as usize) };
    this.savepoint(data);
}
...
pub unsafe extern "C" fn checkpoint<T: Wal>(
    wal: *mut wal_impl,
    db: *mut maniac_libsql_ffi::sqlite3,
    emode: c_int,
    ...
    impl CheckpointCallback for SqliteCheckpointCallback {
        fn frame(
            ...
            unsafe {
                ...
                if rc == 0 { Ok(()) } else { Err(Error::new(rc)) }
            }
        }
    ...
    let buf = unsafe { std::slice::from_raw_parts_mut(z_buf, n_buf as usize) };
    ...
    let in_wal = (!frames_in_wal_out.is_null()).then(|| unsafe { &mut *frames_in_wal_out });
    let backfilled =
        (!checkpointed_frames_out.is_null()).then(|| unsafe { &mut *checkpointed_frames_out });
    let mut db = Sqlite3Db { inner: db };
...
pub unsafe extern "C" fn db<T: Wal>(wal: *mut wal_impl, db: *mut maniac_libsql_ffi::sqlite3) {
    let this = unsafe { &mut (*(wal as *mut T)) };
    let mut db = Sqlite3Db { inner: db };
    this.set_db(&mut db);
}
