...
use crate::wal::{Wal, WalManager, ffi::make_wal_manager};
...
impl FromStr for Cipher {
    type Err = maniac_libsql_ffi::Error;
...
extern "C" {
    ...
    fn libsql_leak_pager(db: *mut maniac_libsql_ffi::sqlite3) -> *mut crate::ffi::Pager;
...
pub fn pghdr_creator(
    data: &mut [u8; 4096],
    _db: *mut maniac_libsql_ffi::sqlite3,
) -> maniac_libsql_ffi::libsql_pghdr {
    #[cfg(feature = "encryption")]
...
pub unsafe fn set_encryption_cipher(db: *mut maniac_libsql_ffi::sqlite3, cipher_id: i32) -> i32 {
...
/// # Safety
/// db must point to a vaid sqlite database
pub unsafe fn set_encryption_key(db: *mut maniac_libsql_ffi::sqlite3, key: &[u8]) -> i32 {
...
pub fn reset_encryption_key(db: *mut maniac_libsql_ffi::sqlite3, key: &[u8]) -> i32 {
    unsafe { sqlite3_rekey(db, key.as_ptr() as _, key.len() as _) as i32 }
}

#[cfg(feature = "encryption")]
pub fn leak_pager(db: *mut maniac_libsql_ffi::sqlite3) -> *mut crate::ffi::Pager {
...
impl<W: Wal> Connection<W> {
        ...
        T: WalManager<Wal = W>,
    {
        ...
        let conn = unsafe {
            ...
            let mut rc = maniac_libsql_ffi::libsql_open_v3(
                path.as_ptr(),
                &mut conn as *mut _,
                flags,
                vfs,
            ...
            if let Some(cfg) = encryption_config {
                ...
                if set_encryption_key(conn, &cfg.encryption_key) != maniac_libsql_ffi::SQLITE_OK {
                    return Err(Error::Bug("failed to set encryption key"));
                }
            }

            if rc == 0 {
                rc = maniac_libsql_ffi::sqlite3_wal_autocheckpoint(conn, auto_checkpoint as _);
            }

            if rc != 0 {
                maniac_libsql_ffi::sqlite3_close(conn);
    ...
    pub unsafe fn handle(&self) -> *mut maniac_libsql_ffi::sqlite3 {
        #[cfg(feature = "rusqlite")]
        {
            self.conn.handle()
        }
    ...
    pub fn db_change_counter(&self) -> Result<u32, std::ffi::c_int> {
        let mut file_ptr: *mut maniac_libsql_ffi::sqlite3_file = std::ptr::null_mut();
        let rc = unsafe {
            maniac_libsql_ffi::sqlite3_file_control(
                self.handle(),
                "main\0".as_ptr() as *const _,
                maniac_libsql_ffi::SQLITE_FCNTL_FILE_POINTER,
                &mut file_ptr as *mut _ as *mut _,
            )
        };
        if rc != maniac_libsql_ffi::SQLITE_OK {
            return Err(rc);
        }
        let counter = unsafe {
    ...
    fn reserved_bytes(&self, reserve: Option<i32>) -> Result<i32, std::ffi::c_int> {
        ...
        let rc = unsafe {
            maniac_libsql_ffi::sqlite3_file_control(
                self.handle(),
                "main\0".as_ptr() as *const _,
                maniac_libsql_ffi::SQLITE_FCNTL_RESERVE_BYTES,
