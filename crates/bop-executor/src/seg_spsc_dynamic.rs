...
impl<T: Copy> SegSpsc<T> {
    ...
    pub fn new(
        seg_shift: usize,
        num_segs_shift: usize,
    ) -> (SegSpscProducer<T>, SegSpscConsumer<T>) {
        let queue = Arc::new(unsafe { Self::new_unsafe(seg_shift, num_segs_shift) });
        (
            SegSpscProducer {
    ...
    pub fn new_with_gate(
        ...
        let queue =
            Arc::new(unsafe { Self::new_unsafe_with_gate(seg_shift, num_segs_shift, gate) });
        (
            SegSpscProducer {
                queue: Arc::clone(&queue),
            },
    ...
    pub unsafe fn new_unsafe(seg_shift: usize, num_segs_shift: usize) -> Self {
        let seg_size = 1usize << seg_shift;
        let num_segs = 1usize << num_segs_shift;
        ...
        Self {
            ...
            num_segs_shift,
            num_segs: num_segs,
            num_segs_mask: num_segs - 1,
            capacity: (seg_size * num_segs) - 1,
            _t: PhantomData,
        }
    }
    ...
    pub unsafe fn new_unsafe_with_gate(
        seg_shift: usize,
        num_segs_shift: usize,
        signal: SignalGate,
    ) -> Self {
    ...
    pub fn try_push_n(&self, src: &[T]) -> Result<usize, PushError<()>> {
        ...
        let mut free = capacity - (h - t) as usize;
        ...
        let seg_mask = self.seg_mask;
        let num_segs_mask = self.num_segs_mask;

        while n > 0 {
            let seg_idx = ((i >> seg_shift) as usize) & num_segs_mask;
            ...
            if ((i as usize) & seg_mask) == 0 {
                let _ =
                    self.segs[((i >> seg_shift) as usize) & num_segs_mask].load(Ordering::Relaxed);
            }
        }
    ...
    pub fn try_pop_n(&self, dst: &mut [T]) -> Result<usize, PopError> {
        ...
        while n > 0 {
            ...
            if ((i as usize) & seg_mask) == 0 && i > 0 {
                let s_done = (i >> seg_shift).wrapping_sub(1);
        ...
        F: FnMut(&[T]) -> usize,
    {
        ...
        while remaining > 0 {
            let seg_idx = ((t >> seg_shift) as usize) & num_segs_mask;
            ...
            if ((t as usize) & seg_mask) == 0 && t > 0 {
                let s_done = (t >> seg_shift).wrapping_sub(1);
                self.seal_after(s_done);
            }
    ...
    fn ensure_segment_for(&self, seg_idx: usize) -> *mut MaybeUninit<T> {
        ...
        if lo < hi {
            let src_idx = (lo as usize) & num_segs_mask;
...
mod tests {
    ...
    #[test]
    fn basic_push_pop() {
        let q = unsafe { Q::new_unsafe(P, NUM_SEGS_P2) };
        for i in 0..200u64 {
            q.try_push(i).unwrap();
        }
        for i in 0..200u64 {
    ...
    fn test_different_types() {
        ...
        type QStruct = SegSpsc<TestStruct>;
        let q = unsafe { QStruct::new_unsafe(5, 6) };
    ...
    fn test_small_queue_configuration() {
        ...
        type SmallQ = SegSpsc<u32>; // 4 items per segment, 4 segments = 15 capacity
        let q = unsafe { SmallQ::new_unsafe(2, 2) };

        assert_eq!(q.capacity(), 15);
        assert_eq!(q.seg_size, 4);
        assert_eq!(q.num_segs, 4);
