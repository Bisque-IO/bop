...
// use bop_executor::{EXECUTING, IDLE, Mpmc, PushError, SCHEDULED};
...
// //             wheel: Mutex::new(TimerWheel::new(start_time, Duration::from_millis(1), 512)),
...
// thread_local! {
//     static PRODUCER: UnsafeCell<*mut bop_executor::Producer<TaskPtr, 8, 12>> = UnsafeCell::new(std::ptr::null_mut());
// }

// fn get_producer(executor: &ExecutorInner) -> *mut bop_executor::Producer<TaskPtr, 8, 12> {
...
//             let p = unsafe {
//                 Box::into_raw(p) as *mut _ as *mut bop_executor::Producer<TaskPtr, 8, 12>
//             };
//             unsafe {
...
// thread_local! {
//     // static PRODUCERS: UnsafeCell<Vec<*mut bop_executor::Producer<TaskPtr, 8, 12>>> = UnsafeCell::new(Vec::with_capacity(8));
//     static PRODUCERS: UnsafeCell<*mut bop_executor::Producer<TaskPtr, 8, 12>> = UnsafeCell::new(std::ptr::null_mut());
// }

// fn get_producers(executor: &Arc<ExecutorInner>) -> *mut bop_executor::Producer<TaskPtr, 8, 12> {
//     PRODUCERS.with(|producer_entry| {
//         let producer = unsafe { *producer_entry.get() };
//         if producer.is_null() {
//             let p = Box::new(executor.queue.create_producer_handle().unwrap());
//             let p = unsafe {
...
//                     .clone()
//                     .into_iter()
//                     .filter(|id| id.lt(&core_affinity::CoreId { id: 16 })),
//             )
...
//         let tasks_count = 100_000;
//         let task_counters: Arc<Vec<_>> = Arc::new(
//             (0..tasks_count)
//                 .map(|_| Arc::new(AtomicUsize::new(0)))
//                 .collect(),
//         );
//         let executor = Arc::new(Executor::new(16));
