...
use bop_mpmc::timer_wheel::{TimerHashWheel, TimerWheel};
...
fn main() {
    ...
    // ========================================================================
    // Run TimerHashWheel benchmarks
    // ========================================================================
    println!("\n\n");
    println!("========================================================================");
    println!("              TimerHashWheel Benchmarks (HashMap overflow)");
    println!("========================================================================");

    benchmark_schedule_hash(1_000_000, tick_1ms, 512);
    benchmark_schedule_hash(1_000_000, tick_1ms, 512);
    benchmark_cancel_hash(500_000, tick_1ms, 512);
    benchmark_cancel_random_hash(500_000, tick_1ms, 512);
    benchmark_poll_hash(500_000, tick_1ms, 512);
    benchmark_mixed_workload_hash(1_000_000, tick_1ms, 512);
    benchmark_vs_binary_heap_hash(1_000_000);

    println!("\n=== HashWheel Summary ===");
    println!("TimerHashWheel uses single slot per tick + HashMap overflow");
    println!("Optimized for well-distributed timers (low collision rate)");
    println!("True O(1) operations with no fragmentation");
    println!("Memory scales with actual usage, not worst-case allocation");
}

// ============================================================================
// TimerHashWheel Benchmarks
// ============================================================================

fn benchmark_schedule_hash(num_timers: usize, tick_res: Duration, ticks: usize) {
    println!(
        "\n=== Benchmark: Schedule {} timers (HashWheel) ===",
        num_timers
    );

    let start_time = Instant::now();
    let mut wheel = TimerHashWheel::<u32>::new(start_time, tick_res, ticks);
...
fn benchmark_cancel_hash(num_timers: usize, tick_res: Duration, ticks: usize) {
    println!(
        "\n=== Benchmark: Schedule + Cancel {} timers (sequential, HashWheel) ===",
        num_timers
    );
    ...
    for i in 0..num_timers {
        ...
        let tick = (i * ticks / num_timers) as u64;
        let deadline_ns = (tick + 1) * tick_res_ns;
        let id = wheel.schedule_timer(deadline_ns, i as u32);
        timer_ids.push(id);
    }
...
fn benchmark_cancel_random_hash(num_timers: usize, tick_res: Duration, ticks: usize) {
    println!(
        "\n=== Benchmark: Schedule + Cancel {} timers (random order, HashWheel) ===",
        num_timers
    );
...
fn benchmark_poll_hash(num_timers: usize, tick_res: Duration, ticks: usize) {
    println!(
        "\n=== Benchmark: Schedule + Poll {} timers (HashWheel) ===",
        num_timers
    );
...
fn benchmark_mixed_workload_hash(num_ops: usize, tick_res: Duration, ticks: usize) {
    println!(
        "\n=== Benchmark: Mixed workload {} ops (poll once per tick, HashWheel) ===",
        num_ops
    );
...
fn benchmark_vs_binary_heap_hash(num_timers: usize) {
    println!(
        "\n=== Comparison: TimerHashWheel vs BinaryHeap ({} timers) ===",
        num_timers
    );
    ...
    println!("\n  TimerHashWheel:");
    let start_time = Instant::now();
    let mut wheel = TimerHashWheel::<u32>::new(start_time, Duration::from_nanos(1048576), 1024);
