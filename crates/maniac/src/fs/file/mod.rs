...
impl File {
    ...
    pub async fn sync_all(&self) -> io::Result<()> {
        ...
        #[cfg(not(all(target_os = "linux", feature = "iouring")))]
        {
            ...
            unsafe { crate::blocking::unblock_fsync(fd, false).await }
        }
    }
    ...
    pub async fn sync_data(&self) -> io::Result<()> {
        ...
        #[cfg(not(all(target_os = "linux", feature = "iouring")))]
        {
            ...
            #[cfg(windows)]
            let fd = self.fd.raw_socket() as std::os::windows::io::RawHandle;
            unsafe { crate::blocking::unblock_fsync(fd, true).await }
        }
    }
...
impl AsyncWriteRent for File {
    ...
    /// #[main]
    /// async fn main() -> std::io::Result<()> {
    ///     let buf_vec = buf::VecBuf::from(vec![
...
impl AsyncReadRent for File {
    ...
    /// async fn main() -> std::io::Result<()> {
    ///     let mut file = fs::File::open("example.txt").await?;
    ///     let buffers = buf::VecBuf::from(vec![
