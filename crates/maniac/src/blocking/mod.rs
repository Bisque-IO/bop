            ...
            if self.must_wait_on_drop {
                ...
                loop {
                    match rx.close_if_not_executing() {
                        ...
                        Err(rx_back) => {
                            // Since we are blocking an async thread, let's tell the OS to
                            // cancel the I/O operation. Do so up to max_cancels with an
                            // increasing sleep duration each iteration. It is quick and
                            // dirty. Async threads may be preempted by the maniac runtime
                            // to effectively unblock this if it blocks too long. This is
                            // a very weird edge case that is may be impossible to happen
                            // and if it happens we should think about crashing the process
                            // because something is really off in the kernel.
                            if iteration < max_cancels {
...
const MAX_MULTISHOT_POOL_SIZE: usize = 100;
    ...
    ($name:ident, $type:ty, $recycle_fn:ident, $get_fn:ident) => {
        ...
        fn $recycle_fn(rx: Receiver<$type>) {
            $name.with(|pool| {
                ...
                if vec.len() < MAX_MULTISHOT_POOL_SIZE {
                    vec.push(rx);
                }
                pool.set(vec);
            });
...
mod sys {
    use crate::blocking::multishot::Sender;
    use std::io;
    #[cfg(unix)]
    use std::os::unix::prelude::RawFd;
    #[cfg(windows)]
