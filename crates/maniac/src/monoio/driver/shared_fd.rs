...
impl SharedFd {
    ...
    pub(crate) fn new<const FORCE_POLL: bool>(fd: RawFd) -> io::Result<SharedFd> {
        ...
        #[cfg(feature = "poll")]
        let scheduled_io =
            std::sync::Arc::new(crate::monoio::driver::scheduled_io::ScheduledIo::new());
    ...
    pub(crate) fn new<const FORCE_POLL: bool>(fd: RawSocket) -> io::Result<SharedFd> {
        ...
        let state = {
            ...
            let reg = CURRENT.with(|inner| match inner {
                super::Inner::Poller(inner) => super::poller::PollerDriver::register(
                    &inner,
                    &mut mio_stream,
                    RW_INTERESTS,
                    &scheduled_io,
        ...
        SharedFd {
            inner: Arc::new(Inner {
                ...
                scheduled_io: std::sync::Arc::new(
                    crate::monoio::driver::scheduled_io::ScheduledIo::new(),
                ),
            }),
        }
    }
    ...
    pub(crate) fn scheduled_io(
        &self,
    ) -> &std::sync::Arc<crate::monoio::driver::scheduled_io::ScheduledIo> {
            ...
            match inner {
                ...
                super::Inner::Poller(inner) => {
                    ...
                    if let Some(idx) = idx {
                        ...
                        let mut mio_stream = mio::net::TcpStream::from_std(std_stream);
                        let _ =
                            super::poller::PollerDriver::deregister(inner, idx, &mut mio_stream);
                        std::mem::forget(mio_stream);
                    }
                }
            }
