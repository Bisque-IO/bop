...
pub async fn remove_file<P: AsRef<Path>>(path: P) -> io::Result<()> {
    #[cfg(target_os = "linux")]
    {
        crate::monoio::driver::op::Op::unlink(path)?
            .await
            .meta
            .result?;
    }
    #[cfg(not(target_os = "linux"))]
    {
        ...
        crate::monoio::blocking::unblock_remove_file(path).await?;
    }
    Ok(())
}
...
pub async fn remove_dir<P: AsRef<Path>>(path: P) -> io::Result<()> {
    #[cfg(target_os = "linux")]
    {
        crate::monoio::driver::op::Op::rmdir(path)?
            .await
            .meta
            .result?;
    }
    ...
    {
        let path = path.as_ref().to_owned();
        crate::monoio::blocking::unblock_remove_dir(path).await?;
    }
    Ok(())
}
...
pub async fn rename<P: AsRef<Path>, Q: AsRef<Path>>(from: P, to: Q) -> io::Result<()> {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        crate::monoio::blocking::unblock_rename(from, to).await?;
    }
    Ok(())
}
