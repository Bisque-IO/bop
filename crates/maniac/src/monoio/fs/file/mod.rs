...
use crate::monoio::io::{AsyncReadRentAt, AsyncWriteRentAt};
...
impl File {
    ...
    async fn read<T: IoBufMut>(&mut self, buf: T) -> crate::monoio::BufResult<usize, T> {
        #[cfg(any(unix, windows))]
        return file_impl::read(self.fd.clone(), buf).await;
        #[cfg(not(any(unix, windows)))]
        unreachable!()
    }

    async fn read_vectored<T: IoVecBufMut>(&mut self, buf_vec: T) -> crate::monoio::BufResult<usize, T> {
        #[cfg(any(unix, windows))]
        return file_impl::read_vectored(self.fd.clone(), buf_vec).await;
        #[cfg(not(any(unix, windows)))]
        unreachable!()
    }
    ...
    /// }
    /// ```
    pub async fn read_at<T: IoBufMut>(&self, buf: T, pos: u64) -> crate::monoio::BufResult<usize, T> {
        #[cfg(any(unix, windows))]
        return file_impl::read_at(self.fd.clone(), buf, pos).await;
        #[cfg(not(any(unix, windows)))]
        unreachable!()
    }
    ...
    async fn write<T: IoBuf>(&mut self, buf: T) -> crate::monoio::BufResult<usize, T> {
        #[cfg(any(unix, windows))]
        return file_impl::write(self.fd.clone(), buf).await;
        #[cfg(not(any(unix, windows)))]
        unreachable!()
    }

    async fn write_vectored<T: IoVecBuf>(&mut self, buf_vec: T) -> crate::monoio::BufResult<usize, T> {
        #[cfg(any(unix, windows))]
        return file_impl::write_vectored(self.fd.clone(), buf_vec).await;
        #[cfg(not(any(unix, windows)))]
        unreachable!()
    }
    ...
    ///
    /// [`Ok(n)`]: Ok
    pub async fn write_at<T: IoBuf>(&self, buf: T, pos: u64) -> crate::monoio::BufResult<usize, T> {
        #[cfg(any(unix, windows))]
        return file_impl::write_at(self.fd.clone(), buf, pos).await;
        #[cfg(not(any(unix, windows)))]
        unreachable!()
    }
    ...
    /// [`ErrorKind::Interrupted`]: std::io::ErrorKind::Interrupted
    pub async fn write_all_at<T: IoBuf>(&self, mut buf: T, pos: u64) -> crate::monoio::BufResult<(), T> {
    ...
    pub async fn sync_all(&self) -> io::Result<()> {
        ...
        #[cfg(not(target_os = "linux"))]
        {
            ...
            #[cfg(windows)]
            let fd = self.fd.raw_socket() as std::os::windows::io::RawHandle;
            unsafe { crate::monoio::blocking::unblock_fsync(fd, false).await }
        }
    }
    ...
    pub async fn sync_data(&self) -> io::Result<()> {
        ...
        #[cfg(not(target_os = "linux"))]
        {
            ...
            #[cfg(windows)]
            let fd = self.fd.raw_socket() as std::os::windows::io::RawHandle;
            unsafe { crate::monoio::blocking::unblock_fsync(fd, true).await }
        }
    }
...
impl AsyncWriteRent for File {
    ...
    /// }
    /// ```
    async fn writev<T: crate::monoio::buf::IoVecBuf>(&mut self, buf_vec: T) -> crate::monoio::BufResult<usize, T> {
...
impl AsyncReadRent for File {
    ...
    /// }
    /// ```
    async fn readv<T: crate::monoio::buf::IoVecBufMut>(&mut self, buf: T) -> crate::monoio::BufResult<usize, T> {
