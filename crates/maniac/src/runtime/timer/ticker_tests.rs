...
fn test_single_handler_registration() {
    ...
    tick_service.shutdown();
    assert!(
        handler.shutdown_called(),
        "Handler should receive shutdown notification"
    );
}
...
fn test_multiple_handlers_same_tick_rate() {
    ...
    assert!(
        (count1 as i64 - count2 as i64).abs() <= 2,
        "Handlers should have similar tick counts: {} vs {}",
        count1,
        count2
    );
    assert!(
        (count2 as i64 - count3 as i64).abs() <= 2,
        "Handlers should have similar tick counts: {} vs {}",
        count2,
        count3
    );
...
fn test_multiple_handlers_different_tick_rates() {
    ...
    assert!(
        count1 > count2,
        "Handler 1 (5ms) should tick more than Handler 2 (10ms): {} vs {}",
        count1,
        count2
    );
    assert!(
        count2 > count3,
        "Handler 2 (10ms) should tick more than Handler 3 (20ms): {} vs {}",
        count2,
        count3
    );
    ...
    assert!(
        ratio_2_1 > 1.5 && ratio_2_1 < 2.5,
        "Handler 1:2 ratio should be ~2.0, got {}",
        ratio_2_1
    );
    assert!(
        ratio_3_2 > 1.5 && ratio_3_2 < 2.5,
        "Handler 2:3 ratio should be ~2.0, got {}",
        ratio_3_2
    );
}
...
fn test_tick_stats_accumulation() {
    ...
    assert!(
        stats.avg_tick_loop_duration_ns > 0,
        "Should have non-zero tick duration"
    );
    assert!(
        stats.avg_tick_loop_duration_ns < 5_000_000, // Less than 5ms
        "Tick duration should be reasonable: {} ns",
        stats.avg_tick_loop_duration_ns
    );

    // Sleep duration should be close to tick duration
    assert!(
        stats.avg_tick_loop_sleep_ns > 1_000_000, // At least 1ms
        "Sleep duration should be reasonable: {} ns",
        stats.avg_tick_loop_sleep_ns
    );
}
...
fn test_handler_stats_tracking() {
    ...
    assert!(
        fast_stats.avg_tick_loop_duration_ns > 0,
        "Fast handler should have duration"
    );
    assert!(slow_stats.total_ticks > 0, "Slow handler should have ticks");
    assert!(
        slow_stats.avg_tick_loop_duration_ns > 0,
        "Slow handler should have duration"
    );
    ...
    assert!(
        ...
        fast_stats.total_ticks,
        slow_stats.total_ticks
    );
}
...
fn test_empty_service_runs_without_handlers() {
    ...
    assert!(
        stats.total_ticks > 0,
        "Service should tick even without handlers"
    );
...
fn test_manual_unregister() {
    ...
    assert!(
        count_after <= count_immediately_after + 1,
        "Handler should not tick after unregistration: {} vs {}",
        count_immediately_after,
        count_after
    );
...
fn test_fast_handler_slow_handler_coexistence() {
    ...
    assert!(
        fast_count > slow_count * 10,
        "Fast handler should tick much more: {} vs {}",
        fast_count,
        slow_count
    );
    ...
    assert!(
        ratio > 30.0 && ratio < 70.0,
        "Ratio should be approximately 50:1, got {}",
        ratio
    );
}
