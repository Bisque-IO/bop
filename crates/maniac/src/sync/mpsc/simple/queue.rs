...
//! - TTAS (Test-and-Test-and-Set) pattern before CAS
...
/// # Performance Notes
///
/// This queue requires power-of-two capacity for optimal performance:
/// - Branchless index wrapping via bit masking
...
impl<T> Queue<T> {
    ...
    pub(super) fn new(capacity: usize) -> Queue<T> {
        ...
        let closed_channel_mask = actual_capacity;
    ...
    pub(super) fn push(&self, value: T) -> Result<(), PushError<T>> {
        ...
        loop {
            ...
            let index = enqueue_pos & self.index_mask;
            let slot = unsafe { self.buffer.get_unchecked(index) };

            // TTAS pattern: first do a relaxed load to avoid cache line bouncing
            let stamp = slot.stamp.load(Ordering::Relaxed);
            let stamp_delta = stamp.wrapping_sub(enqueue_pos) as isize;

            match stamp_delta.cmp(&0) {
                cmp::Ordering::Equal => {
                    // The slot looks available. Now do an Acquire load to ensure
                    // we see any writes from the consumer who freed this slot.
    ...
    pub(super) unsafe fn pop(&self) -> Result<T, PopError> {
        ...
        // SAFETY: index_mask ensures we're always in bounds (power-of-two capacity)
        let index = dequeue_pos & self.index_mask;
        let slot = unsafe { self.buffer.get_unchecked(index) };
        let stamp = slot.stamp.load(Ordering::Acquire);
    ...
    /// This method may not be called concurrently from multiple threads.
    #[inline]
    pub(super) unsafe fn pop_batch(&self, buffer: &mut [MaybeUninit<T>]) -> usize {
        let mut count = 0;
        let max_count = buffer.len();

        while count < max_count {
            let dequeue_pos = unsafe { self.dequeue_pos.with(|p| *p) };
            ...
            // Read the value
            let value = unsafe { slot.value.with(|v| v.read()) };
            buffer[count] = value;
            count += 1;
    ...
    /// should be cleared.
    ///
    /// This is now branchless due to power-of-two capacity.
    #[inline(always)]
    fn next_queue_pos(&self, queue_pos: usize) -> usize {
        debug_or_loom_assert_eq!(queue_pos & self.closed_channel_mask, 0);

        // With power-of-two capacity, we can use branchless wrapping.
        // The index wraps naturally with the mask, and we need to handle
        // the sequence count increment when index wraps to 0.
        let new_queue_pos = queue_pos.wrapping_add(1);
        let new_index = new_queue_pos & self.index_mask;

        // If new_index is 0, we wrapped around and need to add sequence increment.
        // sequence_increment = index_mask + 1 (which equals capacity)
        // We use a branchless select: if new_index == 0, add (capacity - 1) to skip
        // over the flag bit region.
        //
        // Actually, with power-of-two, the simple increment works because:
        // - index_mask = capacity - 1
        // - When index wraps from (capacity-1) to 0, the +1 naturally increments
        //   the sequence bits due to how the bit layout works.
        //
        // The key insight: with power-of-two capacity where capacity = index_mask + 1,
        // incrementing past the last index automatically carries into the sequence bits.
        // We just need to ensure the closed_channel_mask bit (at position = capacity)
        // doesn't get set incorrectly.

        // Check if we've crossed into the closed_channel_mask bit
        if new_queue_pos & self.closed_channel_mask != 0 {
            // We've wrapped - clear the closed bit and continue into sequence space
            // This happens when index reaches capacity (which is closed_channel_mask)
