...
use crate::generator::detail::gen_init;
use crate::generator::reg_context::RegContext;
use crate::generator::rt::{Context, ContextStack, Error};
use crate::generator::scope::Scope;
use crate::generator::stack::{Func, Stack, StackBox};
...
pub struct GeneratorObj<'a, A, T, const LOCAL: bool> {
    inner: StackBox<GeneratorImpl<'a, A, T>>,
}
...
impl<'a, A, T, const LOCAL: bool> GeneratorObj<'a, A, T, LOCAL> {
    ...
    pub unsafe fn from_raw(raw: *mut usize) -> Self {
        GeneratorObj {
            inner: unsafe { StackBox::from_raw(raw as *mut GeneratorImpl<'a, A, T>) },
        }
    }
    ...
    pub fn into_raw(self) -> *mut usize {
        let ret = self.inner.as_ptr() as *mut usize;
        std::mem::forget(self);
        ret
    }
...
impl<A> Gn<A> {
    ...
    where
        ...
        A: Send + 'a,
    {
        let mut inner_box = GeneratorImpl::<A, T>::new(Stack::new(size));
        inner_box.scoped_init(f);
        Generator { inner: inner_box }
    }
        ...
        A: 'a,
    {
        let mut inner_box = GeneratorImpl::<A, T>::new(Stack::new(size));
        inner_box.scoped_init(f);
        LocalGenerator { inner: inner_box }
    }
}
...
impl<A: Any> Gn<A> {
    ...
    {
        let mut inner_box = GeneratorImpl::<A, T>::new(Stack::new(size));
        inner_box.init_context();
        inner_box.init_code(f);
        Generator { inner: inner_box }
    }
}
...
impl<A, T> Drop for GeneratorImpl<'_, A, T> {
    fn drop(&mut self) {
        ...
        if !self.is_done() {
            log::trace!("generator is not done while drop");
            self.raw_cancel()
        }
