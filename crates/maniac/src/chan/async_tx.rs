use crate::chan::sink::AsyncSink;
use crate::chan::{MTx, Tx, channel::*};
...
impl<T: Send> AsyncTx<T> {
    ...
    ) -> SendTimeoutFuture<'a, T, crate::time::Sleep> {
        self.send_with_timer(item, crate::time::sleep(duration))
    }
    ...
    /// ```
    #[inline]
    pub fn send_with_timer<'a, F>(&'a self, item: T, fut: F) -> SendTimeoutFuture<'a, T, F>
    where
        F: Future,
    {
        SendTimeoutFuture {
...
unsafe impl<T> Send for SendFuture<'_, T> {}

// Safety: SendFuture doesn't structurally pin anything - MaybeUninit<T> is just storage
// and the waker/tx references don't need pinning guarantees.
...
impl<T: Send> Future for SendFuture<'_, T> {
    ...
    fn poll(mut self: Pin<&mut Self>, ctx: &mut Context) -> Poll<Self::Output> {
        ...
        match _self
            .tx
            .poll_send(ctx, &_self.item, &mut _self.waker, false)
        {
            Poll::Ready(Ok(())) => {
...
#[must_use]
pub struct SendTimeoutFuture<'a, T, F> {
    tx: &'a AsyncTx<T>,
    sleep: F,
    item: MaybeUninit<T>,
    waker: Option<SendWaker<T>>,
...
unsafe impl<T: Send, F: Send> Send for SendTimeoutFuture<'_, T, F> {}

impl<T, F> Drop for SendTimeoutFuture<'_, T, F> {
    fn drop(&mut self) {
        if let Some(waker) = self.waker.take() {
            // Cancelling the future, poll is not ready
            if self.tx.shared.abandon_send_waker(waker) {
...
impl<T: Send, F: Future> Future for SendTimeoutFuture<'_, T, F> {
    type Output = Result<(), SendTimeoutError<T>>;

    fn poll(self: Pin<&mut Self>, ctx: &mut Context) -> Poll<Self::Output> {
        // Safety: We only access sleep through a pinned reference, and other fields don't need pinning
        let (tx, sleep, item, waker) = unsafe {
        ...
        match tx.poll_send(ctx, item, waker, false) {
            Poll::Ready(Ok(())) => {
            ...
            Poll::Pending => {
                if let Poll::Ready(_) = sleep.poll(ctx) {
                    if let Some(w) = waker.take() {
                        if tx.shared.abandon_send_waker(w) {
...
pub trait AsyncTxTrait<T: Send>:
    Send + fmt::Debug + fmt::Display + AsRef<ChannelShared<T>> + Sized + Into<AsyncSink<T>>
{
    ...
    ///
    /// * `fut`: The sleep function. It's possible to wrap this function with cancelable handle,
    /// you can control when to stop polling. the return value of `fut` is ignored.

    fn send_with_timer<'a, F>(&'a self, item: T, fut: F) -> SendTimeoutFuture<'a, T, F>
    where
        F: Future;
}

impl<T: Send> AsyncTxTrait<T> for AsyncTx<T> {
    #[inline(always)]
    fn clone_to_vec(self, count: usize) -> Vec<Self> {
        assert_eq!(count, 1);
        vec![self]
...
impl<T: Send> AsyncTxTrait<T> for MAsyncTx<T> {
    #[inline(always)]
    fn clone_to_vec(self, count: usize) -> Vec<Self> {
