...
impl<T> AsyncRx<T> {
    ...
    ) -> RecvTimeoutFuture<'a, T, crate::time::Sleep> {
        self.recv_with_timer(crate::time::sleep(duration))
    }
    ...
    /// ```
    #[inline]
    pub fn recv_with_timer<'a, F>(&'a self, fut: F) -> RecvTimeoutFuture<'a, T, F>
    where
        F: Future,
    {
        return RecvTimeoutFuture {
            rx: self,
            waker: None,
            sleep: fut,
        };
...
// Safety: RecvFuture doesn't structurally pin anything.
...
#[must_use]
pub struct RecvTimeoutFuture<'a, T, F> {
    rx: &'a AsyncRx<T>,
    waker: Option<RecvWaker>,
    sleep: F,
}

unsafe impl<T: Send, F: Send> Send for RecvTimeoutFuture<'_, T, F> {}

impl<T, F> Drop for RecvTimeoutFuture<'_, T, F> {
    fn drop(&mut self) {
        if let Some(waker) = self.waker.take() {
            // cancelled
            self.rx.shared.abandon_recv_waker(waker);
...
impl<T: Send, F: Future> Future for RecvTimeoutFuture<'_, T, F> {
    ...
    fn poll(self: Pin<&mut Self>, ctx: &mut Context) -> Poll<Self::Output> {
        ...
        match rx.poll_item(ctx, waker, false) {
...
pub trait AsyncRxTrait<T: Send>:
    Send + fmt::Debug + fmt::Display + AsRef<ChannelShared<T>> + Sized + Into<AsyncStream<T>>
{
    ...
    fn recv_with_timer<'a, F>(&'a self, fut: F) -> RecvTimeoutFuture<'a, T, F>
    where
        F: Future;
...
impl<T: Send> AsyncRxTrait<T> for AsyncRx<T> {
    #[inline(always)]
...
impl<T: Send> AsyncRxTrait<T> for MAsyncRx<T> {
    #[inline(always)]
    fn clone_to_vec(self, count: usize) -> Vec<Self> {
