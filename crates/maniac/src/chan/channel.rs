use crate::chan::backoff::*;
use crate::chan::crossbeam::array_queue::ArrayQueue;
pub use crate::chan::crossbeam::err::*;
pub use crate::chan::locked_waker::*;
pub use crate::chan::waker_registry::*;
use crate::trace_log;
...
impl<T> ChannelShared<T> {
    pub(crate) fn new(
        ...
        if let Some(bound) = inner.capacity() {
            ...
            if bound >= 10 {
                large = true;
                backoff_limit = crate::chan::backoff::DEFAULT_LIMIT;
