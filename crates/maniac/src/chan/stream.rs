...
use crate::chan::{AsyncRx, MAsyncRx};
...
impl<T: Send> stream::Stream for AsyncStream<T> {
    type Item = T;
...
impl<T: Send> stream::FusedStream for AsyncStream<T> {
...
impl<T: Send> From<AsyncRx<T>> for AsyncStream<T> {
    #[inline]
...
impl<T: Send> From<MAsyncRx<T>> for AsyncStream<T> {
    #[inline]
