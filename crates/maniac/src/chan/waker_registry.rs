use crate::chan::channel::ChannelShared;
use crate::chan::collections::WeakCell;
use crate::chan::locked_waker::*;
...
impl<P> RegistryMulti<P> {
    ...
    pub fn new() -> Self {
        Self {
            inner: Mutex::new(RegistryMultiInner {
    ...
    {
        if let Some((waker, last_seq)) = self.pop() {
            let r = handle(&waker);
            trace_log!("wake {} {:?} {:?}", _tag, waker, r);
            if r.is_done() {
                return r;
            }
            // Use wrapping arithmetic to handle sequence wrap-around correctly
            // We stop when we've processed wakers up to the sequence we started with
            let stop_seq = last_seq.wrapping_sub(1);
            while let Some((_waker, _)) = self.pop() {
                ...
                // Use wrapping comparison to handle wrap-around correctly
                let waker_seq = _waker.get_seq();
                // Check if waker_seq is "at or past" stop_seq using wrapping arithmetic
                // (waker_seq - stop_seq) will be small positive if waker_seq >= stop_seq
                let diff = waker_seq.wrapping_sub(stop_seq);
                if diff < (usize::MAX / 2) && diff > 0 {
                    trace_log!(
                        "wake {} stop at {} (waker_seq={})",
                        _tag,
                        stop_seq,
                        waker_seq
                    );
                ...
                // Also stop if we've wrapped around completely
                if waker_seq == stop_seq {
                    trace_log!("wake {} stop at {} (exact match)", _tag, stop_seq);
                    return WakeResult::Next;
                }
            }
        }
    ...
    fn clear_wakers(&self, old_waker: &ChannelWaker<P>, oneshot: bool, _tag: &str) {
        ...
        // Helper function for wrapping sequence comparison
        // Returns true if seq_a is "after" seq_b (handling wrap-around)
        #[inline(always)]
        fn seq_is_after(seq_a: usize, seq_b: usize) -> bool {
            // Using wrapping subtraction: if seq_a > seq_b (without wrap),
            // the difference will be small positive. If seq_a < seq_b,
            // the difference will be large (close to usize::MAX).
            let diff = seq_a.wrapping_sub(seq_b);
            diff > 0 && diff < (usize::MAX / 2)
        }
                    ...
                    if _seq == old_seq {
                        ...
                        if state == WakerState::Init as u8 {
                            ...
                            if oneshot {
                                ...
                                true
                            } else if seq_is_after(_seq, old_seq) {
                                // Use wrapping-aware comparison
...
mod tests {
    ...
    use super::*;
    use crate::chan::locked_waker::RecvWaker;
    #[test]
