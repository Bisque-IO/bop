        ...
        loop {
            match &mut self.state {
                ...
                State::WithMut(task) => {
                    ...
                    let io = futures_lite::ready!(Pin::new(task).poll(cx));
                ...
                State::Streaming(any, task) => {
                    ...
                    let iter = futures_lite::ready!(Pin::new(task).poll(cx));
                ...
                State::Reading(reader, task) => {
                    ...
                    let (res, io) = futures_lite::ready!(Pin::new(task).poll(cx));
                ...
                State::Seeking(task) => {
                    // Poll the task to wait for it to finish.
                    let (_, res, io) = futures_lite::ready!(Pin::new(task).poll(cx));
                    // Make sure to move into the idle state before reporting errors.
                    self.state = State::Idle(Some(io));
                    res?;
                }
        ...
        loop {
            match &mut self.state {
                ...
                | State::Seeking(..) => {
                    ...
                    futures_lite::ready!(self.poll_stop(cx)).ok();
                }
                ...
                State::Streaming(Some(any), task) => {
                    ...
                    // Poll the channel.
                    let opt = futures_lite::ready!(receiver.as_mut().poll_next(cx));
    ...
    fn poll_read(
        ...
        loop {
            match &mut self.state {
                ...
                | State::Seeking(..) => {
                    // Wait for the running task to stop.
                    futures_lite::ready!(self.poll_stop(cx))?;
                }
                ...
                State::Reading(Some(reader), task) => {
                    // Poll the pipe.
                    let n = futures_lite::ready!(reader.poll_drain(cx, buf))?;
...
impl<T: Write + Send + 'static> AsyncWrite for Unblock<T> {
    ...
    fn poll_close(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ...
        futures_lite::ready!(Pin::new(&mut self).poll_flush(cx))?;
    ...
    fn poll_seek(
        ...
        loop {
            match &mut self.state {
                ...
                State::Seeking(task) => {
                    // Poll the task to wait for it to finish.
                    let (original_pos, res, io) = futures_lite::ready!(Pin::new(task).poll(cx));
                    // Make sure to move into the idle state before reporting errors.
                    self.state = State::Idle(Some(io));
                    let current = res?;
