...
pub struct BlockingTask {
    task: Option<crate::monoio::task::Task<NoopScheduler>>,
...
struct BlockingTaskVtable {
    pub(crate) drop: unsafe fn(&mut crate::monoio::task::Task<NoopScheduler>),
}
...
fn blocking_task_drop<V>(task: &mut crate::monoio::task::Task<NoopScheduler>) {
...
impl BlockingTask {
    ...
    pub fn run(mut self) {
        ...
        // crate::monoio::runtime::DEFAULT_CTX.with(|ctx| {
        //     crate::monoio::runtime::CURRENT.set(ctx, || task.run());
        // });
    }
}
    ...
    R: Send + 'static,
{
    ...
    crate::monoio::runtime::CURRENT.with(|inner| {
...
impl crate::monoio::task::Schedule for NoopScheduler {
    fn schedule(&self, _task: crate::monoio::task::Task<Self>) {
        unreachable!()
    }

    fn yield_now(&self, _task: crate::monoio::task::Task<Self>) {
        unreachable!()
    }
}

pub(crate) enum BlockingHandle {
    Attached(Box<dyn crate::monoio::blocking::ThreadPool + Send + 'static>),
...
mod tests {
    ...
    fn hello_blocking() {
        ...
        let mut rt = crate::monoio::RuntimeBuilder::<crate::monoio::FusionDriver>::new()
        ...
        rt.block_on(async {
            ...
            let join = crate::monoio::spawn_blocking(|| {
                // Simulate a heavy computation.
                std::thread::sleep(std::time::Duration::from_millis(400));
                "hello spawn_blocking!".to_string()
            });
            let sleep_async = crate::monoio::time::sleep(std::time::Duration::from_millis(400));
            let (result, _) = crate::monoio::join!(join, sleep_async);
    ...
    fn blocking_panic() {
        let mut rt = crate::monoio::RuntimeBuilder::<crate::monoio::FusionDriver>::new()
            .with_blocking_strategy(crate::monoio::blocking::BlockingStrategy::Panic)
            .enable_timer()
            .build()
            .unwrap();
        rt.block_on(async {
            let join = crate::monoio::spawn_blocking(|| 1);
            let _ = join.await;
        });
    }
    ...
    fn blocking_current() {
        let mut rt = crate::monoio::RuntimeBuilder::<crate::monoio::FusionDriver>::new()
            .with_blocking_strategy(crate::monoio::blocking::BlockingStrategy::ExecuteLocal)
            .enable_timer()
            .build()
            .unwrap();
        rt.block_on(async {
            ...
            let sleep_async = crate::monoio::time::sleep(std::time::Duration::from_millis(100));
            let (result, _) = crate::monoio::join!(join, sleep_async);
            let eps = begin.elapsed();
    ...
    fn drop_task() {
        ...
        let mut rt = crate::monoio::RuntimeBuilder::<crate::monoio::FusionDriver>::new()
            ...
            .unwrap();
        rt.block_on(async {
            let ret = crate::monoio::spawn_blocking(|| 1).await;
    ...
    fn default_pool() {
        ...
        rt.block_on(async {
            let begin = std::time::Instant::now();
            let join1 = crate::monoio::spawn_blocking(thread_sleep!("hello spawn_blocking1!"));
            let join2 = crate::monoio::spawn_blocking(thread_sleep!("hello spawn_blocking2!"));
            let join3 = crate::monoio::spawn_blocking(thread_sleep!("hello spawn_blocking3!"));
            let join4 = crate::monoio::spawn_blocking(thread_sleep!("hello spawn_blocking4!"));
            let join5 = crate::monoio::spawn_blocking(thread_sleep!("hello spawn_blocking5!"));
            let join6 = crate::monoio::spawn_blocking(thread_sleep!("hello spawn_blocking6!"));
            let sleep_async = crate::monoio::time::sleep(std::time::Duration::from_millis(500));
            let (result1, result2, result3, result4, result5, result6, _) =
                crate::monoio::join!(join1, join2, join3, join4, join5, join6, sleep_async);
            let eps = begin.elapsed();
