...
use crate::monoio::buf::IoBuf;
use crate::monoio::driver::shared_fd::SharedFd;
...
macro_rules! uring_op {
    ($fn_name:ident<$trait_name:ident>($op_name: ident, $buf_name:ident $(, $pos:ident: $pos_type:ty)?)) => {
        pub(crate) async fn $fn_name<T: $trait_name>(fd: SharedFd, $buf_name: T, $($pos: $pos_type)?) -> $crate::monoio::BufResult<usize, T> {
            let op = $crate::monoio::driver::op::Op::$op_name(fd, $buf_name, $($pos)?).unwrap();
            op.result().await
        }
    };
}
...
pub async fn remove_file<P: AsRef<Path>>(path: P) -> io::Result<()> {
    #[cfg(target_os = "linux")]
    {
        crate::monoio::driver::op::Op::unlink(path)?.await.meta.result?;
    }
    #[cfg(not(target_os = "linux"))]
    {
        let path = path.as_ref().to_owned();
        crate::monoio::blocking::unblock(move || std::fs::remove_file(path)).await?;
    }
    Ok(())
}
...
pub async fn remove_dir<P: AsRef<Path>>(path: P) -> io::Result<()> {
    #[cfg(target_os = "linux")]
    {
        crate::monoio::driver::op::Op::rmdir(path)?.await.meta.result?;
    }
    #[cfg(not(target_os = "linux"))]
    {
        let path = path.as_ref().to_owned();
        crate::monoio::blocking::unblock(move || std::fs::remove_dir(path)).await?;
    }
    Ok(())
}
...
pub async fn rename<P: AsRef<Path>, Q: AsRef<Path>>(from: P, to: Q) -> io::Result<()> {
    #[cfg(target_os = "linux")]
    {
        crate::monoio::driver::op::Op::rename(from.as_ref(), to.as_ref())?
            .await
            .meta
            .result?;
    }
    #[cfg(not(target_os = "linux"))]
    {
        ...
        crate::monoio::blocking::unblock(move || std::fs::rename(from, to)).await?;
    }
    Ok(())
}
