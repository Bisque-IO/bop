...
macro_rules! syscall {
    ($fn: ident @FD ( $($arg: expr),* $(,)* ) ) => {{
        ...
        if res == -1 {
            ...
            Ok(unsafe { $crate::monoio::driver::op::MaybeFd::new_fd(res as u32) })
        }
    }};
    ($fn: ident @NON_FD ( $($arg: expr),* $(,)* ) ) => {{
        ...
        if res == -1 {
            ...
            Ok($crate::monoio::driver::op::MaybeFd::new_non_fd(res as u32))
        }
    }};
...
macro_rules! syscall {
    ($fn: ident @FD ( $($arg: expr),* $(,)* ), $err_test: path, $err_value: expr) => {{
        ...
        if $err_test(&res, &$err_value) {
            ...
            Ok(unsafe { $crate::monoio::driver::op::MaybeFd::new_fd(res as _) })
        }
    }};
    ($fn: ident @NON_FD ( $($arg: expr),* $(,)* ), $err_test: path, $err_value: expr) => {{
        ...
        if $err_test(&res, &$err_value) {
            ...
            Ok($crate::monoio::driver::op::MaybeFd::new_non_fd(res as _))
        }
    }};
