use bop_mdbx::{DbFlags, Dbi, Env, EnvFlags, OptionKey, TxnFlags, get, put};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Begin a write txn
    let mut wtx = bop_mdbx::Txn::begin(&env, None, TxnFlags::READWRITE)?;
    // Open unnamed DB
    ...
    // Read it back
    let rtx = bop_mdbx::Txn::begin(&env, None, TxnFlags::RDONLY)?;
    let v = get(&rtx, dbi, b"hello")?.expect("missing");
    println!("hello -> {}", String::from_utf8_lossy(v));
    // Cursor example
    let mut cur = bop_mdbx::Cursor::open(&rtx, dbi)?;
...
impl<'e> AbortExt for bop_mdbx::Txn<'e> {
    fn abort(self) -> Result<(), Box<dyn std::error::Error>> {
        Ok(())
    }
}
