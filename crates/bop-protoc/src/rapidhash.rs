...
#[inline(always)]
pub(crate) const fn rapidhash_core(
    mut a: u64,
    mut b: u64,
    mut seed: u64,
    data: &[u8],
    ...
    if data.len() <= 16 {
        ...
        while slice.len() >= 96 {
            seed = rapid_mix(
                read_u64(slice, 0) ^ RAPID_SECRET[0],
                read_u64(slice, 8) ^ seed,
            );
            see1 = rapid_mix(
                read_u64(slice, 16) ^ RAPID_SECRET[1],
                read_u64(slice, 24) ^ see1,
            );
            see2 = rapid_mix(
                read_u64(slice, 32) ^ RAPID_SECRET[2],
                read_u64(slice, 40) ^ see2,
            );
            seed = rapid_mix(
                read_u64(slice, 48) ^ RAPID_SECRET[0],
                read_u64(slice, 56) ^ seed,
            );
            see1 = rapid_mix(
                read_u64(slice, 64) ^ RAPID_SECRET[1],
                read_u64(slice, 72) ^ see1,
            );
            see2 = rapid_mix(
                read_u64(slice, 80) ^ RAPID_SECRET[2],
                read_u64(slice, 88) ^ see2,
            );
        ...
        if slice.len() > 16 {
            seed = rapid_mix(
                read_u64(slice, 0) ^ RAPID_SECRET[2],
                read_u64(slice, 8) ^ seed ^ RAPID_SECRET[1],
            );
            if slice.len() > 32 {
                seed = rapid_mix(
                    read_u64(slice, 16) ^ RAPID_SECRET[2],
                    read_u64(slice, 24) ^ seed,
                );
            }
        }
...
pub fn into_32(value: u64) -> u32 {
    use core::ops::BitXor;
    (value as u32).wrapping_mul(SEED32).rotate_left(ROTATE).bitxor((value >> 32) as u32).wrapping_mul(SEED32)
}
...
mod tests {
    ...
    fn test_u32_to_u128_delta() {
        ...
        assert_eq!(
            outputs,
            inputs.iter().map(|&x| formula2(x)).collect::<Vec<u64>>()
        );
    }
