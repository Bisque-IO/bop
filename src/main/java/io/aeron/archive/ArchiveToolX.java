 ...
 * which is described in a {@link Catalog}.
 */
public class ArchiveToolX {
  /**
  ...
  public static void main(final String[] args) {
    ...
    if (args.length > 1 && "describe-all".equals(args[1])) {
      ...
      dump(
          ...
          ArchiveToolX::continueOnFrameLimit);
      ...
      if (args.length == 2) {
        hasErrors =
            !verify(out, archiveDir, emptySet(), null, ArchiveToolX::truncateOnPageStraddle);
      } else if (args.length == 3) {
        if (VERIFY_ALL_SEGMENT_FILES == ArchiveTool.VerifyOption.byFlag(args[2])) {
          hasErrors = !verify(
              out,
              archiveDir,
              EnumSet.of(VERIFY_ALL_SEGMENT_FILES),
              null,
              ArchiveToolX::truncateOnPageStraddle);
        } else {
        ...
        if (APPLY_CHECKSUM == ArchiveTool.VerifyOption.byFlag(args[2])) {
          hasErrors = !verify(
              out,
              archiveDir,
      ...
      markRecordingValid(out, archiveDir, Long.parseLong(args[2]));
    } else if (args.length == 3 && "mark-invalid".equals(args[1])) {
      markRecordingInvalid(out, archiveDir, Long.parseLong(args[2]));
    } else {
   ...
   * {@link Catalog} capacity then this method is a no op.
   *
   * @param archiveDir containing the {@link Catalog}.
  ...
  /**
   * Describe the metadata for all entries in the {@link Catalog}. This will include entries that
  ...
  /**
   * Describe the metadata for all valid entries in the {@link Catalog}. This will not include
   * entries that have been invalidated.
   *
   * @param out to which the entries will be printed.
   * @param archiveDir containing the {@link Catalog}.
   */
   ...
   */
  public static void describeRecording(
      final PrintStream out, final File archiveDir, final long recordingId) {
  ...
  /**
   * Dump the contents of an {@link Archive} so it can be inspected or debugged. Each recording will
   * have its contents dumped up to fragmentCountLimit before requesting to continue.
   *
          ...
          (buffer, offset, length, frameType, flags, reservedValue) -> {
            ...
            if (HDR_TYPE_PAD == frameType) {
              ...
              if ((flags & UNFRAGMENTED) != UNFRAGMENTED) {
                String suffix =
                    (flags & BEGIN_FRAG_FLAG) == BEGIN_FRAG_FLAG ? "BEGIN_FRAGMENT" : "";
   ...
   *
   * <p>Faulty entries are marked as unusable.
   *
   ...
   *     straddles the page boundary, i.e. if {@code true} the file will be truncated (last fragment
   ...
   *     entries are marked as unusable if {@code false} is returned)
   */
   ...
   * @param checksumClassName (optional) fully qualified class name of the {@link Checksum}
   *     implementation.
   * @return {@code true} if no errors have been encountered, {@code false} otherwise (the recording
   *     is marked as unusable if {@code false} is returned)
  ...
  /**
   * Compute and persist CRC-32(c) checksums for every fragment of a segment file for all recordings
   * in the catalog.
   *
   * @param out stream to print results and errors to.
   * @param archiveDir that contains {@link MarkFile}, {@link Catalog}, and recordings.
  ...
  public static void checksum(
      ...
      final String checksumClassName) {
    checksum(
        out, archiveDir, allFiles, newInstance(validateChecksumClass(checksumClassName)), INSTANCE);
  }
  ...
  public static void checksumRecording(
    ...
    checksumRecording(
        ...
        recordingId,
        allFiles,
        newInstance(validateChecksumClass(checksumClassName)),
        INSTANCE);
  }
  ...
  public static void migrate(final PrintStream out, final File archiveDir) {
    ...
    try {
      ...
      try (ArchiveMarkFile markFile = openMarkFileReadWrite(archiveDir, epochClock);
          Catalog catalog =
              openCatalogReadWrite(archiveDir, epochClock, MIN_CAPACITY, null, noVersionCheck)) {
  ...
  /**
   * Compact Catalog file by removing all recordings in state {@link RecordingState#INVALID} and
   * delete the corresponding segment files.
   *
   * @param out stream to print results and errors to.
   * @param archiveDir that contains {@link MarkFile}, {@link Catalog}, and recordings.
   */
  ...
  /**
   * Delete orphaned recording segments that have been detached for all recordings, i.e. outside the
   * start and stop recording range, but are not deleted.
   *
   * @param out stream to print results and errors to.
   * @param archiveDir that contains {@link MarkFile}, {@link Catalog}, and recordings.
   */
   ...
   * @param out stream to print results and errors to.
   * @param archiveDir that contains {@link MarkFile}, {@link Catalog}, and recordings.
   * @param targetRecordingId optional recordingId to delete orphaned segments for a specific
   *     recording. If {@link Aeron#NULL_VALUE}, delete orphaned segments for all recordings.
   */
  public static void deleteOrphanedSegments(
      final PrintStream out, final File archiveDir, final long targetRecordingId) {
          ...
          (recordingDescriptorOffset,
            ...
            if (NULL_RECORD_ID == targetRecordingId || targetRecordingId == recordingId) {
              ...
              final List<String> files =
                  segmentFilesByRecordingId.getOrDefault(recordingId, emptyList());
  ...
  static boolean verify(
      ...
      final ArchiveTool.ActionConfirmation<File> truncateOnPageStraddle) {
    try (Catalog catalog =
        openCatalogReadWrite(archiveDir, epochClock, MIN_CAPACITY, checksum, null)) {
  ...
  private static void changeRecordingState(
    ...
    try (Catalog catalog = openCatalogReadWrite(archiveDir, INSTANCE, MIN_CAPACITY, null, null)) {
      final MutableBoolean found = new MutableBoolean(false);
      catalog.forEach(
          (recordingDescriptorOffset, he, hDecoder, descriptorEncoder, descriptorDecoder) -> {
  ...
  private static Checksum createChecksum(
      final Set<ArchiveTool.VerifyOption> options, final String checksumClassName) {
  ...
  private static CatalogEntryProcessor createVerifyEntryProcessor(
    ...
    final ByteBuffer buffer =
        BufferUtil.allocateDirectAligned(FILE_IO_MAX_LENGTH_DEFAULT, CACHE_LINE_LENGTH);
    ...
    return (recordingDescriptorOffset,
        ...
        headerDecoder,
        descriptorEncoder,
        descriptorDecoder) -> verifyRecording(
        out,
        archiveDir,
  ...
  private static void dump(
      ...
      final Catalog catalog,
      final long fragmentCountLimit,
      final ArchiveTool.ActionConfirmation<Long> continueActionOnFragmentLimit,
    ...
    try (RecordingReader reader = new RecordingReader(
        catalog.recordingSummary(descriptor.recordingId(), new RecordingSummary()), archiveDir,
        descriptor.startPosition(), NULL_POSITION)) {
        ...
        if (reader.poll(fragmentHandler, 1) > 0) {
          if (--fragmentCount == 0) {
            ...
            if (NULL_POSITION != stopPosition) {
              out.println(streamLength - reader.replayPosition() + " bytes of (from " + streamLength
  ...
  private static void verifyRecording(
    ...
    final List<String> segmentFiles =
        segmentFileByRecordingId.getOrDefault(recordingId, emptyList());
      ...
      if (maxSegmentFile != null) {
        ...
        if (startPosition > maxSegmentPosition || stopPosition > maxSegmentPosition) {
          out.println("(recordingId=" + recordingId + ") ERR: Invariant violation: startPosition="
              + startPosition
              + " and/or stopPosition=" + stopPosition + " exceed max segment file position="
              + maxSegmentPosition);
          errorCount.increment();
          headerEncoder.state(INVALID);
          return;
      ...
      do {
        ...
        if (isInvalidHeader(headerFlyweight, streamId, termId, termOffset)) {
          out.println("(recordingId=" + recordingId + ", file=" + file + ") ERR: fragment "
              + "termOffset=" + headerFlyweight.termOffset()
              + " (expected=" + termOffset + "), " + "termId=" + headerFlyweight.termId()
              + " (expected=" + termId + "), " + "streamId=" + headerFlyweight.streamId()
              + " (expected=" + streamId + ")");
          return true;
        }
  ...
  private static void printErrors(final PrintStream out, final ArchiveMarkFile markFile) {
    ...
    final MappedByteBuffer cncByteBuffer =
        IoUtil.mapExistingFile(cncFile, FileChannel.MapMode.READ_ONLY, "cnc");
    ...
    try (Catalog catalog =
      ...
      final CatalogEntryProcessor catalogEntryProcessor =
          (recordingDescriptorOffset,
              ...
              descriptorDecoder) -> {
            final ByteBuffer buffer =
                ByteBuffer.allocateDirect(align(descriptorDecoder.mtuLength(), CACHE_LINE_LENGTH));
            buffer.order(LITTLE_ENDIAN);
  ...
  private static void checksum(
    ...
    if (allFiles) {
      ...
      checksumSegmentFile(
          buffer, out, archiveDir, checksum, recordingId, lastFile, startPosition, termLength);
    }
  }
  ...
  static void checksum(
    ...
    try (Catalog catalog =
        openCatalogReadWrite(archiveDir, epochClock, MIN_CAPACITY, checksum, null)) {
      final ByteBuffer buffer =
          ByteBuffer.allocateDirect(align(Configuration.MAX_UDP_PAYLOAD_LENGTH, CACHE_LINE_LENGTH));
      buffer.order(LITTLE_ENDIAN);
      final Long2ObjectHashMap<List<String>> segmentFilesByRecordingId =
          indexSegmentFiles(archiveDir);
  ...
  private static void deleteOrphanedSegmentFiles(
    ...
    final long maxBaseOffset = NULL_POSITION == descriptorDecoder.stopPosition()
        ? NULL_POSITION
  ...
  private static void printHelp() {
    System.out.format("Usage: <archive-dir> <command> (items in square brackets are optional)%n%n"
   ...
   * ArchiveTool.ActionConfirmation)} and {@link #verifyRecording(PrintStream, File, long, Set,
   * String, ArchiveTool.ActionConfirmation)} methods.
   */
  public enum VerifyOption {
    /**
     * Enables verification for all segment files of a given recording. By default, only last
     * segment file is verify.
     */
    ...
    private static final Map<String, VerifyOption> BY_FLAG =
        Stream.of(values()).collect(toMap((opt) -> opt.flag, (opt) -> opt));
  ...
  public static void loadRecordingDescriptors(
      final File archiveDir, final Long2ObjectHashMap<RecordingDescriptor> recordings) {
    try (Catalog catalog = openCatalogReadOnly(archiveDir, INSTANCE)) {
      catalog.forEach(
          (recordingDescriptorOffset,
              headerEncoder,
            ...
            rec.set(
                descriptorDecoder.controlSessionId(),
