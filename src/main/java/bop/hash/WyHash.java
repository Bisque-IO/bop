 ...
 * Original @author <a href="mailto:godspeed_china@yeah.net">Wang Yi</a> Adapted @author <a
 * href="mailto:firestrand@gmail.com">Travis Silvers</a> This implementation provides
 * endian-independant hash values, but it's slower on big-endian platforms. The original C based
 * version is also much faster.
 */
class WyHash {
  // Primes
  ...
  static <T> long wyHash64(long seed, T input, Access<T> access, long off, long length) {
    if (length <= 0) return 0;
      ...
      return _wymum(
          _wymum(_wyr3(access, input, off, length) ^ seed ^ _wyp0, seed ^ _wyp1) ^ seed,
          length ^ _wyp4);
    else if (length <= 8)
      return _wymum(
          _wymum(
                  access.u32(input, off) ^ seed ^ _wyp0,
      ...
      return _wymum(
          _wymum(
                  u64Rorate32(access, input, off) ^ seed ^ _wyp0,
      ...
      return _wymum(
          _wymum(
              ...
              ^ _wymum(u64Rorate32(access, input, off + length - 8) ^ seed ^ _wyp2, seed ^ _wyp3),
          length ^ _wyp4);
    else if (length <= 32)
      return _wymum(
          _wymum(
                  ...
                  u64Rorate32(access, input, off + 8) ^ seed ^ _wyp1)
              ^ _wymum(
                  u64Rorate32(access, input, off + 16) ^ seed ^ _wyp2,
                  u64Rorate32(access, input, off + length - 8) ^ seed ^ _wyp3),
          length ^ _wyp4);
    long see1 = seed;
    long i = length, p = off;
    for (; i > 256; i -= 256, p += 256) {
      seed = _wymum(access.i64(input, p) ^ seed ^ _wyp0, access.i64(input, p + 8) ^ seed ^ _wyp1)
          ^ _wymum(
              access.i64(input, p + 16) ^ seed ^ _wyp2, access.i64(input, p + 24) ^ seed ^ _wyp3);
      see1 = _wymum(
              access.i64(input, p + 32) ^ see1 ^ _wyp1, access.i64(input, p + 40) ^ see1 ^ _wyp2)
          ^ _wymum(
              access.i64(input, p + 48) ^ see1 ^ _wyp3, access.i64(input, p + 56) ^ see1 ^ _wyp0);
      seed = _wymum(
              access.i64(input, p + 64) ^ seed ^ _wyp0, access.i64(input, p + 72) ^ seed ^ _wyp1)
          ^ _wymum(
              access.i64(input, p + 80) ^ seed ^ _wyp2, access.i64(input, p + 88) ^ seed ^ _wyp3);
      see1 = _wymum(
              access.i64(input, p + 96) ^ see1 ^ _wyp1, access.i64(input, p + 104) ^ see1 ^ _wyp2)
          ^ _wymum(
              access.i64(input, p + 112) ^ see1 ^ _wyp3, access.i64(input, p + 120) ^ see1 ^ _wyp0);
      seed = _wymum(
              access.i64(input, p + 128) ^ seed ^ _wyp0, access.i64(input, p + 136) ^ seed ^ _wyp1)
          ^ _wymum(
              access.i64(input, p + 144) ^ seed ^ _wyp2, access.i64(input, p + 152) ^ seed ^ _wyp3);
      see1 = _wymum(
              access.i64(input, p + 160) ^ see1 ^ _wyp1, access.i64(input, p + 168) ^ see1 ^ _wyp2)
          ^ _wymum(
              access.i64(input, p + 176) ^ see1 ^ _wyp3, access.i64(input, p + 184) ^ see1 ^ _wyp0);
      seed = _wymum(
              access.i64(input, p + 192) ^ seed ^ _wyp0, access.i64(input, p + 200) ^ seed ^ _wyp1)
          ^ _wymum(
              access.i64(input, p + 208) ^ seed ^ _wyp2, access.i64(input, p + 216) ^ seed ^ _wyp3);
      see1 = _wymum(
              access.i64(input, p + 224) ^ see1 ^ _wyp1, access.i64(input, p + 232) ^ see1 ^ _wyp2)
          ^ _wymum(
              access.i64(input, p + 240) ^ see1 ^ _wyp3, access.i64(input, p + 248) ^ see1 ^ _wyp0);
    }
    for (; i > 32; i -= 32, p += 32) {
      seed = _wymum(access.i64(input, p) ^ seed ^ _wyp0, access.i64(input, p + 8) ^ seed ^ _wyp1);
      see1 = _wymum(
          access.i64(input, p + 16) ^ see1 ^ _wyp2, access.i64(input, p + 24) ^ see1 ^ _wyp3);
    }
    if (i < 4) {
      seed = _wymum(_wyr3(access, input, p, i) ^ seed ^ _wyp0, seed ^ _wyp1);
    } else if (i <= 8) {
      seed =
          _wymum(access.u32(input, p) ^ seed ^ _wyp0, access.u32(input, p + i - 4) ^ seed ^ _wyp1);
    } else if (i <= 16) {
      seed = _wymum(
          u64Rorate32(access, input, p) ^ seed ^ _wyp0,
      ...
      seed = _wymum(
          ...
          u64Rorate32(access, input, p + 8) ^ seed ^ _wyp1);
      see1 = _wymum(
          u64Rorate32(access, input, p + 16) ^ see1 ^ _wyp2,
  ...
  private static class AsLongHashFunction extends LongHashFunction {
    ...
    public long hashLong(long input) {
      ...
      return _wymum(_wymum(hi ^ seed() ^ _wyp0, lo ^ seed() ^ _wyp1) ^ seed(), 8 ^ _wyp4);
    }
    ...
    public long hashInt(int input) {
      ...
      return _wymum(
          _wymum(longInput ^ seed() ^ _wyp0, longInput ^ seed() ^ _wyp1) ^ seed(), 4 ^ _wyp4);
    }
    ...
    public long hashShort(short input) {
      ...
      return _wymum(_wymum(wyr3 ^ seed() ^ _wyp0, seed() ^ _wyp1) ^ seed(), 2 ^ _wyp4);
    }
    ...
    public long hashByte(final byte input) {
      ...
      return _wymum(_wymum(wyr3 ^ seed() ^ _wyp0, seed() ^ _wyp1) ^ seed(), 1 ^ _wyp4);
    }
