...
import static bop.unsafe.Util.checkArrayOffs;

import bop.unsafe.Access;
import bop.unsafe.ByteBufferAccess;
import bop.unsafe.UnsafeAccess;
import bop.unsafe.Util;
import java.nio.ByteBuffer;
import java.nio.ByteOrder;

/**
 * Hash function producing {@code long}-valued result from byte sequences of any length and a plenty
 * of different sources which "feels like byte sequences". Except {@link #hashBytes(byte[])},
 * {@link #hashBytes(ByteBuffer)} (with their "sliced" versions) and {@link #hashMemory(long, long)}
 * methods, which actually accept byte sequences, notion of byte sequence is defined as follows:
 *
 * <ul>
 ...
 *   <li>For methods accepting single primitive values, byte sequence is how this primitive would be
 *       put into memory with {@link ByteOrder#nativeOrder() native} byte order, or equivalently,
 *       {@code hashXxx(primitive)} has always the same result as {@code hashXxxs(new xxx[]
 *       {primitive})}, where "xxx" is any Java primitive type name.
 *   <li>For {@link #hash(Object, Access, long, long)} method byte sequence abstraction is defined
 *       by the given {@link Access} strategy to the given object.
 * </ul>
 *
 ...
 * are given for information and could be changed at any moment. However, it could hardly cause any
 * issues with subclassing, except probably little performance degradation. Methods documented as
 * "shortcuts" could either delegate to the referenced method or delegate directly to the method to
 * which the referenced method delegates.
 *
 * <p>{@code LongHashFunction} implementations shouldn't assume that {@code Access} strategies do
 * defensive checks, and access only bytes within the requested range.
 */
public abstract class LongHashFunction {
   ...
   * produces equal results for equal input on platforms with different {@link ByteOrder}, but is
   * slower on big-endian platforms than on little-endian.
   *
   * @see #xx3(long)
   */
   ...
   * input on platforms with different {@link ByteOrder}, but is slower on big-endian platforms than
   * on little-endian.
   *
   * @see #xx3()
   */
  ...
  /**
   * Returns a hash function implementing <a
   * href="https://github.com/wangyi-fudan/wyhash/blob/9f68c1b10166a54c17f55b284c21bd455fd0f7e2/wyhash.h">
   * wyhash algorithm, version 3</a> without a seed value (0 is used as default seed value). This
   * implementation produces equal results for equal input on platforms with different
   ...
   * the result of {@code hashLong(v)} call is identical to the result of {@code hashLongs(new
   * long[] {v})} call for any {@code long} value.
   */
   ...
   * {@code LongHashFunction} methods that accept sequences of bytes, assuming the {@code input}
   * value is interpreted in {@linkplain ByteOrder#nativeOrder() native} byte order. For example,
   * the result of {@code hashInt(v)} call is identical to the result of {@code hashInts(new int[]
   * {v})} call for any {@code int} value.
   */
   ...
   * {@code LongHashFunction} methods that accept sequences of bytes, assuming the {@code input}
   * value is interpreted in {@linkplain ByteOrder#nativeOrder() native} byte order. For example,
   * the result of {@code hashShort(v)} call is identical to the result of {@code hashShorts(new
   * short[] {v})} call for any {@code short} value. As a consequence, {@code hashShort(v)} call
   * produce always the same result as {@code hashChar((char) v)}.
   */
   ...
   * {@code LongHashFunction} methods that accept sequences of bytes, assuming the {@code input}
   * value is interpreted in {@linkplain ByteOrder#nativeOrder() native} byte order. For example,
   * the result of {@code hashChar(v)} call is identical to the result of {@code hashChars(new
   * char[] {v})} call for any {@code char} value. As a consequence, {@code hashChar(v)} call
   * produce always the same result as {@code hashShort((short) v)}.
   */
   ...
   * {@code hashByte(v)} call is identical to the result of {@code hashBytes(new byte[] {v})} call
   * for any {@code byte} value.
   */
  public abstract long hashByte(byte input);

  /**
   * Returns the hash code for the empty (zero-length) bytes sequence, for example
   * {@code hashBytes(new byte[0])}.
   */
  public abstract long hashVoid();

  /**
   * Returns the hash code for {@code len} continuous bytes of the given {@code input} object,
   * starting from the given offset. The abstraction of input as ordered byte sequence and "offset
   * within the input" is defined by the given {@code access} strategy.
   *
   * <p>This method doesn't promise to throw a {@code RuntimeException} if {@code [off, off + len -
   * 1]} subsequence exceeds the bounds of the bytes sequence, defined by {@code access} strategy
   * for the given {@code input}, so use this method with caution.
   *
   * @param input the object to read bytes from
   * @param access access which defines the abstraction of the given input as ordered byte sequence
  ...
  public long hashBoolean(boolean input) {
    return hashByte(input ? UnsafeAccess.TRUE_BYTE_VALUE : UnsafeAccess.FALSE_BYTE_VALUE);
  }
  ...
  public long hashBooleans(boolean[] input) {
    return unsafeHash(input, UnsafeAccess.BOOLEAN_BASE, input.length);
  }

  /**
   * Returns the hash code for the specified subsequence of the given {@code boolean} array.
   *
   * <p>Default implementation delegates to {@link #hash(Object, Access, long, long)} method using
   * {@linkplain Access#unsafe() unsafe} {@code Access}.
   *
   ...
   * @throws IndexOutOfBoundsException if {@code off < 0} or {@code off + len > input.length} or
   *     {@code len < 0}
   */
  public long hashBooleans(boolean[] input, int off, int len) {
    checkArrayOffs(input.length, off, len);
    return unsafeHash(input, UnsafeAccess.BOOLEAN_BASE + off, len);
  }

  public long hashString(String input) {
    final byte[] bytes = UnsafeAccess.INSTANCE.getBytes(input);
    return unsafeHash(bytes, UnsafeAccess.BYTE_BASE, bytes.length);
  }
  ...
  public long hashBytes(byte[] input, int off, int len) {
    checkArrayOffs(input.length, off, len);
    return unsafeHash(input, UnsafeAccess.BYTE_BASE + off, len);
  }

  /**
   * Shortcut for {@link #hashBytes(ByteBuffer, int, int) hashBytes(input, input.position(),
   * input.remaining())}.
   */
   ...
   *
   * <p>Default implementation delegates to {@link #hash(Object, Access, long, long)} method using
   * {@link Access#toByteBuffer()}.
   *
   * @param input the buffer to read bytes from
   * @param off index of the first {@code byte} in the subsequence to hash
   * @param len length of the subsequence to hash
   * @return hash code for the specified subsequence
   * @throws IndexOutOfBoundsException if {@code off < 0} or {@code off + len > input.capacity()} or
   *     {@code len < 0}
   */
  ...
  private long hashByteBuffer(ByteBuffer input, int off, int len) {
    if (input.hasArray()) {
      return unsafeHash(input.array(), UnsafeAccess.BYTE_BASE + input.arrayOffset() + off, len);
    } else if (input.isDirect()) {
      return unsafeHash(null, Util.getDirectBufferAddress(input) + off, len);
    } else {
  ...
  public long hashShorts(short[] input) {
    return unsafeHash(input, UnsafeAccess.SHORT_BASE, input.length * 2L);
  }

  /**
   * Returns the hash code for bytes, as they lay in memory, of the specified subsequence of the
   ...
   * @param len length of the subsequence to hash, in shorts (i. e. the length of the bytes sequence
  ...
  public long hashShorts(short[] input, int off, int len) {
    checkArrayOffs(input.length, off, len);
    return unsafeHash(input, UnsafeAccess.SHORT_BASE + (off * 2L), len * 2L);
  }

  /** Shortcut for {@link #hashInts(int[], int, int) hashInts(input, 0, input.length)}. */
  public long hashInts(int[] input) {
    return unsafeHash(input, UnsafeAccess.INT_BASE, input.length * 4L);
  }
  ...
  public long hashInts(int[] input, int off, int len) {
    checkArrayOffs(input.length, off, len);
    return unsafeHash(input, UnsafeAccess.INT_BASE + (off * 4L), len * 4L);
  }

  /** Shortcut for {@link #hashLongs(long[], int, int) hashLongs(input, 0, input.length)}. */
  public long hashLongs(long[] input) {
    return unsafeHash(input, UnsafeAccess.LONG_BASE, input.length * 8L);
  }
  ...
  public long hashLongs(long[] input, int off, int len) {
    checkArrayOffs(input.length, off, len);
    return unsafeHash(input, UnsafeAccess.LONG_BASE + (off * 8L), len * 8L);
  }
}
