...
/**
 * Adapted version of XXH3 implementation from https://github.com/Cyan4973/xxHash. This
 * implementation provides endian-independant hash values, but it's slower on big-endian platforms.
 */
class XXH3 {
  private static final Access<Object> unsafeLE =
      UnsafeAccess.INSTANCE.byteOrder(null, LITTLE_ENDIAN);
  ...
  private static long XXH3_mix2Accs(
      final long acc_lh, final long acc_rh, final byte[] secret, final long offSec) {
    return Maths.unsignedLongMulXorFold(
        acc_lh ^ unsafeLE.i64(secret, offSec), acc_rh ^ unsafeLE.i64(secret, offSec + 8));
  }
  ...
  private static <T> long XXH3_64bits_internal(
    ...
    if (length <= 16) {
      ...
      if (length > 8) {
        ...
        final long acc = length
            + Long.reverseBytes(input_lo)
            + input_hi
            + Maths.unsignedLongMulXorFold(input_lo, input_hi);
      ...
      if (length != 0) {
        ...
        final long combined =
            Primitives.unsignedInt((c1 << 16) | (c2 << 24) | c3 | ((int) length << 8));
        final long bitflip =
            Primitives.unsignedInt(unsafeLE.i32(XXH3_kSecret, UnsafeAccess.BYTE_BASE)
      ...
      if (length > 32) {
        if (length > 64) {
          if (length > 96) {
            ...
            acc +=
                XXH3_mix16B(seed, input, access, off + length - 64, UnsafeAccess.BYTE_BASE + 112);
          }
    ...
    if (length <= 240) {
      ...
      int i = 0;
      for (; i < 8; ++i) {
        acc += XXH3_mix16B(seed, input, access, off + 16L * i, UnsafeAccess.BYTE_BASE + 16L * i);
      }
      acc = XXH3_avalanche(acc);

      for (; i < nbRounds; ++i) {
        acc += XXH3_mix16B(
            seed, input, access, off + 16L * i, UnsafeAccess.BYTE_BASE + 16L * (i - 8) + 3);
      }
    ...
    for (long n = 0; n < nb_blocks; n++) {
      ...
      for (long s = 0; s < nbStripesPerBlock; s++) {
        ...
        final long offSec = s * 8;
        {
          ...
          final long data_key_0 =
              data_val_0 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec);
          final long data_key_1 =
              data_val_1 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8);
          /* swap adjacent lanes */
        ...
        }
        {
          ...
          final long data_key_0 =
              data_val_0 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8 * 2);
          final long data_key_1 =
              data_val_1 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8 * 3);
          /* swap adjacent lanes */
        ...
        }
        {
          ...
          final long data_key_0 =
              data_val_0 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8 * 4);
          final long data_key_1 =
              data_val_1 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8 * 5);
          /* swap adjacent lanes */
        ...
        }
        {
          ...
          final long data_key_0 =
              data_val_0 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8 * 6);
          final long data_key_1 =
              data_val_1 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8 * 7);
          /* swap adjacent lanes */
      ...
      acc_0 = (acc_0 ^ (acc_0 >>> 47) ^ unsafeLE.i64(secret, offSec)) * XXH_PRIME32_1;
      acc_1 = (acc_1 ^ (acc_1 >>> 47) ^ unsafeLE.i64(secret, offSec + 8)) * XXH_PRIME32_1;
    ...
    for (long s = 0; s < nbStripes; s++) {
      ...
      final long offSec = s * 8;
      {
        final long data_val_0 = access.i64(input, offStripe);
        final long data_val_1 = access.i64(input, offStripe + 8);
        final long data_key_0 = data_val_0 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec);
        final long data_key_1 =
            data_val_1 ^ unsafeLE.i64(secret, UnsafeAccess.BYTE_BASE + offSec + 8);
        /* swap adjacent lanes */
        acc_0 += data_val_1 + (0xFFFFFFFFL & data_key_0) * (data_key_0 >>> 32);
  ...
  private static <T> long XXH3_128bits_internal(
      final long seed,
      final byte[] secret,
      final T input,
      final Access<T> access,
    ...
    if (length <= 16) {
      ...
      if (length > 8) {
        // XXH3_len_9to16_128b
        final long bitflipl = (unsafeLE.i64(XXH3_kSecret, 32 + UnsafeAccess.BYTE_BASE)
                ^ unsafeLE.i64(XXH3_kSecret, 40 + UnsafeAccess.BYTE_BASE))
            - seed;
        final long bitfliph = (unsafeLE.i64(XXH3_kSecret, 48 + UnsafeAccess.BYTE_BASE)
                ^ unsafeLE.i64(XXH3_kSecret, 56 + UnsafeAccess.BYTE_BASE))
            + seed;
        ...
        if (null != result) {
          ...
          result[1] = XXH3_avalanche(
              Maths.unsignedLongMulHigh(m128_lo, XXH_PRIME64_2) + m128_hi * XXH_PRIME64_2);
        }
        return low;
      }
      if (length >= 4) {
        ...
        final long input_hi = access.i32(input, off + length - 4); // high int will be shifted

        final long bitflip = (unsafeLE.i64(XXH3_kSecret, 16 + UnsafeAccess.BYTE_BASE)
                ^ unsafeLE.i64(XXH3_kSecret, 24 + UnsafeAccess.BYTE_BASE))
            + s;
        final long keyed = (input_lo + (input_hi << 32)) ^ bitflip;
        final long pl = XXH_PRIME64_1
            + (length
                << 2); /* Shift len to the left to ensure it is even, this avoids even multiplies. */
      ...
      if (length != 0) {
        ...
        final long bitflipl =
            Primitives.unsignedInt(unsafeLE.i32(XXH3_kSecret, UnsafeAccess.BYTE_BASE)
                    ^ unsafeLE.i32(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 4))
                + seed;
        final long bitfliph =
            Primitives.unsignedInt(unsafeLE.i32(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 8)
                    ^ unsafeLE.i32(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 12))
                - seed;
      ...
      final long low = XXH64_avalanche(seed
          ^ unsafeLE.i64(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 64)
          ^ unsafeLE.i64(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 72));
      if (null != result) {
        result[0] = low;
        result[1] = XXH64_avalanche(seed
            ^ unsafeLE.i64(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 80)
            ^ unsafeLE.i64(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 88));
      }
      return low;
    }
    if (length <= 128) {
      ...
      if (length > 32) {
        if (length > 64) {
          if (length > 96) {
            ...
            acc0 = XXH128_mix32B_once(
                seed, UnsafeAccess.BYTE_BASE + 96, acc0, input0, input1, input2, input3);
            acc1 = XXH128_mix32B_once(
                seed, UnsafeAccess.BYTE_BASE + 96 + 16, acc1, input2, input3, input0, input1);
          }
          ...
          acc0 = XXH128_mix32B_once(
              seed, UnsafeAccess.BYTE_BASE + 64, acc0, input0, input1, input2, input3);
          acc1 = XXH128_mix32B_once(
              seed, UnsafeAccess.BYTE_BASE + 64 + 16, acc1, input2, input3, input0, input1);
        }
        ...
        acc0 = XXH128_mix32B_once(
            seed, UnsafeAccess.BYTE_BASE + 32, acc0, input0, input1, input2, input3);
        acc1 = XXH128_mix32B_once(
            seed, UnsafeAccess.BYTE_BASE + 32 + 16, acc1, input2, input3, input0, input1);
      }
      ...
      acc1 = XXH128_mix32B_once(
          seed, UnsafeAccess.BYTE_BASE + 16, acc1, input2, input3, input0, input1);

      final long low = XXH3_avalanche(acc0 + acc1);
      if (null != result) {
        result[0] = low;
        result[1] = -XXH3_avalanche(
            acc0 * XXH_PRIME64_1 + acc1 * XXH_PRIME64_4 + (length - seed) * XXH_PRIME64_2);
      }
      return low;
    }
    ...
    if (length <= 240) {
      ...
      for (; i < 4; ++i) {
        ...
        acc0 = XXH128_mix32B_once(
            seed, UnsafeAccess.BYTE_BASE + 32L * i, acc0, input0, input1, input2, input3);
        acc1 = XXH128_mix32B_once(
            seed, UnsafeAccess.BYTE_BASE + 32L * i + 16, acc1, input2, input3, input0, input1);
      }
      ...
      for (; i < nbRounds; ++i) {
        ...
        final long input3 = access.i64(input, off + 32L * i + 24);
        acc0 = XXH128_mix32B_once(
            seed, UnsafeAccess.BYTE_BASE + 3 + 32L * (i - 4), acc0, input0, input1, input2, input3);
        acc1 = XXH128_mix32B_once(
            seed,
            UnsafeAccess.BYTE_BASE + 3 + 32L * (i - 4) + 16,
            acc1,
            input2,
            input3,
            input0,
      ...
      final long input3 = access.i64(input, off + length - 32 + 8);
      acc0 = XXH128_mix32B_once(
          -seed, UnsafeAccess.BYTE_BASE + 136 - 17 - 16, acc0, input0, input1, input2, input3);
      acc1 = XXH128_mix32B_once(
          -seed, UnsafeAccess.BYTE_BASE + 136 - 17, acc1, input2, input3, input0, input1);
    ...
    if (null != result) {
      ...
      result[1] = XXH3_avalanche(~(length * XXH_PRIME64_2)
          + XXH3_mix2Accs(acc_0, acc_1, secret, UnsafeAccess.BYTE_BASE + 192 - 64 - 11)
          + XXH3_mix2Accs(acc_2, acc_3, secret, UnsafeAccess.BYTE_BASE + 192 - 64 - 11 + 16)
          + XXH3_mix2Accs(acc_4, acc_5, secret, UnsafeAccess.BYTE_BASE + 192 - 64 - 11 + 16 * 2)
          + XXH3_mix2Accs(acc_6, acc_7, secret, UnsafeAccess.BYTE_BASE + 192 - 64 - 11 + 16 * 3));
    }
    return low;
  }
  ...
  private static void XXH3_initCustomSecret(final byte[] customSecret, final long seed64) {
    ...
    for (int i = 0; i < nbRounds; i++) {
      final long lo = unsafeLE.i64(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 16 * i) + seed64;
      final long hi = unsafeLE.i64(XXH3_kSecret, UnsafeAccess.BYTE_BASE + 16 * i + 8) - seed64;
      bb.putLong(16 * i, lo);
  ...
  private static class AsLongHashFunction extends LongHashFunction {
    ...
    public long hashLong(long input) {
      ...
      final long bitflip = (unsafeLE.i64(XXH3.XXH3_kSecret, 8 + UnsafeAccess.BYTE_BASE)
              ^ unsafeLE.i64(XXH3.XXH3_kSecret, 16 + UnsafeAccess.BYTE_BASE))
          - s;
    ...
    public long hashShort(short input) {
      ...
      final long bitflip = (unsafeLE.u32(XXH3.XXH3_kSecret, UnsafeAccess.BYTE_BASE)
              ^ unsafeLE.u32(XXH3.XXH3_kSecret, 4 + UnsafeAccess.BYTE_BASE))
          + seed();
      return XXH64_avalanche(combined ^ bitflip);
    }
    ...
    public long hashVoid() {
      return XXH64_avalanche(seed()
          ^ unsafeLE.i64(XXH3.XXH3_kSecret, 56 + UnsafeAccess.BYTE_BASE)
          ^ unsafeLE.i64(XXH3.XXH3_kSecret, 64 + UnsafeAccess.BYTE_BASE));
    }

    @Override
    public <T> long hash(final T input, final Access<T> access, final long off, final long len) {
      return XXH3.XXH3_64bits_internal(
          0, XXH3.XXH3_kSecret, input, access.byteOrder(input, LITTLE_ENDIAN), off, len);
    }
  }
  ...
  private static class AsLongHashFunctionSeeded extends AsLongHashFunction {
    ...
    public <T> long hash(final T input, final Access<T> access, final long off, final long len) {
      return XXH3.XXH3_64bits_internal(
          this.seed, this.secret, input, access.byteOrder(input, LITTLE_ENDIAN), off, len);
    }
  }
}
