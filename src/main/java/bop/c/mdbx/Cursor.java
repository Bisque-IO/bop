...
public class Cursor {
  ...
  private Cursor(final long scratchPointer, final long scratchSize) {
  ...
  public void ensureTempStorage(long size) {
    if (scratchPointer == 0L) {
      ...
      scratchPointer = Memory.zalloc(size);
      scratchSize = size;
      return;
    }
    ...
    scratchPointer = Memory.realloc(scratchPointer, size);
  }
  ...
  public int setContext(long context) {
    final var cursorPointer = this.cursorPointer;
    if (cursorPointer == 0L) {
  ...
  public int close() {
    ...
    if (cursorPointer != 0L) {
      final int err;
      try {
        err = (int) CFunctions.MDBX_CURSOR_CLOSE2.invokeExact(cursorPointer);
      } catch (Throwable e) {
        throw new RuntimeException(e);
      }
  ...
  public int bind(Txn txn, Dbi dbi) {
    assert txn != null;
    assert dbi != null;
    final var cursorPointer = this.cursorPointer;
    if (cursorPointer == 0L) {
    ...
    final int err;
    try {
      err = (int) CFunctions.MDBX_CURSOR_BIND.invokeExact(txn.txnPointer, cursorPointer, dbi.dbi);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err != Error.SUCCESS) {
  ...
  public int copyTo(final Cursor dst) {
    ...
    if (dst == null || dst.cursorPointer == 0L) {
      return Error.INVALID;
    }
    try {
      return (int) CFunctions.MDBX_CURSOR_COPY.invokeExact(cursorPointer, dst.cursorPointer);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public int compare(Cursor right, boolean ignoreMultiVal) {
    final var cursorPointer = this.cursorPointer;
    if (cursorPointer == 0L) {
      return right == null || right.cursorPointer == 0L ? 0 : -1;
    }
    if (right == null || right.cursorPointer == 0L) {
      return 1;
    }
    try {
      return (int) CFunctions.MDBX_CURSOR_COMPARE.invokeExact(
          cursorPointer, right.cursorPointer, ignoreMultiVal);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public int get(long key, Val data, int op) {
    ...
    try {
      return (int) CFunctions.BOP_MDBX_CURSOR_GET_INT.invokeExact(
          cursorPointer, key, this.key.address(), data.address(), op);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public int put(long key, Val data, int flags) {
    try {
      return (int) CFunctions.BOP_MDBX_CURSOR_PUT_INT2.invokeExact(
          cursorPointer, key, data.address(), flags);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
