...
import bop.hash.LongHashFunction;
 ...
 * <i>ordered byte sequence</i> each {@code T} instance represents. For example, if some
 * {@code Access} implementation returns {@link ByteOrder#LITTLE_ENDIAN} on
 * {@link #byteOrder(Object) byteOrder(input)} call, the following expressions should always have
 * the same value:
 *
 * <ul>
 *   <li>{@code getLong(input, 0)}
 *   <li>{@code getUnsignedInt(input, 0) | (getUnsignedInt(input, 4) << 32)}
 *   <li>
 *       <pre>{@code getUnsignedInt(input, 0) |
 ...
 * efficiency you should override methods used by target {@link LongHashFunction} implementation.
 *
 ...
 * are accessed, but this should be awkward, and hashing using such {@code Access} is expected to be
 * slow.
 *
...
public abstract class Access<T> {
  ...
  /**
   * Returns the {@code Access} delegating {@code getXXX(input, offset)} methods to
   * {@code sun.misc.Unsafe.getXXX(input, offset)}.
   *
   * <p>Usage example:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>{@code null} is a valid input, on accepting {@code null} {@code Unsafe} just interprets the
   * given offset as a wild memory address. Note that for hashing memory by address there is a
   * shortcut {@link LongHashFunction#hashMemory(long, long) hashMemory(address, len)} method.
   *
  ...
  /**
   * Returns the {@code Access} to any {@link ByteBuffer}. This {@code Access} isn't useful in the
   * user code, because methods {@link LongHashFunction#hashBytes(ByteBuffer)} and
   ...
   * @param offset offset to the first byte to read within the byte sequence represented by the
   *     given object
  ...
  /**
   * Shortcut for {@code getShort(input, offset) & 0xFFFF}. Could be implemented more efficiently.
   *
   * @param input the object to access
   * @param offset offset to the first byte to read within the byte sequence represented by the
   *     given object
   ...
   *
   * @param input the object to access
   * @param offset offset to the byte to read within the byte sequence represented by the given
   *     object
  ...
  /**
   * The byte order in which all multi-byte {@code getXXX()} reads from the given {@code input} are
   * performed.
   *
  ...
  /**
   * Get {@code this} or the reversed access object for reading the input as fixed byte order of
   * {@code byteOrder}.
   *
   * @param input the accessed object
   * @param byteOrder the byte order to be used for reading the {@code input}
   * @return a {@code Access} object which will read the {@code input} with the byte order of
   *     {@code byteOrder}.
   */
  ...
  /**
   * Get the {@code Access} object with a different byte order. This method should always return a
   * fixed reference.
   */
