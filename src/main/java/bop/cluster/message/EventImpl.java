...
public sealed class EventImpl
  ...
  static final UnsafeAccess UA = UnsafeAccess.INSTANCE;
  ...
  */
  ...
  protected long computeHash(int offset, int length) {
    return Hash.xx3(address + offset, length);
  }
  ...
  protected int read(int offset, byte[] dst, int dstOffset, int length) {
    ...
    U.copyMemory(null, address + offset, dst, UnsafeAccess.BYTE_BASE + dstOffset, len);
    return len;
  }
  ...
  protected void write(int offset, ByteBuffer buffer) {
    ...
    if (buffer.hasArray()) {
      U.copyMemory(
          buffer.array(),
          UnsafeAccess.BYTE_BASE + buffer.position(),
          null,
          address + offset,
          buffer.remaining());
    } else {
      final var src = UA.getAddress(buffer);
      if (src == 0L) {
        return;
      }
  ...
  protected void write(int offset, byte[] src, int srcOffset, int length) {
    U.copyMemory(src, UnsafeAccess.BYTE_BASE + srcOffset, null, address + offset, length);
  }
  ...
  protected void setTopic(Topic topic) {
    ...
    final var src = UA.getBytes(topic.name());
  ...
  protected void setBody(ByteBuffer buffer) {
    ...
    if (buffer.hasArray()) {
      ...
      U.copyMemory(UA.getAddress(buffer) + buffer.position(), address + pos, buffer.remaining());
    }
    bodyOffset((char) pos);
