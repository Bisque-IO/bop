...
///
/// <p>Based on netty's HashedTimerWheel, which is based on <a
/// href="http://cseweb.ucsd.edu/users/varghese/">George Varghese</a> and Tony Lauck's paper, <a
/// href="http://cseweb.ucsd.edu/users/varghese/PAPERS/twheel.ps.Z">'Hashed and Hierarchical Timing
/// Wheels: data structures to efficiently implement a timer facility'</a>. More comprehensive
// slides
/// are located <a href="http://www.cse.wustl.edu/~cdgill/courses/cs6874/TimingWheels.ppt">here</a>.
///
/// <p>Wheel is backed by arrays. Timer cancellation is O(1). Timer scheduling might be slightly
/// longer if a lot of timers are in the same tick or spoke. The underlying tick is contained in an
/// array. That array grows when needed, but does not shrink.
///
/// <p><b>Caveats</b>
///
/// <p>Timers that expire in the same tick are not ordered with one another. As ticks are fairly
/// coarse resolution normally, this means that some timers may expire out of order.
///
/// <p><b>Note:</b> Not threadsafe.
public class TimerWheel {
  ...
  ///
  /// @see TimerWheel#poll(long, TimerHandler, int)
  @FunctionalInterface
  public interface TimerHandler {
    ...
    /// @return true to consume the timer, or false to keep timer active and abort further
    // polling.
    boolean onTimerExpiry(TimeUnit timeUnit, long now, long timerId, long deadline);
  }

  /// Consumer of timer entries as deadline to timerId.
  ///
  /// @see TimerWheel#forEach(TimerConsumer)
  @FunctionalInterface
  ...
  ///
  /// <p>If the time passed in is less than the current time, nothing is changed. No timers will
  // be
  /// expired when winding forward and thus are still in the wheel and will be expired as
  // encountered
  /// in the wheel during {@link #poll} operations. No guarantee of order for expired timers is
  ...
  /// Schedule a timer for a given absolute time as a deadline in {@link #timeUnit()}s. A timerId
  /// will be assigned and returned for future reference.
  ///
  ...
  public long scheduleTimer(final long deadline) {
    final long deadlineTick =
        Math.max((deadline - startTime) >> resolutionBitsToShift, currentTick);
  ...
  /// @param deadline after which the timer should expire.
  /// @return timerId assigned for the scheduled timer.
  public long scheduleTimerIfAbsent(final long deadline) {
    final long deadlineTick =
        Math.max((deadline - startTime) >> resolutionBitsToShift, currentTick);
    final int spokeIndex = (int) (deadlineTick & tickMask);
    final int tickStartIndex = spokeIndex << allocationBitsToShift;
    ...
    if (timerCount > 0) {
      ...
      for (int i = 0, length = tickAllocation; i < length && expiryLimit > timersExpired; i++) {
        ...
        if (now >= deadline) {
          ...
          if (!handler.onTimerExpiry(
              timeUnit, now, timerIdForSlot(spokeIndex, pollIndex), deadline)) {
  ...
  /// @return deadline for the given timerId or {@link #NULL_DEADLINE} if timerId is not
  // scheduled.
  ...
  private static void checkResolution(final long tickResolution) {
    if (!isPowerOfTwo(tickResolution)) {
  ...
  private static void checkInitialTickAllocation(final int tickAllocation) {
    if (!isPowerOfTwo(tickAllocation)) {
