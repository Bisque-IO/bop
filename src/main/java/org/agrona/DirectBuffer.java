 ...
 *
 * <p>{@link ByteOrder} of a wrapped buffer is not applied to the {@link DirectBuffer}; To control
...
public interface DirectBuffer extends Comparable<DirectBuffer> {
  ...
  /**
   * Name of the system property that specify if the bounds checks should be disabled. To disable
   * bounds checks set this property to {@code true}.
   */
   ...
   * wrapped buffer.
   *
   * <p>When using this method to wrap the view of the ByteBuffer the entire ByteBuffer gets wrapped
   * between index 0 and capacity. If you want to just wrap the ByteBuffer between the position and
   * the limit then you should use the {@link #wrap(ByteBuffer, int, int)} method, eg:
   *
   * <p><code>
   ...
   *
   * <p>The {@link ByteBuffer#order()} is not relevant for accessing the wrapped buffer.
   *
   * @param buffer to which the view is attached.
   * @param offset in bytes at which the view begins.
   * @param length in bytes of the buffer included in the view.
   ...
   *
   * <p>NB: there may not be a one-to-one mapping between indices on this buffer and the underlying
   * {@code byte[]}, see {@link DirectBuffer#wrapAdjustment()}.
   *
   ...
   *
   * <p>NB: there may not be a one-to-one mapping between indices on this buffer and the underlying
   * byte[], see {@link DirectBuffer#wrapAdjustment()}.
   *
   ...
   *
   * <p>Can be overridden in a DirectBuffer subclass to enable an extensible buffer or handle retry
   * after a flush.
   *
  ...
  /**
   * Get from the underlying buffer into a supplied byte array. This method will try to fill the
   * supplied byte array.
   *
  ...
  /**
   * Get bytes from this {@link DirectBuffer} into the provided {@link MutableDirectBuffer} at given
   * indices.
   *
  ...
  /**
   * Get from the underlying buffer into a supplied {@link ByteBuffer} current
   * {@link ByteBuffer#position()}.
   *
   * <p>The destination buffer will have its {@link ByteBuffer#position()} advanced as a result.
   *
   * @param index in the underlying buffer to start from.
   ...
   *
   * <p>The destination buffer will not have its {@link ByteBuffer#position()} advanced as a result.
   *
   * @param index in the underlying buffer to start from.
   * @param dstBuffer into which the bytes will be copied.
  ...
  /**
   * Get part of a String from bytes encoded in ASCII format that is length prefixed and append to
   * an {@link Appendable}.
   *
   * @param index at which the String begins.
   * @param length of the String in bytes to decode.
  ...
  /**
   * Get the adjustment in indices between an index in this buffer and the wrapped object. The
   * wrapped object might be a {@link ByteBuffer} or a {@code byte[]}.
   *
   * <p>You only need to use this adjustment if you plan to perform operations on the underlying
