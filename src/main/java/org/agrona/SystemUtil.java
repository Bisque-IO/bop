...
public final class SystemUtil {
  /**
   * PID value if a process id could not be determined. This value should be equal to a kernel only
   * process id for the platform so that it does not indicate a real process id.
   */
  public static final long PID_NOT_FOUND = 0;

  /**
   * Value a {@link System#getProperties()} can be set to so that {@code null} will be returned as
   ...
   *
   * <p>This is what is returned from {@code System.getProperty("os.name").toLowerCase()}.
   *
   ...
   *
   * <p>This is the same a calling the {@code System.getProperty("os.arch", "unknown")}.
   *
   ...
   *
   * @return {@code true} if the operating system is likely to be Windows based on
   *     {@link #osName()}.
   */
  ...
  /**
   * The system property for <code>java.io.tmpdir</code> and ensuring a {@link File#separator} is at
   * the end.
   *
  ...
  public static void threadDump(final StringBuilder sb) {
    ...
    for (final ThreadInfo threadInfo :
        mxBean.getThreadInfo(mxBean.getAllThreadIds(), Integer.MAX_VALUE)) {
      if (null != threadInfo) {
        sb.append('"')
   ...
   * {@link org.agrona.PropertyAction#REPLACE}.
   *
   * <p>File is first searched for in resources using the system {@link ClassLoader}, then file
   * system, then URL. All are loaded if multiples found.
   *
   * @param filenameOrUrl that holds properties.
   */
  public static void loadPropertiesFile(final String filenameOrUrl) {
    loadPropertiesFile(org.agrona.PropertyAction.REPLACE, filenameOrUrl);
  }
  ...
  public static void loadPropertiesFile(
      final org.agrona.PropertyAction propertyAction, final String filenameOrUrl) {
  ...
  public static void loadPropertiesFiles(final String... filenamesOrUrls) {
    loadPropertiesFiles(org.agrona.PropertyAction.REPLACE, filenamesOrUrls);
  }
  ...
  public static void loadPropertiesFiles(
      final org.agrona.PropertyAction propertyAction, final String... filenamesOrUrls) {
   ...
   * @return the value of a {@link System#getProperty(String)} with the exception that if the value
   *     is {@link #NULL_PROPERTY_VALUE} then return {@code null}.
   */
  ...
  /**
   * Get the value of a {@link System#getProperty(String, String)} with the exception that if the
   * value is {@link #NULL_PROPERTY_VALUE} then return {@code null}, otherwise if the value is not
   * set then return the default value.
   *
   * @param propertyName to get the value for.
   * @param defaultValue to use if the property is not set.
   * @return the value of a {@link System#getProperty(String, String)} with the exception that if
   *     the value is {@link #NULL_PROPERTY_VALUE} then return {@code null}, otherwise if the value
   *     is not set then return the default value.
   */
  ...
  /**
   * Get a size value as an int from a system property. Supports a 'g', 'm', and 'k' suffix to
   * indicate gigabytes, megabytes, or kilobytes respectively.
   *
  ...
  /**
   * Get a size value as a long from a system property. Supports a 'g', 'm', and 'k' suffix to
   * indicate gigabytes, megabytes, or kilobytes respectively.
   *
   * @param propertyName to lookup.
   * @param defaultValue to be applied if the system property is not set.
  ...
  /**
   * Parse a string representation of a value with optional suffix of 'g', 'm', and 'k' suffix to
   * indicate gigabytes, megabytes, or kilobytes respectively.
   *
  ...
  public static long parseSize(final String propertyName, final String propertyValue) {
    ...
    final long value = org.agrona.AsciiEncoding.parseLongAscii(propertyValue, 0, lengthMinusSuffix);
    ...
    switch (lastCharacter) {
      ...
      case 'K':
        if (value > MAX_K_VALUE) {
          throw new NumberFormatException(
              propertyName + " would overflow a long: " + propertyValue);
        }
        return value * 1024;
  ...
  /**
   * Get a string representation of a time duration with an optional suffix of 's', 'ms', 'us', or
   * 'ns' suffix to indicate seconds, milliseconds, microseconds, or nanoseconds respectively.
   *
   * <p>If the resulting duration is greater than {@link Long#MAX_VALUE} then {@link Long#MAX_VALUE}
   * is used.
   *
  ...
  /**
   * Parse a string representation of a time duration with an optional suffix of 's', 'ms', 'us', or
   * 'ns' to indicate seconds, milliseconds, microseconds, or nanoseconds respectively.
   *
   * <p>If the resulting duration is greater than {@link Long#MAX_VALUE} then {@link Long#MAX_VALUE}
   * is used.
   *
  ...
  public static long parseDuration(final String propertyName, final String propertyValue) {
    ...
    if (Character.isDigit(secondLastCharacter)) {
      final long value =
          org.agrona.AsciiEncoding.parseLongAscii(propertyValue, 0, propertyValue.length() - 1);
  ...
  private static void loadProperties(final PropertyAction propertyAction, final InputStream in)
      throws IOException {
