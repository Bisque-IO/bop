 ...
 *
 * <p>The assumptions are: (1) the version field is an int in size, (2) the timestamp field is a
 * long in size, and (3) the version field comes before the timestamp field.
 */
public class MarkFile implements AutoCloseable {
  ...
  /**
   * Create a directory and mark file if none present. Checking if an active Mark file exists and is
   * active. Old Mark file is deleted and recreated if not active.
   *
   * <p>Total length of Mark file will be mapped until {@link #close()} is called.
   *
  ...
  public MarkFile(
    ...
    this.mappedBuffer = org.agrona.IoUtil.mapNewFile(markFile, totalFileLength);
  ...
  /**
   * Create a {@link MarkFile} if none present. Checking if an active {@link MarkFile} exists and is
   * active. Existing {@link MarkFile} is used if not active.
   *
   * <p>Total length of Mark file will be mapped until {@link #close()} is called.
   *
   ...
   *
   * <p>Total length of {@link MarkFile} will be mapped until {@link #close()} is called.
   *
   * @param directory for the {@link MarkFile} file.
   * @param filename of the {@link MarkFile} file.
   * @param versionFieldOffset to use for version field access.
   * @param timestampFieldOffset to use for timestamp field access.
   * @param timeoutMs for the activity check (in milliseconds) and for how long to wait for file to
   *     exist.
   * @param epochClock to use for time checks.
   ...
   *
   * <p>If mappedBuffer is not null, then it will be unmapped upon {@link #close()}.
   *
   ...
   */
  public MarkFile(
      final UnsafeBuffer buffer, final int versionFieldOffset, final int timestampFieldOffset) {
    validateOffsets(versionFieldOffset, timestampFieldOffset);
  ...
  public void close() {
    if (isClosed.compareAndSet(false, true)) {
      org.agrona.BufferUtil.free(mappedBuffer);
    }
  }
   ...
   *
   * <p>This method is identical to {@link #timestampRelease(long)} and that method is preferred.
   *
  ...
  public void deleteDirectory(final boolean ignoreFailures) {
    org.agrona.IoUtil.delete(parentDir, ignoreFailures);
  }
  ...
  /**
   * Ensure the directory exists, i.e. create if it does not exist yet and re-create if it already
   * exists.
   *
   ...
   * @param dirDeleteOnStart should directory be deleted if it already exists. When the flag is set
   *     to {@code false} the check will be made to see if the {@link MarkFile} is active.
  ...
  public static void ensureDirectoryExists(
    ...
    if (directory.isDirectory()) {
      ...
      if (!dirDeleteOnStart) {
        ...
        final int length =
            Math.max(versionFieldOffset, timestampFieldOffset) + SIZE_OF_LONG - offset;
        ...
        try {
          ...
          org.agrona.BufferUtil.free(byteBuffer);
        }
      }

      org.agrona.IoUtil.delete(directory, false);
    }

    org.agrona.IoUtil.ensureDirectoryExists(directory, directory.toString());
  }
  ...
  public static MappedByteBuffer mapExistingMarkFile(
    ...
    final MappedByteBuffer byteBuffer =
        waitForFileMapping(logger, markFile, deadlineMs, epochClock);
    if (byteBuffer.capacity() < (timestampFieldOffset + SIZE_OF_LONG)) {
      throw new IllegalStateException(
          "Mark file mapping is to small: capacity=" + byteBuffer.capacity());
    }
  ...
  /**
   * Ensure a link file exists if required for the actual mark file. A link file will contain the
   * pathname of the actual mark file's parent directory. This is useful if the mark file should be
   * stored on a different storage medium to the directory of the service. This will create a file
   * with name of {@code linkFilename} in the {@code serviceDir}. If {@code actualFile} is an
   * immediate child of {@code serviceDir} then any file with the name of {@code linkFilename} will
   * be deleted from the {@code serviceDir} (so that links won't be present if not required).
   *
   * @param serviceDir directory where the mark file would normally be stored (e.g. archiveDir,
   *     clusterDir).
   ...
   */
  public static void ensureMarkFileLink(
      final File serviceDir, final File actualFile, final String linkFilename) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "failed to resolve canonical path for archiveDir=" + serviceDir);
    }
  ...
  /**
   * Put thread to sleep for the given duration and restore interrupted status if thread is
