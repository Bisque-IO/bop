...
public class BiInt2ObjectMap<V> {
  ...
  public BiInt2ObjectMap() {
    this(MIN_CAPACITY, org.agrona.collections.Hashing.DEFAULT_LOAD_FACTOR);
  }
  ...
  public V put(final int keyPartA, final int keyPartB, final V value) {
    ...
    int index = org.agrona.collections.Hashing.hash(key, mask);
  ...
  /**
   * Retrieve a value from the map or <code>defaultValue</code> if this map contains not mapping for
   * the key.
   *
   * @param keyPartA for the key.
   * @param keyPartB for the key.
   ...
   *
   * <p>If the function returns null, the mapping is removed. If the function itself throws an
   ...
   * results of the given remapping function, or removes if the result is null.
   *
   * @param keyPartA for the key.
   * @param keyPartB for the key.
  ...
  public boolean replace(
      final int keyPartA, final int keyPartB, final V oldValue, final V newValue) {
    final long key = compoundKey(keyPartA, keyPartB);
  ...
  private void rehash(final int newCapacity) {
    ...
    for (int i = 0, size = values.length; i < size; i++) {
      ...
      if (null != value) {
        ...
        int newHash = org.agrona.collections.Hashing.hash(key, mask);
