...
public class Object2ObjectHashMap<K, V> implements Map<K, V> {
  ...
  /**
   * Default constructor, i.e. create a map with {@link #MIN_CAPACITY} and
  ...
  private V getMapped(final Object key) {
    ...
    int keyIndex = org.agrona.collections.Hashing.evenHash(key.hashCode(), mask);
  ...
  private void rehash(final int newCapacity) {
    ...
    for (int keyIndex = 0; keyIndex < length; keyIndex += 2) {
      ...
      if (null != value) {
        ...
        int index = org.agrona.collections.Hashing.evenHash(key.hashCode(), mask);
  ...
  private void compactChain(int deleteKeyIndex) {
    ...
    while (true) {
      ...
      final int hash = org.agrona.collections.Hashing.evenHash(key.hashCode(), mask);
  ...
  /**
   * Handle incoming non-null value and optionally replace it with the <em>null</em> value
   * counterpart. This is the opposite of the {@link #mapNullValue(Object)} method.
   *
   * @param value value to be handled.
   * @return replacement value.
