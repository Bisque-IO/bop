 ...
 * Message are written and read in a FIFO order with capacity up to {@link #maxCapacity()}. Messages
 * can be iterated via for-each methods without consuming and having the option to begin iteration
 * an offset from the current {@link #head()} position.
 *
...
public class ExpandableRingBuffer {
  ...
  /**
   * Consumers of messages implement this interface and pass it to {@link #consume(MessageConsumer,
   * int)}.
   */
  @FunctionalInterface
  public interface MessageConsumer {
    /**
     * Called for the processing of each message from a buffer in turn. Returning false aborts
     * consumption so that current message remains and any after it.
     *
     * @param buffer containing the encoded message.
     * @param offset at which the encoded message begins.
     * @param length in bytes of the encoded message.
     * @param headOffset how much of an offset from {@link #head()} has passed for the end of this
     *     message.
     * @return true is the message was consumed otherwise false. Returning false aborts further
     *     consumption.
     */
   ...
   * {@link #MAX_CAPACITY}, and using a direct {@link ByteBuffer}.
   */
  ...
  /**
   * Create a new ring buffer providing configuration for initial and max capacity, plus whether it
   * is direct or not.
   *
   ...
   */
  public ExpandableRingBuffer(
      final int initialCapacity, final int maxCapacity, final boolean isDirect) {
   ...
   *
   * @return head position in the buffer from which bytes are consumed forward towards the
   *     {@link #tail()}.
   ...
   * @see #append(org.agrona.DirectBuffer, int, int)
   */
  public long tail() {
    return tail;
  }
  ...
  /**
   * Reset the buffer with a new capacity and empty state. Buffer capacity will grow or shrink as
   * necessary.
   *
   * @param requiredCapacity for the ring buffer. If the same as exiting capacity then no adjustment
   *     is made.
   */
  ...
  /**
   * Iterate encoded contents and pass messages to the {@link MessageConsumer} which can stop by
   * returning false.
   *
  ...
  public int forEach(final MessageConsumer messageConsumer, final int limit) {
    ...
    while (count < limit && position < tail) {
      ...
      if (MESSAGE_TYPE_PADDING != typeId) {
        ...
        if (!messageConsumer.onMessage(
            buffer, offset + HEADER_LENGTH, length - HEADER_LENGTH, headOffset)) {
          break;
        }
   ...
   * returning false.
   *
   * @param headOffset offset from {@link #head} which must be &lt;= {@link #tail()}, and be the
   *     start a message.
   * @param messageConsumer to which the encoded messages are passed.
   * @param limit for the number of entries to iterate over.
   * @return count of bytes iterated.
  ...
  public int forEach(final int headOffset, final MessageConsumer messageConsumer, final int limit) {
    ...
    while (count < limit && position < tail) {
      ...
      if (MESSAGE_TYPE_PADDING != typeId) {
        ...
        if (!messageConsumer.onMessage(
            buffer, offset + HEADER_LENGTH, length - HEADER_LENGTH, result)) {
          break;
        }
  ...
  public boolean append(
      final org.agrona.DirectBuffer srcBuffer, final int srcOffset, final int srcLength) {
