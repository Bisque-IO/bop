...
public final class BufferUtil {
  ...
  static {
    try {
      BYTE_BUFFER_HB_FIELD_OFFSET = U.objectFieldOffset(ByteBuffer.class.getDeclaredField("hb"));

      BYTE_BUFFER_OFFSET_FIELD_OFFSET =
          U.objectFieldOffset(ByteBuffer.class.getDeclaredField("offset"));

      BYTE_BUFFER_ADDRESS_FIELD_OFFSET =
          U.objectFieldOffset(Buffer.class.getDeclaredField("address"));
  ...
  public static long address(final ByteBuffer buffer) {
    ...
    return U.getLong(buffer, BYTE_BUFFER_ADDRESS_FIELD_OFFSET);
  }
  ...
  public static byte[] array(final ByteBuffer buffer) {
    ...
    return (byte[]) U.getReference(buffer, BYTE_BUFFER_HB_FIELD_OFFSET);
  }
  ...
  public static int arrayOffset(final ByteBuffer buffer) {
    return U.getInt(buffer, BYTE_BUFFER_OFFSET_FIELD_OFFSET);
  }

  /**
   * Allocate a new direct {@link ByteBuffer} that is aligned on a given alignment boundary.
   *
   * <p><b>Note:</b> Aligned buffers are a {@link ByteBuffer#slice()} so cannot be passed to
   * {@link #free(ByteBuffer)}.
   *
  ...
  /**
   * Free the underlying direct {@link ByteBuffer} by invoking {@code Cleaner} on it. No op if
   * {@code null} or if the underlying {@link ByteBuffer} non-direct.
   *
  ...
  /**
   * Free direct {@link ByteBuffer} by invoking {@code Cleaner} on it. No op if {@code null} or
