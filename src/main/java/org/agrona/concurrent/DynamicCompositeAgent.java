...
public class DynamicCompositeAgent implements Agent {
   ...
   * {@inheritDoc}
   *
   * <p>Note that one agent throwing an exception on start may result in other agents not being
   * started.
   */
  public void onStart() {
    for (final Agent agent : agents) {
   ...
   * {@inheritDoc}
   *
   * <p>Note that one agent throwing an exception on close will not prevent other agents from being
   * closed.
   *
   * @throws RuntimeException if any sub-agent throws an exception onClose. The agents exceptions
   *     are collected as suppressed exceptions in the thrown exception.
   */
  public void onClose() {
  ...
  /**
   * Try and add a new {@link Agent} to the composite. This method does not block and will return
   * false if another concurrent attempt to add is in progress.
   *
   ...
   * @return true is a successful add request is pending otherwise false if another concurrent add
  ...
  /**
   * Has the last successful {@link #tryAdd(Agent)} operation been processed in the
   * {@link #doWork()} cycle?
   *
   * @return the last successful {@link #tryAdd(Agent)} operation been processed in the
   *     {@link #doWork()} cycle?
   * @see #tryAdd(Agent)
   */
  ...
  /**
   * Try and remove an {@link Agent} from the composite. The agent is removed during the next
   * {@link #doWork()} duty cycle if this operation is successful. This method does not block and
   * will return false if another concurrent attempt to remove is in progress.
   *
   * <p>The {@link Agent} is removed by identity. Only the first found is removed.
   *
   * @param agent to be removed.
   * @return true is a successful remove request is pending otherwise false if another concurrent
   *     remove request is in progress.
   ...
   *
   * @return the last {@link #tryRemove(Agent)} operation been processed in the {@link #doWork()}
   *     cycle?
   * @see #tryRemove(Agent)
   */
