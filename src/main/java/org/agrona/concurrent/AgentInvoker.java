...
/**
 * {@link Agent} container which does not start a thread. It instead allows the duty cycle
 * {@link Agent#doWork()} to be invoked directly. {@link #start()} should be called to allow the
 * {@link Agent#onStart()} to fire before any calls to {@link #invoke()} of the agent duty cycle.
 *
 * <p>Exceptions which occur during the {@link Agent#doWork()} invocation will be caught and passed
 * to the provided {@link ErrorHandler}.
 *
...
public class AgentInvoker implements AutoCloseable {
   ...
   */
  public AgentInvoker(
      final ErrorHandler errorHandler, final AtomicCounter errorCounter, final Agent agent) {
   ...
   *
   * <p>Startup logic will only be performed once.
   */
  public void start() {
    try {
      if (!isStarted) {
   ...
   *
   * <p>If an error occurs then the {@link AtomicCounter#increment()} will be called on the
   * errorCounter if not null and the {@link Throwable} will be passed to the
   * {@link ErrorHandler#onError(Throwable)} method. If the error is an
   * {@link AgentTerminationException} then {@link #close()} will be called after the error handler.
   *
   * <p>If not successfully started or after closed then this method will return without invoking
   * the {@link Agent}.
   *
   ...
   *
   * <p>The cleanup logic will only be performed once.
   */
