...
public abstract class AbstractConcurrentArrayQueue<E> extends AbstractConcurrentArrayQueuePadding3
  ...
  static {
    try {
      BUFFER_ARRAY_BASE = UnsafeApi.U.arrayBaseOffset(Object[].class);
      SHIFT_FOR_SCALE = BitUtil.calculateShiftForScale(UnsafeApi.U.arrayIndexScale(Object[].class));
      TAIL_OFFSET = UnsafeApi.U.objectFieldOffset(
          AbstractConcurrentArrayQueueProducer.class.getDeclaredField("tail"));
      SHARED_HEAD_CACHE_OFFSET = UnsafeApi.U.objectFieldOffset(
  ...
  @SuppressWarnings("unchecked")
  public E peek() {
    return (E) UnsafeApi.U.getReferenceVolatile(buffer, sequenceToBufferOffset(head, capacity - 1));
  }
  ...
  public boolean contains(final Object o) {
    ...
    for (long i = head, limit = tail; i < limit; i++) {
      final Object e = UnsafeApi.U.getReferenceVolatile(buffer, sequenceToBufferOffset(i, mask));
      if (o.equals(e)) {
        return true;
      }
    }
