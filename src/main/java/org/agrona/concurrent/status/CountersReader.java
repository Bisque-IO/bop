 ...
 *
 * <p>This class is threadsafe and can be used across threads.
 *
 * <p><b>Values Buffer</b>
 *
 * <pre>
...
public class CountersReader {
  ...
  /**
   * Offset in the record at which the registration id field is stored. When a counter is allocated
   * the action can be given a registration id to indicate a specific term of use. This can be
   * useful to differentiate the reuse of a counter id for another purpose even with the same type
   * id.
   */
  public static final int REGISTRATION_ID_OFFSET = SIZE_OF_LONG;

  /**
   * Offset in the record at which the owner id field is stored. The owner is an abstract concept
   * which can be used to associate counters to an owner for lifecycle management.
   */
  public static final int OWNER_ID_OFFSET = REGISTRATION_ID_OFFSET + SIZE_OF_LONG;

  /**
   * Offset in the record at which the reference id field is stored. This id can be used to
   * associate this counter with a registration id for something else, such as an Image,
  ...
  public static final int MAX_KEY_LENGTH =
      (CACHE_LINE_LENGTH * 2) - (SIZE_OF_INT * 2) - SIZE_OF_LONG;
   ...
   *
   * <p>Counter labels default to {@link StandardCharsets#UTF_8}.
   *
  ...
  public void forEach(final CounterConsumer consumer) {
    ...
    for (int offset = 0, capacity = metaDataBuffer.capacity();
        offset < capacity;
    ...
    for (int offset = 0, capacity = metaDataBuffer.capacity();
      ...
      if (RECORD_ALLOCATED == recordStatus) {
        ...
        final DirectBuffer keyBuffer =
            new UnsafeBuffer(metaDataBuffer, offset + KEY_OFFSET, MAX_KEY_LENGTH);
  ...
  /**
   * Iterate over allocated counters and find the first matching a given type id and registration
   * id.
   *
  ...
  /**
   * Get the registration id for a given counter id as a volatile read. The registration identity
   * may be assigned when the counter is allocated to help avoid ABA issues if the counter id is
   * reused.
   *
   * @param counterId to be read.
  ...
  /**
   * Get the owner id for a given counter id as a normal read. The owner identity may be assigned
   * when the counter is allocated to help associate it with the abstract concept of an owner for
   ...
   * counter is allocated to help associate this counter with a registration id for an Image,
   * Subscription, Publication, etc.
   *
   * @param counterId to be read.
   ...
   *
   * @param counterId to be read.
   * @return deadline (ms) for when a given counter id may be reused or {@link #NOT_FREE_TO_REUSE}
   *     if currently in use.
   */
