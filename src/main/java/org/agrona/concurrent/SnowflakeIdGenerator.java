...
abstract class AbstractSnowflakeIdGeneratorValue extends AbstractSnowflakeIdGeneratorPaddingLhs {
  static final AtomicLongFieldUpdater<AbstractSnowflakeIdGeneratorValue>
      TIMESTAMP_SEQUENCE_UPDATER = AtomicLongFieldUpdater.newUpdater(
          AbstractSnowflakeIdGeneratorValue.class, "timestampSequence");
...
/**
 * Generate unique identifiers based on the Twitter <a
 * href="https://github.com/twitter-archive/snowflake/tree/snowflake-2010">Snowflake</a> algorithm.
 *
 * <p>This implementation is lock-less resulting in greater throughput plus less contention and
 * latency jitter.
 *
 * <p><b>Note:</b> ntpd, or alternative clock source, should be setup correctly to ensure the clock
 * does not go backwards.
 */
public final class SnowflakeIdGenerator extends AbstractSnowflakeIdGeneratorPaddingRhs
  ...
  /**
   * Total number of bits used to represent the distributed node and the sequence within a
   * millisecond.
   */
   ...
   * {@code 10} bits allowing for 1024 nodes (0-1023).
   */
  public static final int NODE_ID_BITS_DEFAULT = 10;

  /**
   * Default number of bits used to represent the sequence within a millisecond which is {@code 12}
   * bits supporting 4,096,000 ids per-second per-node.
   */
   ...
   * @param timestampOffsetMs to adjust the base offset from 1 Jan 1970 UTC to extend the 69-year
   *     range.
  ...
  public SnowflakeIdGenerator(
    ...
    if (nodeIdAndSequenceBits > MAX_NODE_ID_AND_SEQUENCE_BITS) {
      throw new IllegalArgumentException("too many bits used:" + " nodeIdBits="
          + nodeIdBits + " + sequenceBits=" + sequenceBits + " > "
    ...
    if (timestampOffsetMs > nowMs) {
      throw new IllegalArgumentException(
          "timestampOffsetMs=" + timestampOffsetMs + " > nowMs=" + nowMs);
    }
  ...
  /**
   * Construct a new Snowflake id generator for a given node with a 0 offset from 1 Jan 1970 UTC and
   * use {@link SystemEpochClock#INSTANCE} with {@link #NODE_ID_BITS_DEFAULT} node ID bits and
  ...
  /**
   * Offset in milliseconds from Epoch of 1 Jan 1970 UTC which is subtracted to give 69 years of
   * ids.
   *
   * <p>To offset from 1 Jan 2015 UTC then 1420070400000 can be used.
   *
   * @return offset in milliseconds from Epoch of 1 Jan 1970 UTC which is subtracted to give 69
   *     years of ids.
   */
  ...
  /**
   * Generate the next id in sequence. If {@link #maxSequence()} is reached within the same
   * millisecond then this implementation will busy spin until the next millisecond using
   * {@link Thread#onSpinWait()} and checking for {@link Thread#isInterrupted()}.
   *
