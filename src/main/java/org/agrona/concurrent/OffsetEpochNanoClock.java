...
/**
 * An accurate, zero-gc, pure-java, {@link EpochNanoClock} that calculates an initial epoch nano
 * time based on {@link System#currentTimeMillis()} and then uses that offset to adjust the return
 * value of {@link System#nanoTime()} to the UNIX epoch.
 *
 * <p>The {@link #sample()} method can be used in order to reset these initial values if your system
 * clock gets updated.
 *
 * <p>This class can be used from multiple threads safely.
 *
 * @see SystemEpochNanoClock
 */
public class OffsetEpochNanoClock implements EpochNanoClock {
   ...
   *
   * @param maxMeasurementRetries the maximum number of times that this clock will attempt to
   *     re-sample the initial time values.
   * @param measurementThresholdNs the desired accuracy window for the initial clock samples.
   * @param resampleIntervalNs the desired interval before the samples are automatically
   *     recalculated. The seed recalculation enables the system to minimise clock drift if the
   *     system clock is updated.
   */
  @SuppressWarnings("this-escape")
  public OffsetEpochNanoClock(
      final int maxMeasurementRetries,
      final long measurementThresholdNs,
      final long resampleIntervalNs) {
  ...
  public void sample() {
    ...
    timeFields = new TimeFields(bestInitialCurrentNanoTime, bestInitialNanoTime, false);
  }
