...
/**
 * A logging {@link ErrorHandler} that records to a {@link DistinctErrorLog} and if the log is full
 * then overflows to a {@link PrintStream}. If closed then error will be sent to
 * {@link #errorOverflow()}.
 */
public class LoggingErrorHandler implements ErrorHandler, AutoCloseable {
  ...
  /**
   * Construct error handler wrapping a {@link DistinctErrorLog} with a default of
   * {@link System#err} for the {@link #errorOverflow()}.
   *
   * @param log to wrap.
   */
  ...
  /**
   * Construct error handler wrapping a {@link DistinctErrorLog} and {@link PrintStream} for error
   * overflow.
   *
   * @param log to wrap.
  ...
  /**
   * Close error handler so that is does not attempt to write to underlying storage which may be
   * unmapped.
   */
  public void close() {
    isClosed = true;
