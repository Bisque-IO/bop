 ...
 *
 * <p>This is code adapted from <a href="https://netty.io/">the Netty project</a> adopted to support
 * {@link org.agrona.DirectBuffer}.
 */
public final class PrintBufferUtil {
   ...
   */
  public static String hexDump(final org.agrona.DirectBuffer buffer) {
  ...
  public static String hexDump(
      final org.agrona.DirectBuffer buffer, final int fromIndex, final int length) {
    return HexUtil.hexDump(buffer, fromIndex, length);
  }

  /**
   * Returns a <a href="http://en.wikipedia.org/wiki/Hex_dump">hex dump</a> of the specified byte
   * array.
   *
  ...
  /**
   * Returns a hexadecimal dump of the specified {@link org.agrona.DirectBuffer} that is easy to
   * read by humans.
   *
   * @param buffer dumped buffer.
   * @return pretty hex dump in a string representation.
   */
  public static String prettyHexDump(final org.agrona.DirectBuffer buffer) {
    return prettyHexDump(buffer, 0, buffer.capacity());
  }

  /**
   * Returns a hexadecimal dump of the specified {@link org.agrona.DirectBuffer} that is easy to
   * read by humans, starting at the given {@code offset} using the given {@code length}.
   *
   * @param buffer dumped buffer.
   ...
   * {@link org.agrona.DirectBuffer} to the specified {@link StringBuilder} that is easy to read by
   * humans.
   *
  ...
  public static void appendPrettyHexDump(
      final StringBuilder dump, final org.agrona.DirectBuffer buffer) {
   ...
   * Appends the prettified multi-line hexadecimal dump of the specified
   * {@link org.agrona.DirectBuffer} to the specified {@link StringBuilder} that is easy to read by
   * humans, starting at the given {@code offset} using the given {@code length}.
   *
   * @param dump where should we append string representation of the buffer.
   * @param buffer dumped buffer.
   * @param offset where should we start to print.
  ...
  static final class HexUtil {
     ...
     */
    static short getUnsignedByte(final org.agrona.DirectBuffer buffer, final int index) {
     ...
     * @return {@code true} if the requested {@code index} and {@code length} will fit within
     *     {@code capacity}. {@code false} if this would result in an index out of bounds exception.
     */
    ...
    static void appendHexDumpRowPrefix(
        final StringBuilder dump, final int row, final int rowStartIndex) {
