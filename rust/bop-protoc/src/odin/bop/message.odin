...
default_buffer :: proc(allocator := context.allocator) -> Buffer {
	return Buffer{allocator = allocator}
}

make_buffer :: proc(allocator := context.allocator) -> (^Buffer, runtime.Allocator_Error) {
...
buffer_reset :: proc "contextless" (buf: ^Buffer) {
...
test_allocate :: proc(t: ^testing.T) {
	...
	bar_id_get :: #force_inline proc "contextless" (self: Ref(Bar)) -> i64 {
		return self.id
	}

	bar_enabled :: #force_inline proc "contextless" (self: Ref(Bar)) -> bool {
	...
	bar_name_get :: #force_inline proc "contextless" (self: Ref(Bar)) -> string {
		return _str_of(self._buf, &self.name)
	}

	bar_name_set :: #force_inline proc "contextless" (self: Ref(Bar), value: string) {

	}

	//    bar_id_get :: proc "contextless" (self: Msg(Bar)) -> i64 {
	//        return self.id
	//    }
	//
	//    bar_name :: proc{bar_name_get, bar_name_set}
	//
	//    bar_name_get :: proc "contextless" (self: Msg(Bar)) -> string {
	//        return _str_of(self._buf, &self.name)
	//    }
	//
	//    bar_name_set :: proc "contextless" (self: Msg(Bar), value: string) {
	//
	//    }
