...
use crate::usockets::{Loop, Socket, SocketContext, SslMode, SocketContextOptions};
...
impl TokenBucketRateLimiter {
    ...
    pub fn try_consume_bytes(&self, bytes: usize) -> bool {
        ...
        if current >= bytes {
            ...
            match self.tokens.compare_exchange_weak(current, current - bytes, Ordering::Relaxed, Ordering::Relaxed) {
                Ok(_) => true,
...
impl ProductionSocket {
    ...
    pub fn write(&self, data: &[u8]) -> Result<bool, String> {
        ...
        {
            let mut buffer = self.write_buffer.lock().map_err(|e| format!("Lock error: {}", e))?;
    ...
    pub fn drain_write_buffer(&self) -> Result<usize, String> {
        ...
        while !buffer.is_empty() && bytes_processed < chunk_size {
            if let Some(entry) = buffer.pop_front() {
                ...
                self.stats.bytes_sent.fetch_add(entry.data.len(), Ordering::Relaxed);
            }
        }

        // Update buffer size
        self.buffer_size.fetch_sub(bytes_processed, Ordering::Relaxed);
