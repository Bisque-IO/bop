...
fn demonstrate_basic_allocations() {
    ...
    if stats_after.has_leaks() {
        println!("⚠️  Memory leaks detected: {} outstanding allocations", 
...
fn demonstrate_collection_growth() {
    ...
    for i in 0..5 {
        ...
        println!("After adding {} items (total: {}): Current memory: {} bytes, Peak: {} bytes", 
                 items_to_add, vec.len(), stats.current_memory, stats.peak_memory);
    }
    ...
    println!("After clearing vector: Current memory: {} bytes", final_stats.current_memory);
}
...
fn demonstrate_memory_leaks() {
    ...
    
    {
        ...
        let _leaked_boxes: Vec<*mut u64> = (0..10)
        ...
        if stats_with_leaks.outstanding_allocations() > stats_before.outstanding_allocations() {
            println!("🔴 Leak detected: {} new outstanding allocations", 
...
fn demonstrate_peak_usage() {
    ...
    for size_mb in [1, 5, 10, 3, 7] {
        {
            ...
            let stats = GLOBAL.stats();
            println!("Allocated {} MB: Current = {} MB, Peak = {} MB", 
                     size_mb,
        ...
        let stats_after = GLOBAL.stats();
        println!("After freeing {} MB: Current = {} MB, Peak = {} MB", 
                 size_mb,
    ...
    println!("Total peak increase during test: {} MB", peak_increase / (1024 * 1024));
}
...
fn final_report() {
    ...
    println!("Outstanding allocations: {}", stats.outstanding_allocations());
    println!("Total bytes allocated: {} ({:.2} MB)", 
             stats.bytes_allocated, 
             stats.bytes_allocated as f64 / (1024.0 * 1024.0));
    println!("Total bytes deallocated: {} ({:.2} MB)", 
             stats.bytes_deallocated,
             stats.bytes_deallocated as f64 / (1024.0 * 1024.0));
    println!("Current memory usage: {} ({:.2} MB)", 
    ...
    if stats.has_leaks() {
        ...
        println!("  - {} outstanding allocations", stats.outstanding_allocations());
        println!("  - ~{} bytes potentially leaked", stats.outstanding_memory());
    ...
    if stats.allocations > 0 {
        ...
        println!("  - Allocation/deallocation ratio: {:.2}", 
