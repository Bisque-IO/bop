...
fn demonstrate_heap_allocations() {
    ...
    println!("Vector with {} elements, capacity: {}", vec.len(), vec.capacity());
}
...
fn demonstrate_custom_types() {
    ...
    println!("Custom struct with {} data items and {} metadata entries", 
             custom.data.len(), custom.metadata.len());
    
    // Allocate many instances
    let instances: Vec<CustomStruct> = (0..10)
        .map(|_| CustomStruct::new())
        .collect();
...
fn demonstrate_direct_allocation() {
    ...
    unsafe {
        ...
        if !ptr.is_null() {
            ...
            println!("First byte: 0x{:02X}, Last byte: 0x{:02X}", first_byte, last_byte);
    ...
    unsafe {
        ...
        if !initial_ptr.is_null() {
            ...
            if !expanded_ptr.is_null() {
                ...
                println!("Reallocated and preserved value: 0x{:08X}", preserved_value as u32);
