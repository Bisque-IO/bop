...
use bop_rs::aof::record::{FlushCheckpointStamp, SegmentMetadataRecord};
use bop_rs::aof::{
    Aof, AofConfig, AofError, AofResult, AsyncFileSystem, FilesystemArchiveStorage, FlushStrategy,
    SegmentFooter,
};
...
fn build_config(segment_size: u64, index_path: String) -> AofConfig {
    ...
    config.pre_allocate_enabled = false;
...
fn find_segment_file(temp: &TempDir) -> AofResult<std::path::PathBuf> {
    ...
    for (id, size, path) in &candidates {
        println!("DEBUG candidate id {} size {} path {:?}", id, size, path);
    }
...
async fn finalization_writes_metadata_and_footer() -> AofResult<()> {
    ...
    println!("DEBUG using segment path {:?}", segment_path);
    ...
    println!("DEBUG bytes sample: {:?}", &bytes[..200.min(bytes.len())]);
    println!(
        "DEBUG first occurrence of 97: {:?}",
        bytes.iter().position(|&b| b == 97)
    );
    let stamp_magic = FlushCheckpointStamp::MAGIC.to_le_bytes();
    println!(
        "DEBUG stamp positions: {:?}",
        bytes
            .windows(stamp_magic.len())
            .enumerate()
            .filter(|(_, window)| *window == stamp_magic)
            .map(|(i, _)| i)
            .collect::<Vec<_>>()
    );
...
#[tokio::test(flavor = "multi_thread")]
async fn reopen_recovers_next_id_and_existing_records() -> AofResult<()> {
    let temp = new_tempdir()?;
    ...
    let mut reopened = create_aof(&temp).await?;
    assert_eq!(reopened.next_id(), 3);

    let mut reader = reopened.create_reader_from_id(1)?;
    let first = reader
        .read_next_record()
        .await?
        .expect("expected record 1 after reopen");
    assert_eq!(first, b"one");

    let second = reader
        .read_next_record()
        .await?
        .expect("expected record 2 after reopen");
    assert_eq!(second, b"two");

    assert!(reader.read_next_record().await?.is_none());

    let third_id = reopened.append(b"three")?;
