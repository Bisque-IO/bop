...
use bop_rs::usockets::{
    Loop, Socket, SocketContext, SocketContextOptions, SslMode, Timer
};
...
fn test_loop_callbacks() {
    let mut loop_ = Loop::new().expect("create loop");
...
fn test_socket_context_creation_plain() {
    ...
    assert_eq!(ctx.ssl_mode(), SslMode::Plain);
...
fn test_tls_server_context_with_options_and_integration() {
    ...
    match ctx {
        Ok(mut tls_ctx) => {
            assert_eq!(tls_ctx.ssl_mode(), SslMode::Tls);
