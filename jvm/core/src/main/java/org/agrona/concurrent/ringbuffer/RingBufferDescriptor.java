...
/**
 * Layout description for the underlying buffer used by a {@link RingBuffer}. The buffer consists of
 * a ring of messages which is a power of 2 in size, followed by a trailer section containing state
...
public final class RingBufferDescriptor {
   ...
   * {@link RingBufferDescriptor#TRAILER_LENGTH}) and that it satisfies minimum capacity size.
   *
   * @param capacity to be checked.
   * @param minCapacity minimum required capacity of the ring buffer.
   * @return the capacity to store the data in the ring buffer, i.e. {@code capacity -
   *     TRAILER_LENGTH}.
  ...
  public static int checkCapacity(final int capacity, final int minCapacity) {
    ...
    if (dataCapacity < minCapacity) {
      throw new IllegalArgumentException("insufficient capacity: minCapacity="
          + (minCapacity + TRAILER_LENGTH) + ", capacity=" + capacity);
    }
    return dataCapacity;
  }
}
