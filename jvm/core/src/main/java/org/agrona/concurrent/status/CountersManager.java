 ...
 *
 * <p>This class in not threadsafe. Counters should be centrally managed.
 *
 * <p><b>Values Buffer</b>
 *
 * <pre>
...
public class CountersManager extends CountersReader {
   ...
   * @param epochClock to use for determining time for keep counter from being reused after being
   *     freed.
   * @param freeToReuseTimeoutMs timeout (in milliseconds) to keep counter from being reused after
   *     being freed.
   */
  public CountersManager(
    ...
    if (metaDataBuffer.capacity()
        < (valuesBuffer.capacity() * (METADATA_LENGTH / COUNTER_LENGTH))) {
  ...
  /**
   * Number of counters available to be allocated which is a function of {@link #capacity()} minus
   * the number already allocated.
   *
  ...
  public int available() {
    ...
    if (!freeList.isEmpty()) {
      ...
      for (int i = 0, size = freeList.size(); i < size; i++) {
        ...
        if (nowMs
            >= metaDataBuffer.getLong(metaDataOffset(counterId) + FREE_FOR_REUSE_DEADLINE_OFFSET)) {
          freeListCount++;
        }
      }
    }
   ...
   *
   * <p>The key function will be called with a buffer with the exact length of available key space
   ...
   */
  public int allocate(
      final String label, final int typeId, final Consumer<MutableDirectBuffer> keyFunc) {
    final int counterId = nextCounterId();
  ...
  /**
   * Allocate a counter with the minimum of allocation by allowing the label a key to be provided
   * and copied.
   *
   * <p>If the keyBuffer is null then a copy of the key is not attempted.
   *
  ...
  public int allocate(
    ...
    try {
      ...
      metaDataBuffer.putBytes(
          recordOffset + LABEL_OFFSET + SIZE_OF_INT, labelBuffer, labelOffset, length);
   ...
   * {@link org.agrona.concurrent.status.AtomicCounter} for use with a default type of
   * {@link #DEFAULT_TYPE_ID}.
   *
   * @param label to describe the counter.
   * @return a newly allocated {@link org.agrona.concurrent.status.AtomicCounter}
   */
  public org.agrona.concurrent.status.AtomicCounter newCounter(final String label) {
    return new org.agrona.concurrent.status.AtomicCounter(valuesBuffer, allocate(label), this);
  }

  /**
   * Allocate a counter record and wrap it with a new
   * {@link org.agrona.concurrent.status.AtomicCounter} for use.
   *
   * @param label to describe the counter.
   * @param typeId for the type of counter.
   * @return a newly allocated {@link org.agrona.concurrent.status.AtomicCounter}
   */
  public org.agrona.concurrent.status.AtomicCounter newCounter(
      final String label, final int typeId) {
    return new org.agrona.concurrent.status.AtomicCounter(
        valuesBuffer, allocate(label, typeId), this);
  }
  ...
  public org.agrona.concurrent.status.AtomicCounter newCounter(
      final String label, final int typeId, final Consumer<MutableDirectBuffer> keyFunc) {
    return new org.agrona.concurrent.status.AtomicCounter(
        valuesBuffer, allocate(label, typeId, keyFunc), this);
  }
  ...
  public void free(final int counterId) {
    ...
    metaDataBuffer.putLong(
        offset + FREE_FOR_REUSE_DEADLINE_OFFSET, epochClock.time() + freeToReuseTimeoutMs);
    freeList.addInt(counterId);
  }

  /**
   * Set an {@link org.agrona.concurrent.status.AtomicCounter} value based for a counter id with
   * ordered memory ordering.
   *
  ...
  /**
   * Set an {@link org.agrona.concurrent.status.AtomicCounter} registration id for a counter id with
   * ordered memory ordering.
   *
   * @param counterId to be set.
  ...
  /**
   * Set an {@link org.agrona.concurrent.status.AtomicCounter} owner id for a counter id.
   *
   * @param counterId to be set.
  ...
  /**
   * Set an {@link org.agrona.concurrent.status.AtomicCounter} reference id for a counter id.
   *
   * @param counterId to be set.
  ...
  /**
   * Set an {@link org.agrona.concurrent.status.AtomicCounter} label by counter id.
   *
   * @param counterId to be set.
  ...
  /**
   * Set an {@link org.agrona.concurrent.status.AtomicCounter} key by on counter id, using a
   * consumer callback to update the key metadata buffer.
   *
   * @param counterId to be set.
  ...
  public void setCounterKey(final int counterId, final Consumer<MutableDirectBuffer> keyFunc) {
    validateCounterId(counterId);
    keyFunc.accept(
        new UnsafeBuffer(metaDataBuffer, metaDataOffset(counterId) + KEY_OFFSET, MAX_KEY_LENGTH));
  }

  /**
   * Set an {@link org.agrona.concurrent.status.AtomicCounter} key by on counter id, copying the key
   * metadata from the supplied buffer.
   *
   * @param counterId to be set.
   ...
   */
  public void setCounterKey(
      final int counterId, final DirectBuffer keyBuffer, final int offset, final int length) {
    validateCounterId(counterId);
  ...
  public String toString() {
    return getClass().getSimpleName() + "{" + "freeToReuseTimeoutMs="
        + freeToReuseTimeoutMs + ", highWaterMarkId="
        + highWaterMarkId + ", freeList="
        + freeList + ", epochClock="
        + epochClock + '}';
  }
  ...
  private void checkCountersCapacity(final int counterId) {
    if (counterId > maxCounterId) {
      throw new IllegalStateException(
          "unable to allocate counter, buffer is full: maxCounterId=" + maxCounterId);
    }
  }
}
