 ...
 * {@link org.agrona.concurrent.broadcast.BroadcastReceiver}s.
 *
 * <p><b>Note:</b> This class is not threadsafe. Only one transmitter is allowed per broadcast
 * buffer.
 */
public class BroadcastTransmitter {
  ...
  /**
   * Construct a new broadcast transmitter based on an underlying {@link AtomicBuffer}. The
   ...
   * @throws IllegalArgumentException of the msgTypeId is not valid, or if the message length is
   *     greater than {@link #maxMsgLength()}.
   */
  ...
  private static void insertPaddingRecord(
      final AtomicBuffer buffer, final int recordOffset, final int length) {
