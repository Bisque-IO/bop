...
/**
 * Distinct record of error observations. Rather than grow a record indefinitely when many errors of
 * the same type are logged, this log takes the approach of only recording distinct errors of the
 * same type and stack trace and keeping a count and time of observation so that the record only
 * grows with new distinct observations.
 *
 * <p>The provided {@link AtomicBuffer} can wrap a memory-mapped file so logging can be out of
 * process. This provides the benefit that if a crash or lockup occurs then the log can be read
 * externally without loss of data.
 *
 * <p><b>Note:</b> This class is threadsafe to be used from multiple logging threads.
 *
 * <p>The error records are recorded to the memory mapped buffer in the following format.
 *
 * <pre>
 *   0                   1                   2                   3
...
public class DistinctErrorLog {
  ...
  public static final int LAST_OBSERVATION_TIMESTAMP_OFFSET =
      OBSERVATION_COUNT_OFFSET + SIZE_OF_INT;
  ...
  public static final int FIRST_OBSERVATION_TIMESTAMP_OFFSET =
      LAST_OBSERVATION_TIMESTAMP_OFFSET + SIZE_OF_LONG;
   ...
   *
   * <p>The {@link Charset} with default to {@link StandardCharsets#UTF_8} for encoding the
   * exceptions.
   *
  ...
  public DistinctErrorLog(
      final AtomicBuffer buffer, final EpochClock clock, final Charset charset) {
  ...
  /**
   * Record an observation of an error. If it is the first observation of this error type for a
   * stack trace then a new entry will be created. For subsequent observations of the same error
   * type and stack trace a counter and time of last observation will be updated.
   *
  ...
  private static boolean equals(
      final StackTraceElement[] lhsStackTrace, final StackTraceElement[] rhsStackTrace) {
