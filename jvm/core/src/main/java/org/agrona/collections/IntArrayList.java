...
public class IntArrayList extends AbstractList<Integer> implements List<Integer>, RandomAccess {
  ...
  public IntArrayList(
      final int[] initialElements, @DoNotSub final int initialSize, final int nullValue) {
  ...
  private void ensureCapacityPrivate(@DoNotSub final int requiredCapacity) {
    ...
    if (requiredCapacity > currentCapacity) {
      if (requiredCapacity > org.agrona.collections.ArrayUtil.MAX_CAPACITY) {
        throw new IllegalStateException(
            "max capacity: " + org.agrona.collections.ArrayUtil.MAX_CAPACITY);
      }
      ...
      while (newCapacity < requiredCapacity) {
        ...
        if (newCapacity < 0 || newCapacity >= org.agrona.collections.ArrayUtil.MAX_CAPACITY) {
