...
/// Transactions are the mechanism used for reading and writing data.
/// Writes are serialized, so there can only be a single write transaction
/// at any given time. Since, MDBX uses MVCC, read-only transactions can
/// run both concurrent and parallel to a write transaction. Read-only
/// transactions do not block the write transaction and vice versa.
/// Each read-only transaction must only be used from a single thread
/// at any given time. It is up to the user to ensure thread-safety.
/// Multiple threads can access a single transaction as long as it is
/// serialized by the user (locking, queues, etc.).
///
///
public class Txn {
  Env env;
  ...
  public Stat stat() {
    ...
    try {
      err = (int) CFunctions.MDBX_ENV_STAT_EX.invokeExact(
          env.envAddress, txnPointer, scratchPointer, Stat.SIZE);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err != Error.SUCCESS) {
  ...
  public int open(Table table) {
    ...
    final int err;
    try {
      err = (int) CFunctions.MDBX_DBI_OPEN.invokeExact(
          txnPointer, table.namePtr, table.flags, scratchPointer);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err != Error.SUCCESS) {
    ...
    table.dbi = Danger.getInt(scratchPointer);
    return err;
  }
  ...
  ///
  /// \returns A non-zero error code, or 0 on success.
  public int rename(Table table, String name) {
    if (name == null) {
      name = "";
    }
    ...
    final int err;
    try {
      err = (int) CFunctions.MDBX_DBI_RENAME.invokeExact(txnPointer, table.dbi, namePtr);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err != Error.SUCCESS) {
    ...
    table.name = name;
    if (table.namePtr != 0L) {
      Memory.dealloc(table.namePtr);
    }
    table.namePtr = namePtr;
    return err;
  }
  ...
  public int stat(Table table) {
    ...
    if (table.statPtr == 0L) {
      table.statPtr = Memory.zalloc(Stat.SIZE);
    }
    final int err;
    try {
      err = (int)
          CFunctions.MDBX_DBI_STAT.invokeExact(txnPointer, table.dbi, table.statPtr, Stat.SIZE);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err == Error.SUCCESS) {
      table.stat.update(table.statPtr);
    }
    return err;
  }
  ...
  ///
  /// \returns A non-zero error value on failure and 0 on success.
  public int flagsAndState(Table table) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    final int err;
    try {
      err = (int) CFunctions.MDBX_DBI_FLAGS_EX.invokeExact(
          txnPointer, table.dbi, scratchPointer, scratchPointer + 8L);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err == Error.SUCCESS) {
      table.flags = Danger.getInt(scratchPointer);
      table.state = Danger.getInt(scratchPointer + 8L);
    }
    return err;
  }
  ...
  ///
  /// \returns A non-zero error value on failure and 0 on success.
  public int close(Table table) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    final int err;
    try {
      err = (int) CFunctions.MDBX_DBI_CLOSE.invokeExact(txnPointer, table.dbi);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err == Error.SUCCESS) {
  ...
  ///
  /// \returns A non-zero error value on failure and 0 on success.
  public int drop(Table table, boolean delete) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    final int err;
    try {
      err = (int) CFunctions.MDBX_DROP.invokeExact(txnPointer, table.dbi, delete);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    if (err == Error.SUCCESS) {
  ...
  /// \retval MDBX_EINVAL    An invalid parameter was specified.
  public int get(Table table, Val key, Val data) {
    assert key != null;
    assert data != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    ...
    try {
      return (int)
          CFunctions.MDBX_GET.invokeExact(txnPointer, table.dbi, key.address(), data.address());
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  public int get(Table table) {
    ...
    return get(table, this.key, this.data);
  }

  public int get(Table table, Val key) {
    assert key != null;
    return get(table, key, this.data);
  }

  public int get(Table table, String key) {
    final var b = Danger.getBytes(key);
    return get(table, b, 0, b.length);
  }

  public int get(Table table, byte[] key) {
    return get(table, key, 0, key.length);
  }

  public int get(Table table, byte[] key, int offset, int length) {
    assert key != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
  ...
  public int get(Table table, long key) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
      return (int)
          CFunctions.BOP_MDBX_GET_INT.invokeExact(txnPointer, table.dbi, key, this.data.address());
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }

  public int get(Table table, long key, Val data) {
    assert data != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
      return (int)
          CFunctions.BOP_MDBX_GET_INT.invokeExact(txnPointer, table.dbi, key, data.address());
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  /// \retval MDBX_NOTFOUND      The key was not in the table.
  /// \retval MDBX_EINVAL        An invalid parameter was specified.
  public int getEqualOrGreater(Table table, Val key, Val data) {
    assert key != null;
    assert data != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    ...
    try {
      return (int) CFunctions.MDBX_GET_EQUAL_OR_GREATER.invokeExact(
          txnPointer, table.dbi, key.address(), data.address());
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }

  public int getEqualOrGreater(Table table, String key) {
    final var b = Danger.getBytes(key);
    return getEqualOrGreater(table, b, 0, b.length);
  }

  public int getEqualOrGreater(Table table, byte[] key) {
    return getEqualOrGreater(table, key, 0, key.length);
  }

  public int getEqualOrGreater(Table table, byte[] key, int offset, int length) {
    assert key != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
  ...
  public int getEqualOrGreater(Table table, long key) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    this.key.set(scratchPointer, 8L);
    final int err;
    try {
      err = (int) CFunctions.BOP_MDBX_GET_EQUAL_OR_GREATER_INT.invokeExact(
          txnPointer, table.dbi, key, scratchPointer, this.data.address());
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
    return err;
  ...
  /// \retval MDBX_EINVAL    An invalid parameter was specified.
  public int put(Table table, Val key, Val data, int flags) {
    assert key != null;
    assert data != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    ...
    try {
      return (int) CFunctions.MDBX_PUT.invokeExact(
          txnPointer, table.dbi, key.address(), data.address(), flags);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }

  public int put(Table table, String key, String data, int flags) {
    return put(table, Danger.getBytes(key), Danger.getBytes(data), flags);
  }

  public int put(Table table, String key, byte[] data, int flags) {
    return put(table, Danger.getBytes(key), data, flags);
  }

  public int put(Table table, String key, byte[] data, int offset, int length, int flags) {
    final var k = Danger.getBytes(key);
    return put(table, k, 0, k.length, data, offset, length, flags);
  }

  public int put(Table table, String key, Val data, int flags) {
    return put(table, Danger.getBytes(key), data, flags);
  }

  public int put(Table table, String key, int flags) {
    return put(table, Danger.getBytes(key), data, flags);
  }

  public int put(Table table, byte[] key, byte[] data, int flags) {
    return put(table, key, 0, key.length, data, 0, data.length, flags);
  }

  public int put(Table table, byte[] key, int offset, int length, byte[] data, int flags) {
    return put(table, key, offset, length, data, 0, data.length, flags);
  }

  public int put(Table table, byte[] key, byte[] data, int offset, int length, int flags) {
    return put(table, key, 0, key.length, data, offset, length, flags);
  }
  ...
  public int put(Table table, byte[] key, int flags) {
    return put(table, key, 0, key.length, data, flags);
  }

  public int put(Table table, byte[] key, Val data, int flags) {
    return put(table, key, 0, key.length, data, flags);
  }

  public int put(Table table, byte[] key, int offset, int length, int flags) {
    return put(table, key, offset, length, data, flags);
  }

  public int put(Table table, byte[] key, int keyOffset, int keyLength, Val data, int flags) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
  ...
  public int put(Table table, Val key, byte[] data, int offset, int length, int flags) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
  ...
  public int put(Table table, long key, String data) {
    return put(table, key, data.getBytes(), PutFlags.UPSERT);
  }

  public int put(Table table, long key, String data, int flags) {
    return put(table, key, Danger.getBytes(data), flags);
  }

  public int put(Table table, long key, byte[] data) {
    return put(table, key, data, 0, data.length, PutFlags.UPSERT);
  }

  public int put(Table table, long key, byte[] data, int flags) {
    return put(table, key, data, 0, data.length, flags);
  }

  public int put(Table table, long key, byte[] data, int offset, int length) {
    return put(table, key, data, offset, length, PutFlags.UPSERT);
  }

  public int put(Table table, long key, byte[] data, int offset, int length, int flags) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
  ...
  public int put(Table table, long key) {
    return put(table, key, PutFlags.UPSERT);
  }

  public int put(Table table, long key, int flags) {
    return put(table, key, data, flags);
  }

  public int put(Table table, long key, Val data) {
    return put(table, key, data, PutFlags.UPSERT);
  }

  public int put(Table table, long key, Val data, int flags) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
      return (int) CFunctions.BOP_MDBX_PUT_INT2.invokeExact(
          txnPointer, table.dbi, key, data.address(), flags);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  /// \see \ref c_crud_hints "Quick reference for Insert/Update/Delete operations"
  /// \returns A non-zero error value on failure and 0 on success.
  public int replace(Table table, Val key, Val newData, Val oldData, int flags) {
    assert key != null;
    assert newData != null;
    assert oldData != null;
    if (table == null || table.dbi < 0) {
    ...
    try {
      return (int) CFunctions.MDBX_REPLACE.invokeExact(
          txnPointer, table.dbi, key.address(), newData.address(), oldData.address(), flags);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }

  public int replace(Table table, long key, Val newData, Val oldData, int flags) {
    assert newData != null;
    assert oldData != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    Danger.putLong(scratchPointer, key);
    this.key.base(scratchPointer);
    this.key.len(8L);
    try {
      return (int) CFunctions.MDBX_REPLACE.invokeExact(
          txnPointer, table.dbi, this.key.address(), newData.address(), oldData.address(), flags);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  ///                       in a read-only transaction.
  /// \retval MDBX_EINVAL   An invalid parameter was specified.
  public int delete(Table table, Val key, Val data) {
    assert key != null;
    assert data != null;
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    ...
    try {
      return (int)
          CFunctions.MDBX_DEL.invokeExact(txnPointer, table.dbi, key.address(), data.address());
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }

  public int delete(Table table, byte[] key, int offset, int length) {
    data.clear();
    return delete(table, key, offset, length, data);
  }

  public int delete(Table table, byte[] key, byte[] data, int offset, int length) {
    return delete(table, key, 0, key.length, data, offset, length);
  }

  public int delete(Table table, byte[] key, int offset, int length, byte[] data) {
    return delete(table, key, offset, length, data, 0, data.length);
  }
  ...
  public int delete(Table table, String key) {
    data.clear();
    return delete(table, Danger.getBytes(key), data);
  }

  public int delete(Table table, String key, Val data) {
    return delete(table, Danger.getBytes(key), data);
  }

  public int delete(Table table, byte[] key) {
    return delete(table, key, (Val) null);
  }

  public int delete(Table table, byte[] key, Val data) {
    return delete(table, key, 0, key.length, data);
  }

  public int delete(Table table, byte[] key, int offset, int length, Val data) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
  ...
  public int delete(Table table, Val key, byte[] data, int offset, int length) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
      return (int) CFunctions.BOP_MDBX_DEL3.invokeExact(
          txnPointer, table.dbi, key.address(), MemorySegment.ofArray(data), (long) offset, (long)
              length);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  ...
  public int delete(Table table, long key) {
    return delete(table, key, (Val) null);
  }

  public int delete(Table table, long key, String data) {
    return delete(table, key, Danger.getBytes(data));
  }

  public int delete(Table table, long key, byte[] data) {
    return delete(table, key, data, 0, data.length);
  }

  public int delete(Table table, long key, byte[] data, int offset, int length) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
      return (int) CFunctions.BOP_MDBX_DEL_INT.invokeExact(
          txnPointer, table.dbi, key, MemorySegment.ofArray(data), (long) offset, (long) length);
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }

  public int delete(Table table, long key, Val data) {
    if (table == null || table.dbi < 0) {
      return Error.BAD_DBI;
    }
    try {
      return (int) CFunctions.BOP_MDBX_DEL_INT2.invokeExact(
          txnPointer, table.dbi, key, data == null ? 0L : data.address());
    } catch (Throwable e) {
      throw new RuntimeException(e);
    }
  }
  ...
  ///                               by current thread.
  /// \retval MDBX_EINVAL  An invalid parameter was specified.
  public int openCursor(Table table, Cursor cursor) {
    assert cursor != null;
  ...
  public static class Info {
    ...
    void update(long ptr) {
      txnId = Danger.getLong(ptr);
      txnReaderLag = Danger.getLong(ptr + 8L);
      txnSpaceUsed = Danger.getLong(ptr + 16L);
      txnSpaceLimitSoft = Danger.getLong(ptr + 24L);
      txnSpaceLimitHard = Danger.getLong(ptr + 32L);
      txnSpaceRetired = Danger.getLong(ptr + 40L);
      txnSpaceLeftover = Danger.getLong(ptr + 48L);
      txnSpaceDirty = Danger.getLong(ptr + 56L);
    }
