...
//
/// **
// * Tool for inspecting and performing administrative tasks on an {@link Archive} and its contents
// which is described
// * in a {@link Catalog}.
// */
// public class ArchiveToolXX {
//  /**
...
//        ArchiveToolXX::continueOnFrameLimit);
...
//          emptySet(),
//          null,
//          ArchiveToolXX::truncateOnPageStraddle);
//      } else if (args.length == 3) {
//        if (VerifyOption.VERIFY_ALL_SEGMENT_FILES == VerifyOption.byFlag(args[2])) {
//          hasErrors = !verify(
//            out,
//            archiveDir,
//            EnumSet.of(VerifyOption.VERIFY_ALL_SEGMENT_FILES),
//            null,
//            ArchiveToolXX::truncateOnPageStraddle);
//        } else {
...
//        if (VerifyOption.APPLY_CHECKSUM == VerifyOption.byFlag(args[2])) {
//          hasErrors = !verify(
//            out,
//            archiveDir,
//            EnumSet.of(VerifyOption.APPLY_CHECKSUM),
...
//      out.print(
//        "WARNING: please ensure archive is not running and that a backup has been taken of " +
...
//      out.print(
//        "WARNING: Compacting the Catalog is non-recoverable operation! All recordings in state " +
...
//
//  /**
//   * Set the capacity in bytes of the {@link Catalog}. If new capacity is smaller than current
// {@link Catalog}
...
//      catalog.forEach(
//        (recordingDescriptorOffset, he, hd, e, d) -> out.println(d + "|" + hd.state()));
//    }
//  }
//
//  public static void loadRecordingDescriptors(
//    final File archiveDir,
//    final Long2ObjectHashMap<RecordingDescriptor> recordings
//  ) {
//    try (Catalog catalog = openCatalogReadOnly(archiveDir, INSTANCE)) {
//      catalog.forEach(new CatalogEntryProcessor() {
//        @Override public void accept(
//          int recordingDescriptorOffset, RecordingDescriptorHeaderEncoder headerEncoder,
...
//          var rec = recordings.get(descriptorDecoder.recordingId());
//          if (rec == null) {
//            rec = new RecordingDescriptor();
//            recordings.put(descriptorDecoder.recordingId(), rec);
//          }
//          rec.set(descriptorDecoder.controlSessionId(),
//            descriptorDecoder.correlationId(),
...
//   */
//  public static void describeRecording(final PrintStream out, final File archiveDir, final long
// recordingId)
//  {
...
//   *
//   * @param archiveDir containing the {@link MarkFile}.
...
//
//  /**
//   * Print the errors in the {@link MarkFile}.
//   *
...
//
//  /**
//   * Dump the contents of an {@link Archive} so it can be inspected or debugged. Each recording
// will have its
...
//   * @param fragmentCountLimit                limit of data fragments to print from each recording
// before continue.
...
//    final ArchiveTool.ActionConfirmation<Long> confirmActionOnFragmentCountLimit)
//  {
...
//
//      final SimpleFragmentHandler fragmentHandler = (buffer, offset, length, frameType, flags,
// reservedValue) ->
//      {
...
//
//      catalog.forEach(
//        (recordingDescriptorOffset, headerEncoder, headerDecoder, descriptorEncoder,
// descriptorDecoder) -> dump(
//          out,
//          archiveDir,
//          catalog,
//          fragmentCountLimit,
...
//
//  /**
//   * Set of options for {@link #verify(PrintStream, File, Set, String,
// ArchiveTool.ActionConfirmation)} and
//   * {@link #verifyRecording(PrintStream, File, long, Set, String,
// ArchiveTool.ActionConfirmation)} methods.
//   */
...
//    }
//
//    private static final Map<String, ArchiveTool.VerifyOption> BY_FLAG = Stream.of(values())
//      .collect(toMap((opt) -> opt.flag, (opt) -> opt));
//
//    /**
//     * Lookup the {@link ArchiveTool.VerifyOption} by string format of the command line flag.
//     *
//     * @param flag command line option.
//     * @return the {@link ArchiveTool.VerifyOption} which matches the flag or null if not found.
//     */
//    public static ArchiveTool.VerifyOption byFlag(final String flag)
//    {
...
//   * @param archiveDir             that contains {@link MarkFile}, {@link Catalog}, and
// recordings.
//   * @param options                set of options that control verification behavior.
//   * @param truncateOnPageStraddle action to perform if last fragment in the max segment file
// straddles the page
//   *                               boundary, i.e. if {@code true} the file will be truncated (last
// fragment
//   *                               will be deleted), if {@code false} the fragment is considered
// complete.
//   * @param checksumClassName      (optional) fully qualified class name of the {@link Checksum}
// implementation.
//   * @return {@code true} if no errors have been encountered, {@code false} otherwise (faulty
// entries are marked
...
//    final PrintStream out,
//    final File archiveDir,
//    final Set<ArchiveTool.VerifyOption> options,
//    final String checksumClassName,
//    final ArchiveTool.ActionConfirmation<File> truncateOnPageStraddle)
//  {
...
//
//  /**
//   * Verify descriptor in the catalog according to recordingId, checking recording files
...
//   *                               boundary, i.e. if {@code true} the file will be truncated (last
// fragment
//   *                               will be deleted), if {@code false} the fragment if considered
// complete.
//   * @param checksumClassName      (optional) fully qualified class name of the {@link Checksum}
// implementation.
//   * @return {@code true} if no errors have been encountered, {@code false} otherwise (the
// recording is marked
//   * as unusable if {@code false} is returned)
...
//
//  /**
//   * Compute and persist CRC-32(c) checksums for every fragment of a segment file for all
// recordings in the catalog.
//   *
//   * @param out               stream to print results and errors to.
//   * @param archiveDir        that contains {@link MarkFile}, {@link Catalog}, and recordings.
//   * @param allFiles          should compute checksums for all segment file or only for the last
// one.
//   * @param checksumClassName fully qualified class name of the {@link Checksum} implementation.
//   */
//  public static void checksum(
//    final PrintStream out, final File archiveDir, final boolean allFiles, final String
// checksumClassName)
//  {
//    checksum(out, archiveDir, allFiles, newInstance(validateChecksumClass(checksumClassName)),
// INSTANCE);
//  }
//
//  /**
//   * Compute and persist CRC-32(c) checksums for every fragment of a segment file(s) for a given
// recording.
//   *
//   * @param out               stream to print results and errors to.
//   * @param archiveDir        that contains {@link MarkFile}, {@link Catalog}, and recordings.
...
//  {
//    checksumRecording(
//      out, archiveDir, recordingId, allFiles,
// newInstance(validateChecksumClass(checksumClassName)), INSTANCE);
//  }
//
//  /**
...
//
//  /**
//   * Migrate previous archive {@link MarkFile}, {@link Catalog}, and recordings from previous
// version
...
//           Catalog catalog = openCatalogReadWrite(archiveDir, epochClock, MIN_CAPACITY, null,
//             noVersionCheck)) {
...
//      }
//
//      final List<ArchiveMigrationStep> steps =
// ArchiveMigrationPlanner.createPlan(markFileVersion);
...
//
//  /**
//   * Compact Catalog file by removing all recordings in state {@link RecordingState#INVALID}
...
//
//  /**
//   * Delete orphaned recording segments that have been detached, i.e. outside the start and stop
// recording range,
//   * but are not deleted.
//   *
//   * @param out        stream to print results and errors to.
...
//      final Long2ObjectHashMap<List<String>> segmentFilesByRecordingId = indexSegmentFiles(
//        archiveDir);
//
//      catalog.forEach(
//        (recordingDescriptorOffset, headerEncoder, headerDecoder, descriptorEncoder,
...
//          final List<String> files = segmentFilesByRecordingId.getOrDefault(recordingId,
//            emptyList());
...
//    try (Catalog catalog = openCatalogReadWrite(archiveDir, epochClock, MIN_CAPACITY, checksum,
//      null)) {
//      final MutableInteger errorCount = new MutableInteger();
//      catalog.forEach(createVerifyEntryProcessor(
//        out, archiveDir, options, catalog, checksum, epochClock, errorCount,
//        truncateOnPageStraddle));
//
...
//  }
//
//  public static Catalog openCatalogReadOnly(final File archiveDir, final EpochClock epochClock) {
...
//    return new Catalog(archiveDir, FileChannel.open(archiveDir.toPath()))
...
//    final Set<VerifyOption> options, final String checksumClassName) {
//    if (null == checksumClassName) {
//      if (options.contains(VerifyOption.APPLY_CHECKSUM)) {
//        throw new IllegalArgumentException(
//          "Checksum class name is required when " + VerifyOption.APPLY_CHECKSUM +
//            " option is specified!");
//      }
//
//      return null;
...
//    final ActionConfirmation<File> truncateOnPageStraddle) {
//    final ByteBuffer buffer = BufferUtil.allocateDirectAligned(FILE_IO_MAX_LENGTH_DEFAULT,
//      CACHE_LINE_LENGTH);
...
//      archiveDir);
//
//    return (recordingDescriptorOffset, headerEncoder, headerDecoder, descriptorEncoder,
// descriptorDecoder) ->
//      verifyRecording(
//        out,
//        archiveDir,
...
//    return readContinueAnswer("Specified frame limit " + frameLimit + " reached. Continue?
// (y/n)");
//  }
//
...
//    final File archiveDir, final Consumer<String> logger) {
//    return new ArchiveMarkFile(
//      resolveMarkFileDir(archiveDir), ArchiveMarkFile.FILENAME, INSTANCE,
//      TimeUnit.SECONDS.toMillis(5), logger);
//  }
//
//  private static ArchiveMarkFile openMarkFileReadWrite(
//    final File archiveDir, final EpochClock epochClock) {
//    return new ArchiveMarkFile(
...
//    out.println(
//      "  stream length: " + (NULL_POSITION == stopPosition ? NULL_POSITION : streamLength));
//    out.println(header);
...
//    final List<String> segmentFiles = segmentFileByRecordingId.getOrDefault(recordingId,
//      emptyList());
...
//          parseSegmentFilePosition(maxSegmentFile) + (segmentLength - 1);
...
// +
//            startPosition + " and/or stopPosition=" + stopPosition +
//            " exceed max segment file position=" +
//            maxSegmentPosition);
//          errorCount.increment();
//          headerEncoder.state(INVALID);
//          return;
...
//      out.println(
//        "(recordingId=" + recordingId + ") ERR: " + (null != message ? message : ex.toString()));
//      errorCount.increment();
//      headerEncoder.state(INVALID);
//      return;
//    }
//
//    final boolean applyChecksum = options.contains(VerifyOption.APPLY_CHECKSUM);
//    if (applyChecksum) {
...
//      if (options.contains(VerifyOption.VERIFY_ALL_SEGMENT_FILES)) {
...
//    final PrintStream out, final long recordingId, final long startPosition,
//    final long stopPosition) {
//    if (startPosition < 0) {
//      out.println("(recordingId=" + recordingId + ") ERR: Negative startPosition=" +
// startPosition);
//      return true;
//    } else if (isNotFrameAligned(startPosition)) {
//      out.println(
//        "(recordingId=" + recordingId + ") ERR: Non-aligned startPosition=" + startPosition);
//      return true;
...
//        out.println(
//          "(recordingId=" + recordingId + ") ERR: Non-aligned stopPosition=" + stopPosition);
//        return true;
//      }
//    }
//
...
//        }
//
//        final int termId = computeTermIdFromPosition(position, positionBitsToShift,
// initialTermId);
...
//            out.println(
//              "(recordingId=" + recordingId + ", file=" + file + ") ERR: checksum failed " +
...
//    } catch (final IOException ex) {
//      out.println(
//        "(recordingId=" + recordingId + ", file=" + file + ") ERR: failed to verify file");
//      ex.printStackTrace(out);
//      return true;
//    }
//
...
//
//    final MappedByteBuffer cncByteBuffer = IoUtil.mapExistingFile(cncFile,
//      FileChannel.MapMode.READ_ONLY, "cnc");
//    final DirectBuffer cncMetaDataBuffer = CncFileDescriptor.createMetaDataBuffer(cncByteBuffer);
//    final int cncVersion = cncMetaDataBuffer.getInt(CncFileDescriptor.cncVersionOffset(0));
//
//    CncFileDescriptor.checkVersion(cncVersion);
//    CommonContext.printErrorLog(
//      CncFileDescriptor.createErrorLogBuffer(cncByteBuffer, cncMetaDataBuffer), out);
//  }
//
...
//          checksum(buffer, out, archiveDir, segmentFilesByRecordingId, allFiles, checksum,
//            descriptorDecoder);
//        };
//
//      if (!catalog.forEntry(recordingId, catalogEntryProcessor)) {
...
//    final List<String> segmentFiles = segmentFilesByRecordingId.getOrDefault(recordingId,
//      emptyList());
//
//    if (allFiles) {
//      for (final String fileName : segmentFiles) {
...
//      checksumSegmentFile(buffer, out, archiveDir, checksum, recordingId, lastFile, startPosition,
//        termLength);
//    }
//  }
//
...
//            out.println(
//              "(recordingId=" + recordingId + ", file=" + file + ") ERR: failed to read " +
//                dataLength + " byte(s) of data at offset " + (fileOffset + HEADER_LENGTH));
//            return;
//          }
//
...
//            catalog.updateChecksum(recordingDescriptorOffset);
//            checksum(
//              buffer, out, archiveDir, segmentFilesByRecordingId, allFiles, checksum,
//              descriptorDecoder);
//          } catch (final Exception ex) {
//            out.println(
...
//        delete = offset < minBaseOffset || NULL_POSITION != maxBaseOffset && offset >
// maxBaseOffset;
...
//        "     unless flag '-a' is specified in which case all of the segment files are
// processed.%n%n" +
//        "  compact: compacts Catalog file by removing entries in state `INVALID` and deleting
// the%n" +
//        "     corresponding segment files.%n%n" +
//        "  count-entries: queries the number of `VALID` recording entries in the catalog.%n%n" +
//        "  delete-orphaned-segments: deletes orphaned recording segments that have been
// detached,%n" +
...
//        "  describe [recordingId]: prints out descriptor for the specified valid recording the
// catalog. " +
//        "(Excludes invalidated recordings.)%n%n" +
...
//        "  verify [recordingId] [-a] [-checksum className]: verifies descriptor(s) in the
// catalog%n" +
...
//        "     To perform checksum for each data frame specify the '-checksum' flag together
// with%n" +
