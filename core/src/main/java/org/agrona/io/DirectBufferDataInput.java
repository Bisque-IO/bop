 ...
 * {@link DataInput} description including throwing checked exception on end of file. It adds few
 * more methods to read strings without allocations.
 *
 * <p>Note about byte ordering: by default, this class conforms to {@link DataInput} contract and
 * uses {@link ByteOrder#BIG_ENDIAN} byte order which allows it to read data produced by JDK
 * {@link java.io.DataOutput} implementations. Agrona buffers use {@link ByteOrder#LITTLE_ENDIAN}
 * (unless overridden). Use {@link #byteOrder(ByteOrder)} method to switch between JDK and Agrona
 * compatibility.
 */
public class DirectBufferDataInput implements DataInput {
   ...
   * {@link ByteOrder#BIG_ENDIAN} which allows it to read data produced by JDK
   * {@link java.io.DataOutput} implementations. Agrona buffers use {@link ByteOrder#LITTLE_ENDIAN}
   * (unless overridden). Use this method to switch compatibility between these two worlds.
   *
  ...
  /** {@inheritDoc} */
  public void readFully(final byte[] destination, final int destinationOffset, final int length)
      throws EOFException {
    if (destination == null) {
      throw new NullPointerException("Destination must not be null");
    }
    ...
    if (destinationOffset + length > destination.length) {
      throw new IndexOutOfBoundsException("destinationOffset=" + destinationOffset + " length="
          + length + " not valid for length=" + length);
    }
   ...
   * byte separately into a character, until it encounters a line terminator or end of file; Adheres
   * to the contract of {@link DataInput#readLine()}.
   *
   * @param appendable to append the chars to.
   * @return number of bytes the stream advanced while reading the line (including line
   *     terminators).
   ...
   *
   * <p>This is a thin wrapper over {@link DirectBuffer#getStringUtf8(int, ByteOrder)}. Honours byte
   * order set by {@link #byteOrder(ByteOrder)}.
   *
   ...
   *
   * <p>This is a thin wrapper over {@link DirectBuffer#getStringAscii(int, ByteOrder)}. Honours
   * byte order set by {@link #byteOrder(ByteOrder)}
   *
   * @return the String as represented by the ASCII encoded bytes.
   */
   ...
   * {@link Appendable}. This is a thin wrapper over {@link DirectBuffer#getStringAscii(int,
   * Appendable, ByteOrder)}. Honours byte order set by {@link #byteOrder(ByteOrder)}.
   *
   * @param appendable to append the chars to.
  ...
  private void boundsCheck0(final int requestedReadBytes) throws EOFException {
    ...
    if (resultingPosition > length) {
      throw new EOFException("position=" + position + " requestedReadBytes=" + requestedReadBytes
          + " capacity=" + length);
    }
  }
