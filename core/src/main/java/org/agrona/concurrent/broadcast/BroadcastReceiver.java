...
/**
 * Receive messages broadcast from a {@link BroadcastTransmitter} via an underlying buffer.
 * Receivers can join a transmission stream at any point by consuming the latest message at the
 * point of joining and forward.
 *
 * <p>If a Receiver cannot keep up with the transmission stream then loss will be experienced. Loss
 * is not an error condition.
 *
 * <p><b>Note:</b> Each Receiver is not threadsafe but there can be zero or many receivers to a
 * transmission stream.
 */
public class BroadcastReceiver {
  ...
  /**
   * Construct a new broadcast receiver based on an underlying {@link AtomicBuffer}. The underlying
   * buffer must a power of 2 in size plus sufficient space for the
   * {@link org.agrona.concurrent.broadcast.BroadcastBufferDescriptor#TRAILER_LENGTH}.
   *
   ...
   *     {@link BroadcastBufferDescriptor#TRAILER_LENGTH} in capacity.
   */
   ...
   *
   * <p><b>Note:</b> This method is threadsafe for calling from an external monitoring thread.
   *
   * @return the capacity of the underlying broadcast buffer.
   */
   ...
   *
   * <p>If loss has occurred then {@link #lappedCount()} will be incremented.
   *
   * @return true if transmission is available with {@link #offset()}, {@link #length()} and
   *     {@link #typeId()} set for the next message to be consumed. If no transmission is available
   *     then false.
   */
   ...
   *
   * <p>If the receiver is not consuming messages fast enough to keep up with the transmitter then
   * loss can be experienced resulting in messages being overwritten thus making them no longer
   * valid.
   *
