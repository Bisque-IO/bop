...
/**
 * Atomic counter that is backed by an {@link AtomicBuffer} that can be read across threads and
 * processes.
 *
 * <p>In most cases you want to pair the appropriate methods for ordering. E.g.
 *
 * <ol>
 *   <li>an {@link #increment} (which has volatile semantics) should be combined with a
 *       {@link #get}.
 *   <li>an {@link #incrementRelease} with a {@link #getAcquire}.
 *   <li>an {@link #incrementOpaque} with a {@link #getOpaque}.
 *   <li>an {@link #incrementPlain} with a {@link #getPlain}.
 * </ol>
 */
public class AtomicCounter implements AutoCloseable {
  ...
  public AtomicCounter(
      final AtomicBuffer buffer, final int counterId, final CountersManager countersManager) {
    this.id = counterId;
  ...
  /**
   * Disconnect from {@link CountersManager} if allocated, so it can be closed without freeing the
   * slot.
   */
   ...
   * @param length length of the data within the keyBuffer (must be &lt;=
   *     {@link CountersReader#MAX_KEY_LENGTH})
   * @throws IllegalStateException is not constructed {@link CountersManager}.
   */
  ...
  public long increment() {
    return UnsafeApi.U.getAndAddLong(byteArray, addressOffset, 1);
  }

  /**
   * Perform an atomic increment that is not safe across threads.
   *
   * <p>This method is identical to {@link #incrementRelease()} and that method should be used
   * instead.
   *
   * @return the previous value of the counter
   */
   ...
   *
   * <p>It can result into lost updates due to race condition when called concurrently.
   *
   * <p>The load has plain memory semantics and the store has release memory semantics.
   *
   * <p>The typical use-case is when there is a single writer thread and one or more reader threads
   * and causality needs to be preserved using the {@link #getAcquire()}.
   *
   * <p>This method is likely to outperform the {@link #increment()}. So if there is just a single
   * mutator thread, and one or more reader threads, then it is likely you will prefer this method.
   *
   * <p>If no memory ordering is needed, have a look at the {@link #incrementOpaque()}.
   *
   * @return the previous value of the counter
   * @since 2.1.0
   */
  public long incrementRelease() {
    ...
    final long currentValue = UnsafeApi.U.getLong(array, offset);
    UnsafeApi.U.putLongRelease(array, offset, currentValue + 1);
    return currentValue;
  }
   ...
   * <p>It can result into lost updates due to race condition when called concurrently.
   *
   * <p>The load has plain memory semantics and the store has opaque memory semantics.
   *
   * <p>The typical use-case is when there is a single writer thread and one or more reader threads
   * and surrounding loads/stores don't need to be ordered.
   *
   * <p>This method should be at least fast as {@link #incrementRelease()} since it has weaker
   * memory semantics. So if there is just a single mutator thread, and one or more reader threads,
   * then it is likely you will prefer this method.
   *
   * @return the previous value of the counter
   * @since 2.1.0
   */
  public long incrementOpaque() {
    ...
    final long offset = addressOffset;
    final long currentValue = UnsafeApi.U.getLong(array, offset);
    UnsafeApi.U.putLongOpaque(array, offset, currentValue + 1);
    return currentValue;
  }
   ...
   *
   * <p>This method is not atomic and this can lead to lost-updates due to race conditions. This
   * load and store have plain memory semantics.
   *
   * <p>The typical use-case for this method is when writer and reader are the same thread.
   *
   * @return the previous value of the counter
   * @since 2.1.0
   */
  public long incrementPlain() {
    ...
    final long offset = addressOffset;
    final long currentValue = UnsafeApi.U.getLong(array, offset);
    UnsafeApi.U.putLong(array, offset, currentValue + 1);
    return currentValue;
  }
   ...
   *
   * <p>The loads and store have volatile memory semantics.
   *
   * @return the previous value of the counter
   */
   ...
   *
   * <p>It can result into lost updates to race condition when called concurrently.
   *
   * <p>The load has plain memory semantics and the store has release memory semantics.
   *
   * <p>The typical use-case is when there is one mutator thread, that calls this method, and one or
   * more reader threads.
   *
   * <p>This method is likely to outperform the {@link #increment()} and probably will be a better
   * alternative.
   *
   * @return the previous value of the counter
   * @since 2.1.0
   ...
   *
   * <p>The typical use-case is when there is a single writer thread and one or more reader threads
   * and surrounding loads and stores don't need to be ordered.
   *
   * <p>This method should be at least fast as {@link #incrementRelease()} since it has weaker
   * memory semantics. So if there is just a single mutator thread, and one or more reader threads,
   * then it is likely you will prefer this method.
   ...
   *
   * <p>This method is identical to {@link #setRelease(long)} and that method should be used
   * instead.
   *
   ...
   *
   * <p>The store has release memory semantics.
   *
   ...
   * Set the counter value atomically.
   *
   * <p>The store has opaque memory semantics.
   *
   * @param value to be set
   * @since 2.1.0
   */
   ...
   *
   * <p>This method is identical to {@link #setPlain(long)} and that method should be used instead.
   *
  ...
  public long getAndAdd(final long increment) {
    return UnsafeApi.U.getAndAddLong(byteArray, addressOffset, increment);
  }

  /**
   * Add an increment to the counter with ordered store semantics.
   *
   * <p>This method is identical to {@link #getAndAddRelease(long)} and that method should be used
   * instead.
   *
   ...
   *
   * <p>This method is not atomic; it can suffer from lost-updates due to race conditions.
   *
   * <p>The load has plain memory semantics and the store has release memory semantics.
   *
   * <p>The typical use-case is when there is one mutator thread, that calls this method, and one or
   * more reader threads. Typically, this method is combined with the {@link #getAcquire()} to read
   * the value.
   *
   * @param increment to be added
   * @return the previous value of the counter
  ...
  public long getAndAddRelease(final long increment) {
    ...
    final long offset = addressOffset;
    final long currentValue = UnsafeApi.U.getLong(array, offset);
    UnsafeApi.U.putLongRelease(array, offset, currentValue + increment);
   ...
   *
   * <p>The typical use-case is when there is one mutator thread, that calls this method, and one or
   * more reader threads. Typically, this method is combined with a {@link #getOpaque()} to read the
   * value.
   *
   * <p>If ordering of surrounding loads/stores isn't important, then this method is likely to be
   * faster than {@link #getAndAddRelease(long)} because it has less strict memory ordering
   * requirements.
   *
   * @param increment to be added
   * @return the previous value of the counter
  ...
  public long getAndAddOpaque(final long increment) {
    ...
    final long offset = addressOffset;
    final long currentValue = UnsafeApi.U.getLong(array, offset);
    UnsafeApi.U.putLongOpaque(array, offset, currentValue + increment);
   ...
   * <p>This method is not atomic; it can suffer from lost-updates due to race conditions.
   *
   * <p>The load has plain memory semantics and the store has plain memory semantics.
   *
   * <p>The typical use-case for this method is when writer and reader are the same thread.
   *
   * @param increment to be added
  ...
  public long getAndAddPlain(final long increment) {
    ...
    final long offset = addressOffset;
    final long currentValue = UnsafeApi.U.getLong(array, offset);
    UnsafeApi.U.putLong(array, offset, currentValue + increment);
    return currentValue;
  }
  ...
  public long getAndSet(final long value) {
    return UnsafeApi.U.getAndSetLong(byteArray, addressOffset, value);
  }
  ...
  public boolean compareAndSet(final long expectedValue, final long updateValue) {
    return UnsafeApi.U.compareAndSetLong(byteArray, addressOffset, expectedValue, updateValue);
  }
  ...
  /**
   * Get the value of the counter using weak ordering semantics. This is the same a standard read of
   * a field.
   *
   * <p>This call is identical to {@link #getPlain()} and that method is preferred.
   *
   * @return the value for the counter.
   */
  public long getWeak() {
  ...
  /**
   * Get the value of the counter using plain memory semantics. This is the same a standard read of
   * a field.
   *
   * @return the value for the counter.
   * @since 2.1.0
  ...
  /**
   * Set the value to a new proposedValue if greater than the current value with plain memory
   * semantics.
   *
  ...
  /**
   * Set the value to a new proposedValue if greater than the current value with memory ordering
   * semantics.
   *
   * <p>This method is identical to {@link #proposeMaxRelease(long)} and that method should be used
   * instead.
   *
   * @param proposedValue for the new max.
   * @return true if a new max as been set otherwise false.
   ...
   *
   * <p>This call is not atomic and can suffer from lost updates to race conditions.
   *
   * <p>The load has plain memory semantics and the store has release memory semantics.
   *
   * <p>The typical use-case is when there is one mutator thread, that calls this method, and one or
   ...
   * more reader threads.
   *
   * <p>This method is likely to outperform {@link #proposeMaxRelease(long)} since this method has
   * less memory ordering requirements.
   *
   * @param proposedValue for the new max.
   * @return true if a new max as been set otherwise false.
   * @since 2.1.0
  ...
  public String toString() {
    return "AtomicCounter{" + "isClosed="
        + isClosed() + ", id="
        + id + ", value="
        + (isClosed() ? -1 : get()) + ", countersManager="
        + countersManager + '}';
  }
}
