 ...
 *
 * <p>Threadsafe to write to from a single writer unless methods with plain memory semantics are
 * used.
 */
public abstract class Position extends ReadablePosition {
  ...
  /**
   * Set the position to a new proposedValue if greater than the current value with plain memory
   * semantics.
   *
   ...
   * semantics.
   *
   * <p>This method is identical to {@link #proposeMaxRelease(long)} and that method should be
   * preferred instead.
   *
   * @param proposedValue for the new max.
   * @return true if a new max as been set otherwise false.
