...
/**
 * Agent runner containing an {@link org.agrona.concurrent.Agent} which is run on a {@link Thread}.
 *
 * <p><b>Note:</b> An instance should only be started once and then discarded, it should not be
 * reused.
 */
public class AgentRunner implements Runnable, AutoCloseable {
  ...
  public AgentRunner(
      ...
      final org.agrona.concurrent.Agent agent) {
  ...
  /**
   * The {@link org.agrona.concurrent.Agent} which is contained.
   *
   * @return {@link org.agrona.concurrent.Agent} being contained.
   */
  ...
  /**
   * Has the {@link org.agrona.concurrent.Agent} been closed?
   *
   * @return has the {@link org.agrona.concurrent.Agent} been closed?
   */
  ...
  /**
   * Get the thread which is running that {@link org.agrona.concurrent.Agent}.
   *
   * <p>If null then the runner has not been started. If {@link #TOMBSTONE} then the runner is being
   * closed.
   *
   * @return the thread running the {@link org.agrona.concurrent.Agent}.
   */
  ...
  /**
   * Run an {@link org.agrona.concurrent.Agent}.
   *
   * <p>This method does not return until the run loop is stopped via {@link #close()}.
   */
  public void run() {
    try {
   ...
   *
   * <p>This is equivalent to calling {@link AgentRunner#close(int, Consumer)} using the default
   * {@link AgentRunner#RETRY_CLOSE_TIMEOUT_MS} value and a null action.
   */
   ...
   * Stop the running Agent and cleanup.
   *
   * <p>This will wait for the work loop to exit. The close timeout parameter controls how long we
   * should wait before retrying to stop the agent by interrupting the thread. If the calling thread
   * has its interrupt flag set then this method can return early before waiting for the running
   * agent to close.
   *
   * <p>An optional action can be invoked whenever we time out while waiting which accepts the agent
   * runner thread as the parameter (e.g. to obtain and log a stack trace from the thread). If the
   * action is null, a message is written to stderr. Please note that a retry close timeout of zero
  ...
  private void failAction(
      final Consumer<Thread> closeFailAction, final Thread thread, final String message) {
