...
abstract class ManyToOneConcurrentLinkedQueuePadding1 {
  ...
  static final class Node<E> {
    ...
    void nextOrdered(final Node<E> next) {
      UnsafeApi.U.putReferenceRelease(this, NEXT_OFFSET, next);
    }
  }

  static {
    try {
      HEAD_OFFSET = UnsafeApi.U.objectFieldOffset(
          ManyToOneConcurrentLinkedQueueHead.class.getDeclaredField("head"));
      TAIL_OFFSET = UnsafeApi.U.objectFieldOffset(
          ManyToOneConcurrentLinkedQueueTail.class.getDeclaredField("tail"));
      NEXT_OFFSET = UnsafeApi.U.objectFieldOffset(Node.class.getDeclaredField("next"));
 ...
 */
abstract class ManyToOneConcurrentLinkedQueueTail<E>
...
public class ManyToOneConcurrentLinkedQueue<E> extends ManyToOneConcurrentLinkedQueueHead<E>
    implements Queue<E> {
  byte p128,
      p129,
      p130,
  ...
  public ManyToOneConcurrentLinkedQueue() {
    headOrdered(empty);
    UnsafeApi.U.putReferenceRelease(this, TAIL_OFFSET, empty);
  }
  ...
  private void headOrdered(final Node<E> head) {
    UnsafeApi.U.putReferenceRelease(this, HEAD_OFFSET, head);
  }

  @SuppressWarnings("unchecked")
  private Node<E> swapTail(final Node<E> newTail) {
    return (Node<E>) UnsafeApi.U.getAndSetReference(this, TAIL_OFFSET, newTail);
  }

  private boolean casTail(final Node<E> expectedNode, final Node<E> updateNode) {
    return UnsafeApi.U.compareAndSetReference(this, TAIL_OFFSET, expectedNode, updateNode);
  }
}
