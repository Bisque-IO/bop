...
/**
 * An {@link ErrorHandler} which calls {@link AtomicCounter#increment()} before delegating the
 * exception.
 */
public class CountedErrorHandler implements ErrorHandler, AutoCloseable {
  ...
  /**
   * Close so that {@link #onError(Throwable)} will not delegate and instead print to
   * {@link System#err}.
   */
  public void close() {
    isClosed = true;
