...
/**
 * Expandable {@link MutableDirectBuffer} that is backed by an array. When values are put into the
 * buffer beyond its current length, then it will be expanded to accommodate the resulting position
 * for the value.
 *
 * <p>Put operations will expand the capacity as necessary up to {@link #MAX_ARRAY_LENGTH}. Get
...
public class ExpandableArrayBuffer extends AbstractMutableDirectBuffer {
  /**
   * Maximum length to which the underlying buffer can grow. Some JVMs store state in the last few
   * bytes.
   */
  ...
  public String toString() {
    return "ExpandableArrayBuffer{" + ", capacity="
        + capacity + ", byteArray="
        + (null == byteArray ? "null" : ("byte[" + byteArray.length + "]")) + '}';
  }
