...
/**
 * {@link Map} implementation specialised for int values using open addressing and linear probing
 * for cache efficient access. The implementation is mirror copy of {@link Int2ObjectHashMap} and it
 * also relies on missing value concept from {@link Int2IntHashMap}.
 *
...
public class Object2IntHashMap<K> implements Map<K, Integer> {
  ...
  public Object2IntHashMap(final int missingValue) {
    this(MIN_CAPACITY, org.agrona.collections.Hashing.DEFAULT_LOAD_FACTOR, missingValue);
  }
  ...
  public Object2IntHashMap(
      @DoNotSub final int initialCapacity, final float loadFactor, final int missingValue) {
  ...
  public Object2IntHashMap(
    ...
    /* @DoNotSub */ final int capacity =
        findNextPositivePowerOfTwo(Math.max(MIN_CAPACITY, initialCapacity));
  ...
  /**
   * {@inheritDoc} Overloaded version of {@link Map#containsKey(Object)} that takes a primitive int
   * key.
   *
  ...
  /**
   * Overloaded version of {@link Map#get(Object)} that takes a primitive int key. Due to type
   * erasure have to rename the method.
   *
   * @param key for indexing the {@link Map}.
  ...
  public int getValue(final K key) {
    ...
    @DoNotSub int index = org.agrona.collections.Hashing.hash(key, mask);
   ...
   *
   * <p>If the function returns missingValue, the mapping is removed.
   *
  ...
  public int computeIfPresent(
      final K key,
      final org.agrona.collections.ObjectIntToIntFunction<? super K> remappingFunction) {
    requireNonNull(key);
    final int missingValue = this.missingValue;
    final K[] keys = this.keys;
    final int[] values = this.values;
   ...
   * missingValue if there is no current mapping).
   *
   * <p>If the function returns missingValue, the mapping is removed (or remains absent if initially
   * absent).
   *
   ...
   * Otherwise, replaces the associated value with the results of the given remapping function, or
   * removes if the result is {@link #missingValue()}. This method may be of use when combining
   * multiple mapped values for a key. If the function returns {@link #missingValue()} the mapping
   * is removed.
   *
  ...
  /**
   * If the specified key is not already associated with a value associates it with the given value
   * and returns {@link #missingValue()}, else returns the current value.
   *
   * @param key with which the specified value is to be associated.
   * @param value to be associated with the specified key.
   * @return the existing value associated with the specified key, or {@link #missingValue()} if
  ...
  /**
   * Overloaded version of {@link Map#remove(Object)} that takes a key and returns a primitive int
   * value. Due to type erasure have to rename the method.
   *
   * @param key for indexing the {@link Map}.
   * @return the value if found otherwise missingValue.
   */
  ...
  public final class EntryIterator extends AbstractIterator<Entry<K, Integer>>
