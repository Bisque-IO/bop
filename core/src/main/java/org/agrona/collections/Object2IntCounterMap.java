...
/**
 * An open-addressing with linear probing hash map specialised for object and primitive counter
 * pairs. A counter map views counters which hit {@link #initialValue} as deleted. This means that
 * changing a counter may impact {@link #size()}.
 *
...
public class Object2IntCounterMap<K> {
   ...
   * @return the new value associated with the specified key, or {@link #initialValue()} + amount if
   ...
   * @param key new or existing.
   * @param amount to be added.
   * @return the previous value associated with the specified key, or {@link #initialValue()} if
   *     there was no mapping for the key.
   */
