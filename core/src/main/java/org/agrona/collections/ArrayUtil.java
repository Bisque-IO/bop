...
/**
 * Utility class for operating on arrays as if they were collections. This is useful for critical
 * paths where operations like add and remove are seldom used, but iterating is common and checkcast
 * and indirection are comparatively expensive.
 *
 * <p>In all cases the array being mutated is assumed to be full.
 *
 * <p>In all cases reference equality is used.
 */
public final class ArrayUtil {
  ...
  /**
   * Remove an element from an array resulting in a new array if the element was found otherwise the
   * old array.
   *
   * <p>Returns its input parameter if the element to remove isn't a member.
   *
  ...
  /**
   * Remove an element from an array resulting in a new array if the index was inside the array
   * otherwise the old array.
   *
   * <p>Returns the old elements array if the index isn't inside the array.
   *
   * @param oldElements to have the element removed from.
   * @param index to remove the element at.
   * @param <T> type of the array.
   * @return a new array without the element if the index is inside the array otherwise the original
   *     array.
   */
