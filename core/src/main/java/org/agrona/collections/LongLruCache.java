...
/**
 * A fixed capacity cache of long keyed values that evicts the least-recently-used element when it
 * runs out of space.
 *
 * <p>When an element is evicted it is closed by calling the closer function with the element as an
 * argument.
 *
 * <p>When a new key arrives the factory function is called in order to create the new element
...
public final class LongLruCache<E> implements AutoCloseable {
   ...
   */
  public LongLruCache(
      @DoNotSub final int capacity, final LongFunction<E> factory, final Consumer<E> closer) {
  ...
  /**
   * Looks up an element in the cache, creating a new element if it doesn't exist and evicting the
   * least recently used element if there's no space left in the cache.
   *
   * @param key the key to look up the element by.
   * @return the element associated with this key.
   */
  @SuppressWarnings("unchecked")
  public E lookup(final long key) {
  ...
  private void makeMostRecent(final long key, final Object value, @DoNotSub final int fromIndex) {
    final long[] keys = this.keys;
    final Object[] values = this.values;
