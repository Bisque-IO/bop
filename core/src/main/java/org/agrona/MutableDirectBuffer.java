...
/**
 * Abstraction over a range of buffer types that allows fields to be written in native typed
 * fashion.
 *
 * <p>{@link ByteOrder} of a wrapped buffer is not applied to the {@link MutableDirectBuffer}. To
 * control {@link ByteOrder} use the appropriate method with a {@link ByteOrder} overload.
 */
public interface MutableDirectBuffer extends org.agrona.DirectBuffer {
  /**
   ...
   *
   * <p>If ascii encoding of the number is less than the specified length then the start will be
   * pre-padded with zeros, if the value takes up more space than the allowed length then a <code>
   * {@link NumberFormatException}</code> will be thrown.
   *
   ...
   *
   * <p>The source buffer will have its {@link ByteBuffer#position()} advanced as a result.
   *
   * @param index in the underlying buffer to start from.
  ...
  /**
   * Put bytes into the underlying buffer for the view. Bytes will be copied from the buffer index
   * to the buffer index + length.
   *
   * <p>The source buffer will not have its {@link ByteBuffer#position()} advanced as a result.
   *
   * @param index in the underlying buffer to start from.
  ...
  /**
   * Put bytes from a source {@link org.agrona.DirectBuffer} into this {@link MutableDirectBuffer}
   * at given indices.
   *
  ...
  /**
   * Encode a {@link String} as ASCII bytes in the buffer without a length prefix taking a range of
   * the value.
   *
   * @param index at which the String begins.
   * @param value of the String to be encoded.
   * @param valueOffset in the value String to begin.
   * @param length of the value String to encode. If this is greater than valueOffset - value length
   *     then the lesser will be used.
   * @return the number of bytes encoded.
   */
  int putStringWithoutLengthAscii(int index, String value, int valueOffset, int length);

  /**
   * Encode a {@link CharSequence} as ASCII bytes in the buffer without a length prefix taking a
   * range of the value.
   *
   * @param index at which the CharSequence begins.
   * @param value of the CharSequence to be encoded.
   * @param valueOffset in the value CharSequence to begin.
   * @param length of the value CharSequence to encode. If this is greater than valueOffset - value
   *     length then the lesser will be used.
   * @return the number of bytes encoded.
   */
  ...
  /**
   * Encode a {@link String} as UTF-8 bytes the buffer with a length prefix with a maximum encoded
   * size check.
   *
   * @param index at which the String should be encoded.
   * @param value of the String to be encoded.
